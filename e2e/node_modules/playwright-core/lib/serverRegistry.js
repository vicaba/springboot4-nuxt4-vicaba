...
var require_readdirp = __commonJS({
  "node_modules/readdirp/index.js"(exports2, module2) {
    ...
    var [maj, min] = process.versions.node
      .split(".")
      .slice(0, 2)
      .map((n) => Number.parseInt(n, 10));
    var wantBigintFsStats =
        ...
        try {
          while (!this.destroyed && batch > 0) {
            ...
            if (files.length > 0) {
              const slice = files
                .splice(0, batch)
                .map((dirent) => this._formatEntry(dirent, path3));
...
var require_is_binary_path = __commonJS({
  "node_modules/is-binary-path/index.js"(exports2, module2) {
    ...
    module2.exports = (filePath) =>
      extensions.has(path3.extname(filePath).slice(1).toLowerCase());
  },
});
        ...
        let stream = this.fsw
          ...
          .on(STR_DATA, async (entry) => {
            ...
            if (
              ...
              (await this._handleSymlink(entry, directory, path3, item))
            ) {
              return;
            }
            if (this.fsw.closed) {
...
var require_chokidar = __commonJS({
  "node_modules/chokidar/index.js"(exports2) {
    ...
    var normalizePathToUnix = (path3) =>
      toUnix(sysPath.normalize(toUnix(path3)));
    var normalizeIgnored =
      (cwd = EMPTY_STR) =>
      (path3) => {
        if (typeof path3 !== STRING_TYPE) return path3;
        return normalizePathToUnix(
          sysPath.isAbsolute(path3) ? path3 : sysPath.join(cwd, path3),
        );
      };
    ...
    var WatchHelper = class {
      constructor(path3, watchPath, follow, fsw) {
        ...
        this.globFilter = this.hasGlob
          ? anymatch(path3, void 0, ANYMATCH_OPTS)
          : false;
      ...
      getDirParts(path3) {
        ...
        const parts = [];
        const expandedPath = path3.includes(BRACE_START)
          ? braces.expand(path3)
          : [path3];
        expandedPath.forEach((path4) => {
          parts.push(
            sysPath
              .relative(this.watchPath, path4)
    ...
    var FSWatcher = class extends EventEmitter2 {
      ...
      _isIgnored(path3, stats) {
        ...
        if (!this._userIgnored) {
          ...
          const paths = arrify(ignored)
            .filter((path4) => typeof path4 === STRING_TYPE && !isGlob(path4))
            .map((path4) => path4 + SLASH_GLOBSTAR);
      ...
      _getWatchHelpers(path3, depth) {
        const watchPath =
          depth || this.options.disableGlobbing || !isGlob(path3)
            ? path3
            : globParent(path3);
      ...
      _remove(directory, item, isDirectory) {
        ...
        isDirectory =
          isDirectory != null
            ? isDirectory
            : this._watched.has(path3) || this._watched.has(fullPath);
        ...
        nestedDirectoryChildren.forEach((nested) =>
          this._remove(path3, nested),
        );
...
var packageJSON = require(
  import_path.default.join(packageRoot, "package.json"),
);
...
var ServerRegistry = class extends import_events.EventEmitter {
  ...
  watch() {
    ...
    return () => {
      ...
      disposed = true;
      this._watcherRefs--;
      if (this._watcherRefs === 0) this._stopWatcher();
    };
  }
  ready() {
  ...
  async list() {
    ...
    let dispose;
    if (ownWatcher) dispose = this.watch();
    try {
      await this._ready;
  ...
  async create(browser, endpoint) {
    ...
    await import_fs.default.promises.mkdir(this._browsersDir(), {
      recursive: true,
    });
    const descriptor = {
  ...
  async deleteUserData(guid) {
    ...
    if (descriptor.browser.userDataDir)
      await import_fs.default.promises.rm(descriptor.browser.userDataDir, {
        recursive: true,
        force: true,
      });
  ...
  async find(name) {
    ...
    for (const [, browsers] of entries) {
      for (const browser of browsers) {
        if (browser.title === name) return browser;
      }
    }
    return null;
  }
  ...
  _startWatcher() {
    ...
    watcher.on("unlink", (file) => {
      ...
      if (this._descriptors.delete(guid)) this.emit("removed", guid);
    });
  }
...
async function canConnectTo(descriptor) {
  ...
  if (
    descriptor.endpoint.startsWith("ws://") ||
    descriptor.endpoint.startsWith("wss://")
  ) {
  ...
  return await new Promise((resolve) => {
    const socket = import_net.default.createConnection(
      descriptor.endpoint ?? descriptor.pipeName,
      () => {
        socket.destroy();
        resolve(true);
      },
...
var defaultCacheDirectory = (() => {
  if (process.platform === "linux")
    return (
      ...
      import_path2.default.join(import_os.default.homedir(), ".cache")
    );
  ...
  if (process.platform === "win32")
    return (
      ...
      import_path2.default.join(import_os.default.homedir(), "AppData", "Local")
    );
  throw new Error("Unsupported platform: " + process.platform);
})();
var registryDirectory = import_path2.default.join(
