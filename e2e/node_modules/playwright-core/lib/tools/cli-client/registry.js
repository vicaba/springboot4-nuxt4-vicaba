...
class Registry {
  ...
  async loadEntry(clientInfo, sessionName) {
    const entry = await Registry._loadSessionEntry(
      clientInfo.daemonProfilesDir,
      sessionName + ".session",
    );
    if (!entry) throw new Error(`Could not start the session "${sessionName}"`);
    const key = clientKey(clientInfo);
  ...
  static async load() {
    ...
    const hashDirs = await import_fs.default.promises
      .readdir(baseDaemonDir)
      .catch(() => []);
    for (const workspaceDirHash of hashDirs) {
      const daemonDir = import_path.default.join(
        baseDaemonDir,
        workspaceDirHash,
      );
      const stat = await import_fs.default.promises.stat(daemonDir);
      if (!stat.isDirectory()) continue;
      const files = await import_fs.default.promises
        .readdir(daemonDir)
        .catch(() => []);
...
const baseDaemonDir = (() => {
  ...
  if (process.platform === "linux")
    localCacheDir =
      ...
      import_path.default.join(import_os.default.homedir(), ".cache");
  if (process.platform === "darwin")
    localCacheDir = import_path.default.join(
  ...
  if (process.platform === "win32")
    localCacheDir =
      ...
      import_path.default.join(import_os.default.homedir(), "AppData", "Local");
  if (!localCacheDir)
...
function createClientInfo() {
  ...
  const version =
    ...
    import_package.packageJSON.version;
...
function findWorkspaceDir(startDir) {
  ...
  for (let i = 0; i < 10; i++) {
    if (
      import_fs.default.existsSync(import_path.default.join(dir, ".playwright"))
    )
      return dir;
