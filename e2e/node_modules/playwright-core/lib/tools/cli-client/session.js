...
class Session {
  ...
  isCompatible(clientInfo) {
    return (
      (0, import_socketConnection.compareSemver)(
        clientInfo.version,
        this.config.version,
      ) >= 0
    );
...
to start the browser session.`);
    return await SocketConnectionClient.sendAndClose(socket, "run", {
      args,
      cwd: process.cwd(),
      raw: options?.raw,
      json: options?.json,
  ...
  async stop() {
    if (!(await this.canConnect())) return { wasOpen: false };
    await this._stopDaemon();
    return { wasOpen: true };
  }
  async deleteData() {
    await this.stop();
    const dataDirs = await import_fs.default.promises
      .readdir(this._sessionFile.daemonDir)
      .catch(() => []);
    const matchingEntries = dataDirs.filter(
      (file) =>
        file === `${this.name}.session` || file.startsWith(`ud-${this.name}-`),
    );
    if (matchingEntries.length === 0)
      return { existed: false, deletedUserDataDir: false };
    let deletedUserDataDir = false;
    for (const entry of matchingEntries) {
      const userDataDir = import_path.default.resolve(
        this._sessionFile.daemonDir,
        entry,
      );
      for (let i = 0; i < 5; i++) {
        try {
          await import_fs.default.promises.rm(userDataDir, { recursive: true });
          if (entry.startsWith("ud-")) deletedUserDataDir = true;
          break;
        } catch (e) {
    ...
    return await new Promise((resolve) => {
      ...
      socket.on("error", (error) => {
        if (import_os.default.platform() !== "win32")
          void import_fs.default.promises
  ...
  static async startDaemon(clientInfo, cliArgs, mode) {
    await import_fs.default.promises.mkdir(clientInfo.daemonProfilesDir, {
      recursive: true,
    });
    const cliPath = (0, import_package.libPath)("entry", "cliDaemon.js");
    const sessionName = (0, import_registry.resolveSessionName)(
      cliArgs.session,
    );
    const errLog = import_path.default.join(
      clientInfo.daemonProfilesDir,
      sessionName + ".err",
    );
    const err = import_fs.default.openSync(errLog, "w");
    const args = [cliPath, sessionName];
    if (cliArgs.headed) args.push("--headed");
    if (cliArgs.mobile) args.push("--mobile");
    if (cliArgs.device) args.push(`--device=${cliArgs.device}`);
    if (cliArgs.browser) args.push(`--browser=${cliArgs.browser}`);
    if (cliArgs.persistent) args.push("--persistent");
    if (cliArgs.profile) args.push(`--profile=${cliArgs.profile}`);
    if (cliArgs.config) args.push(`--config=${cliArgs.config}`);
    if (cliArgs.extension) args.push("--extension");
    else if (cliArgs.cdp) args.push(`--cdp=${cliArgs.cdp}`);
    else if (cliArgs.endpoint) args.push(`--endpoint=${cliArgs.endpoint}`);
    ...
    let outLog = "";
    const rejectWithPid = (reject, message) =>
      reject(
        Object.assign(new Error(`Daemon pid=${child.pid}: ${message}`), {
          daemonPid: child.pid,
        }),
      );
    await new Promise((resolve, reject) => {
      child.stdout.on("data", (data) => {
        ...
        if (outLog.includes("Daemon listening on")) resolve();
      });
      child.on("close", (code) => {
        if (!signalled) {
  ...
  async _stopDaemon() {
    ...
    if (!socket) return;
    let error;
    await SocketConnectionClient.sendAndClose(socket, "stop", {}).catch(
      (e) => (error = e),
    );
    if (error && !error?.message?.includes("Session closed")) throw error;
  }
...
class SocketConnectionClient {
  ...
  _onMessage(object) {
    if (object.id && this._callbacks.has(object.id)) {
      ...
      if (object.error) callback.reject(new Error(object.error));
      else callback.resolve(object.result);
    } else if (object.id) {
      throw new Error(`Unexpected message id: ${object.id}`);
    } else {
      throw new Error(
        `Unexpected message without id: ${JSON.stringify(object)}`,
      );
    }
  }
  _rejectCallbacks() {
