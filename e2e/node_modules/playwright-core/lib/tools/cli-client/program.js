...
const globalOptions = ["json", "raw", "session"];
const booleanOptions = ["all", "help", "json", "raw", "version"];
async function program(options) {
  const clientInfo = (0, import_registry.createClientInfo)();
  const help = require(
    (0, import_package.libPath)("tools", "cli-client", "help.json"),
  );
  ...
  const output = args.json
    ? new import_output.JsonOutput()
    : new import_output.TextOutput();
  ...
  if (args.help || args.h || !commandName) {
    if (command) {
      ...
      if (process.env.CLAUDECODE || process.env.COPILOT_CLI)
        lines.push(
          `Agent skill: ${import_path.default.relative(process.cwd(), (0, import_package.libPath)("tools", "skills", "playwright-cli", "SKILL.md"))}`,
        );
  ...
  if (!command) output.errorUnknownCommand(commandName, help.global);
  ...
  switch (commandName) {
    ...
    case "delete-data": {
      ...
      if (!entry) {
        output.deleteData(sessionName, {
          existed: false,
          deletedUserDataDir: false,
        });
        return;
      }
    ...
    case "open": {
      ...
      const toolText = await runInSessionOrStop(
        newEntry,
        clientInfo,
        { _: ["goto", ...(params.length ? params : ["about:blank"])] },
        output,
      );
    ...
    case "attach": {
      ...
      const targetCount =
        ...
        (args.endpoint ? 1 : 0) +
        (args.extension ? 1 : 0);
      if (targetCount > 1) output.errorAttachConflict();
      if (attachTarget) args.endpoint = attachTarget;
      const extensionChannel =
        typeof args.extension === "string" ? args.extension : void 0;
      if (extensionChannel) {
        args.browser = extensionChannel;
        args.extension = true;
      }
      const cdpChannel =
        typeof args.cdp === "string" &&
        (0, import_channelSessions.isKnownChannel)(args.cdp)
          ? args.cdp
          : void 0;
      const targetName =
        attachTarget ??
        ...
        args.endpoint ??
        args.cdp;
      if (!targetName) output.errorAttachNoTarget();
      const attachSessionName =
        (0, import_registry.explicitSessionName)(args.session) ??
        attachTarget ??
        cdpChannel ??
        extensionChannel ??
        sessionName;
    ...
    case "close": {
      ...
      const { wasOpen } = closeEntry
        ? await new import_session.Session(closeEntry).stop()
        : { wasOpen: false };
    ...
    case "detach": {
      ...
      const { wasOpen } = detachEntry
        ? await new import_session.Session(detachEntry).stop()
        : { wasOpen: false };
    ...
    case "install-browser":
      ...
      return;
    case "show": {
      const daemonScript = (0, import_package.libPath)(
        "entry",
        "dashboardApp.js",
      );
      const daemonArgs = [
      ...
      if (explicit) daemonArgs.push(`--sessionName=${explicit}`);
      if (args.port !== void 0) daemonArgs.push(`--port=${args.port}`);
      if (args.host !== void 0) daemonArgs.push(`--host=${args.host}`);
      if (args.kill) {
        daemonArgs.push(`--kill`);
        const child2 = (0, import_child_process.spawn)(
          process.execPath,
          daemonArgs,
          { stdio: "ignore" },
        );
      ...
      if (args.annotate) {
        const entry = registry.entry(clientInfo, sessionName);
        if (!entry) output.errorBrowserNotOpenForTool(sessionName);
        args.raw = true;
      ...
      try {
        await new Promise((resolve, reject) => {
          ...
          child.once("exit", (code, signal) =>
            reject(
              new Error(
                `Dashboard daemon exited (code=${code}, signal=${signal}) before signaling READY${outLog ? "\n" + outLog : ""}`,
              ),
            ),
          );
        });
...
async function startSession(sessionName, registry, clientInfo, args, mode) {
  const entry = registry.entry(clientInfo, sessionName);
  if (entry) await new import_session.Session(entry).stop();
...
async function runInSession(entry, clientInfo, args, output) {
  const raw = !!args.raw;
  for (const globalOption of globalOptions) delete args[globalOption];
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) resolve();
        ...
        reject(
          new Error(`Workspace initialization failed with exit code ${code}`),
        );
    });
  });
}
async function installBrowser() {
  ...
  if (!program2.version()) libCli.decorateProgram(program2);
  program2.parse(argv);
}
const daemonProcessPatterns = [
  "run-mcp-server",
  "run-cli-server",
  "cli-daemon",
  "cliDaemon.js",
...
async function killAllDaemons() {
  ...
  try {
    if (platform === "win32") {
      const clauses = [
        `(${daemonProcessPatterns.map((p) => `$_.CommandLine -like '*${p}*'`).join(" -or ")})`,
      ];
      if (pidFilter)
        clauses.push(
          `(${[...pidFilter].map((p) => `$_.ProcessId -eq ${p}`).join(" -or ")})`,
        );
      ...
      for (const pid of pids) killed.push(parseInt(pid, 10));
    } else {
      const result = (0, import_child_process.execSync)("ps auxww", {
        encoding: "utf-8",
      });
      ...
      for (const line of lines) {
        if (daemonProcessPatterns.some((p) => line.includes(p))) {
          ...
          if (pid && /^\d+$/.test(pid)) {
            ...
            if (pidFilter && !pidFilter.has(numericPid)) continue;
            try {
...
async function collectList(registry, clientInfo, all) {
  ...
  for (const [workspaceKey, list] of entries) {
    if (!all && workspaceKey !== key) continue;
  ...
  if (!all) return { all, browsers };
  const servers = [...serverEntries.values()].flat();
  return {
    all,
    browsers,
    servers,
    channelSessions: await (0, import_channelSessions.listChannelSessions)(),
  };
}
function validateFlags(args, command, output) {
  ...
  for (const key of Object.keys(args)) {
    ...
    if (!(key in command.flags)) unknownFlags.push(key);
  }
