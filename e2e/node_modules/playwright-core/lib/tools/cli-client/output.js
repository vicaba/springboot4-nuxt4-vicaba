...
`);
  ...
  errorUnknownOption(opts, commandHelp) {
    console.error(
      `Unknown option${opts.length > 1 ? "s" : ""}: ${opts.map((f) => `--${f}`).join(", ")}`,
    );
    console.log("");
  ...
  errorTooManyArguments(expected, received, commandHelp) {
    console.error(
      `error: too many arguments: expected ${expected}, received ${received}`,
    );
    console.log("");
    console.log(commandHelp);
    return process.exit(1);
  ...
  errorAttachConflict() {
    console.error(
      `Error: only one of [name], --cdp, --endpoint, or --extension can be specified`,
    );
    return process.exit(1);
  }
  errorDetachNotAttached(session) {
    console.error(
      `Error: session '${session}' was not attached; use \`playwright-cli${session !== "default" ? ` -s=${session}` : ""} close\` to stop it.`,
    );
    return process.exit(1);
  }
  ...
  list({ all, browsers, servers, channelSessions }) {
    ...
    for (const [workspaceKey, list] of byWorkspace) {
      if (count === 0) console.log("### Browsers");
      if (all)
        console.log(
          `${import_path.default.relative(process.cwd(), workspaceKey) || "/"}:`,
        );
      for (const browser of list) console.log(renderBrowser(browser));
      count += list.length;
    }
    if (!all) {
      if (!count) console.log("  (no browsers)");
      return;
    }
    if (servers?.length) {
      ...
      for (const [workspaceKey, list] of serversByWorkspace) {
        ...
        for (const server of list) console.log(renderServer(server));
      }
  ...
  killAll(pids) {
    for (const pid of pids) console.log(`Killed daemon process ${pid}`);
    if (pids.length === 0) console.log("No daemon processes found.");
    else
      console.log(
        `Killed ${pids.length} daemon process${pids.length === 1 ? "" : "es"}.`,
      );
  }
  open(session, pid, toolResult) {
    console.log(`### Browser \`${session}\` opened with pid ${pid}.`);
    if (toolResult) console.log(toolResult);
  }
  attach(session, pid, endpoint, toolResult) {
    if (endpoint) {
      console.log(
        `### Session \`${session}\` created, attached to \`${endpoint}\`.`,
      );
...
class JsonOutput {
  ...
  errorUnknownOption(opts, _commandHelp) {
    this._emit({
      isError: true,
      error: `Unknown option${opts.length > 1 ? "s" : ""}: ${opts.map((f) => `--${f}`).join(", ")}`,
    });
    return process.exit(1);
  }
  errorTooManyArguments(expected, received, _commandHelp) {
    this._emit({
      isError: true,
      error: `error: too many arguments: expected ${expected}, received ${received}`,
    });
    return process.exit(1);
  }
  errorAttachConflict() {
  ...
  errorDetachNotAttached(session) {
    this._emit({
      isError: true,
      error: `session '${session}' was not attached; use close to stop it.`,
    });
    return process.exit(1);
  }
  errorBrowserNotOpenForTool(session) {
...
function renderBrowser(browser) {
  ...
  if (browser.status === "open" && !browser.compatible)
    lines.push(
      `  - version: v${browser.version} [incompatible please re-open]`,
    );
  if (browser.browserType)
    lines.push(
      `  - browser-type: ${browser.browserType}${browser.attached ? " (attached)" : ""}`,
    );
  if (!browser.attached) {
    if (browser.userDataDir === null)
      lines.push(`  - user-data-dir: <in-memory>`);
    else lines.push(`  - user-data-dir: ${browser.userDataDir}`);
    if (browser.headed !== void 0) lines.push(`  - headed: ${browser.headed}`);
  }
  return lines.join("\n");
}
function renderServer(server) {
  ...
  else lines.push(`  - data-dir: <in-memory>`);
...
function renderChannelSession(session) {
  ...
  if (session.extensionInstalled)
    lines.push(
      `  - attach (extension): \`playwright-cli attach --extension=${session.channel}\``,
    );
  else
    lines.push(
      `  - attach (extension): install at ${import_extension.playwrightExtensionInstallUrl}`,
    );
  if (session.endpoint) {
    lines.push(
      `  - attach (remote debugging): \`playwright-cli attach --cdp=${session.channel}\``,
    );
  } else {
    const inspectScheme = session.channel.startsWith("msedge")
      ? "edge"
      : "chrome";
    lines.push(
      `  - attach (remote debugging): enable at ${inspectScheme}://inspect/#remote-debugging`,
    );
  }
  return lines.join("\n");
}
