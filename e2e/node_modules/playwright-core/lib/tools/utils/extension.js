...
function profileRank(profile) {
  return profile === "Default"
    ? -1
    : parseInt(profile.slice("Profile ".length), 10);
}
async function readLastUsedProfile(userDataDir) {
  try {
    const localState = JSON.parse(
      await import_fs.default.promises.readFile(
        import_path.default.join(userDataDir, "Local State"),
        "utf-8",
      ),
    );
...
async function isExtensionInstalledInProfile(profileDir) {
  if (
    await pathExists(
      import_path.default.join(profileDir, "Extensions", playwrightExtensionId),
    )
  )
    return true;
  try {
    const prefs = await import_fs.default.promises.readFile(
      import_path.default.join(profileDir, "Preferences"),
      "utf-8",
    );
