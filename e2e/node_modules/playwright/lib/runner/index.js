...
var FSWatcher = class {
  ...
  async update(watchedPaths, ignoredFolders, reportPending) {
    if (
      JSON.stringify([this._watchedPaths, this._ignoredFolders]) ===
      JSON.stringify([watchedPaths, ignoredFolders])
    )
      return;
    ...
    this._fsWatcher = chokidar
      .watch(watchedPaths, { ignoreInitial: true, ignored })
    ...
    await new Promise((resolve, reject) =>
      this._fsWatcher.once("ready", resolve).once("error", reject),
    );
  }
  async close() {
...
var TeleReporterReceiver = class {
  ...
  _onProject(project) {
    let projectSuite = this._options.mergeProjects
      ? this._rootSuite.suites.find(
          (suite) => suite.project().name === project.name,
        )
      : void 0;
    if (!projectSuite) {
    ...
    if (index === -1) this._config.projects.push(parsed);
    else this._config.projects[index] = parsed;
  ...
  _onStepBegin(testId, resultId, payload) {
    const test = this._tests.get(testId);
    const result = test.results.find((r) => r._id === resultId);
    const parentStep = payload.parentStepId
      ? result._stepMap.get(payload.parentStepId)
      : void 0;
  ...
  _onError(error, workerInfo) {
    let fullWorkerInfo;
    if (workerInfo) {
      const project = this._config.projects.find(
        (p) => p.name === workerInfo.projectName,
      );
      if (project) {
        fullWorkerInfo = {
  ...
  _onStdIO(type, testId, resultId, data, isBase64) {
    ...
    const result =
      test && resultId ? test.results.find((r) => r._id === resultId) : void 0;
    if (type === "stdout") {
  ...
  _mergeSuiteInto(jsonSuite, parent) {
    ...
    if (!targetSuite) {
      targetSuite = new TeleSuite(
        jsonSuite.title,
        parent.type === "project" ? "file" : "describe",
      );
  ...
  _mergeTestInto(jsonTest, parent) {
    let targetTest = this._options.mergeTestCases
      ? parent.tests.find(
          (s) =>
            s.title === jsonTest.title &&
  ...
  _absolutePath(relativePath) {
    ...
    return this._options.resolvePath
      ? this._options.resolvePath(this._rootDir, relativePath)
      : this._rootDir + "/" + relativePath;
  }
};
var TeleSuite = class {
  ...
  allTests() {
    ...
    const visit = (suite) => {
      for (const entry of suite.entries()) {
        if (entry.type === "test") result.push(entry);
        else visit(entry);
      }
    };
    visit(this);
  ...
  skip(_reason) {
    throw new Error(
      "Disposition methods are not supported on a TeleSuite (read-only).",
    );
  }
  fixme(_reason) {
    throw new Error(
...
var TeleTestCase = class {
  ...
  skip(_reason) {
    throw new Error(
      "Disposition methods are not supported on a TeleTestCase (read-only).",
    );
  }
  fixme(_reason) {
    throw new Error(
...
var TeleTestStep = class {
  ...
  get attachments() {
    return (
      this._endPayload?.attachments?.map(
        (index) => this._result.attachments[index],
      ) ?? []
    );
  }
  get annotations() {
...
function debug(s, ...args) {
  if (!process.env.DEBUG_GIT_COMMIT_INFO) return;
  print(s, ...args);
}
var gitCommitInfoPlugin = (fullConfig) => {
  return {
    ...
    setup: async (config2, configDir) => {
      ...
      if (
        fullConfig.captureGitInfo?.commit ||
        (fullConfig.captureGitInfo?.commit === void 0 && ci)
      ) {
        const git = await gitCommitInfo(configDir).catch((e) =>
          print("failed to get git commit info", e),
        );
        if (git) {
      ...
      if (
        fullConfig.captureGitInfo?.diff ||
        (fullConfig.captureGitInfo?.diff === void 0 && ci)
      ) {
  ...
  if (process.env.GITHUB_ACTIONS) {
    ...
    try {
      const json = JSON.parse(
        await fs.promises.readFile(process.env.GITHUB_EVENT_PATH, "utf8"),
      );
      pr = {
    ...
    return {
      ...
      prHref: pr
        ? `${process.env.GITHUB_SERVER_URL}/${process.env.GITHUB_REPOSITORY}/pull/${pr.number}`
        : void 0,
      prTitle: pr?.title,
...
async function gitCommitInfo(gitDir) {
  ...
  const logOutput = await runGit(
    ["log", "-1", `--pretty=format:${tokens.join(separator2)}`],
    gitDir,
  );
  if (!logOutput) return void 0;
...
async function gitDiff(gitDir, ci) {
  ...
  if (ci?.prBaseHash) {
    ...
    if (diff3) return diff3.substring(0, diffLimit);
  }
  if (ci) return;
...
async function runGit(args, cwd) {
  ...
  if (monotonicTime() - start > GIT_OPERATIONS_TIMEOUT_MS) {
    print(
      `timeout of ${GIT_OPERATIONS_TIMEOUT_MS}ms exceeded while running "git ${args.join(" ")}"`,
    );
    return;
  }
  if (result.code)
...
var { monotonicTime: monotonicTime2 } =
  require("playwright-core/lib/coreBundle").iso;
...
var WebServerPlugin = class {
  ...
  async setup(config2, configDir, reporter) {
    ...
    if (this._options.url)
      this._isAvailableCallback = getIsAvailableFunction(
        this._options.url,
        this._checkPortOnly,
        !!this._options.ignoreHTTPSErrors,
        this._reporter.onStdErr?.bind(this._reporter),
      );
    this._options.cwd = this._options.cwd
      ? import_path.default.resolve(configDir, this._options.cwd)
      : configDir;
    try {
  ...
  async _startProcess() {
    ...
    if (isAlreadyAvailable) {
      ...
      throw new Error(
        `${this._options.url ?? `http://localhost${port ? ":" + port : ""}`} is already used, make sure that nothing is running on the port/url or set reuseExistingServer:true in config.webServer.`,
      );
    }
    ...
    const { launchedProcess, gracefullyClose } = await launchProcess({
            ...
            timeout !== 0
              ? setTimeout(
                  () =>
                    reject(
                      new Error(
                        `process didn't close gracefully within timeout`,
                      ),
                    ),
                  timeout,
                )
      ...
      onExit: (code) =>
        processExitedReject(
          new Error(
            code
              ? `Process from config.webServer was not able to start. Exit code: ${code}`
    ...
    launchedProcess.stdout.on("data", (data) => {
      if (debugWebServer.enabled || this._options.stdout === "pipe")
        this._reporter.onStdOut?.(
          prefixOutputLines(data.toString(), this._options.name),
        );
    });
    launchedProcess.stderr.on("data", (data) => {
      if (
        debugWebServer.enabled ||
        this._options.stderr === "pipe" ||
        !this._options.stderr
      )
    ...
    for (const stdio of ["stdout", "stderr"]) {
      launchedProcess[stdio].on("data", (data) => {
        if (
          !this._options.wait?.[stdio] ||
          stdioWaitCollectors[stdio] === void 0
        )
          return;
        stdioWaitCollectors[stdio] += (0, import_util.stripVTControlCharacters)(
          data.toString(),
        );
        this._options.wait[stdio].lastIndex = 0;
        const result = this._options.wait[stdio].exec(
          stdioWaitCollectors[stdio],
        );
        if (result) {
  ...
  async _waitForProcess() {
    ...
    if (this._isAvailableCallback)
      racingPromises.push(
        raceAgainstDeadline(
          () => waitFor(this._isAvailableCallback, cancellationToken),
          deadline,
        ),
      );
    if (this._waitForStdioPromise)
      racingPromises.push(
        raceAgainstDeadline(() => this._waitForStdioPromise, deadline),
      );
    const { timedOut } = await Promise.race(racingPromises);
    cancellationToken.canceled = true;
    if (timedOut)
      throw new Error(
        `Timed out waiting ${launchTimeout}ms from config.webServer.`,
      );
    debugWebServer(`WebServer available`);
  }
};
async function isPortUsed(port) {
  const innerIsPortUsed = (host) =>
    new Promise((resolve) => {
      const conn = import_net.default
        .connect(port, host)
        .on("error", () => {
          resolve(false);
        })
...
function getIsAvailableFunction(
  ...
  if (!checkPortOnly)
    return () =>
      isURLAvailable(urlObject, ignoreHTTPSErrors, debugWebServer, onStdErr);
...
var webServerPluginsForConfig = (config2) => {
  ...
  for (const webServerConfig of config2.webServers) {
    if (webServerConfig.port && webServerConfig.url)
      throw new Error(
        `Either 'port' or 'url' should be specified in config.webServer.`,
      );
    let url;
    if (webServerConfig.port || webServerConfig.url) {
    ...
    webServerPlugins.push(
      new WebServerPlugin(
        { ...webServerConfig, url },
        webServerConfig.port !== void 0,
      ),
    );
  }
...
var { parseErrorStack } = require("playwright-core/lib/coreBundle").utils;
var { getPackageManagerExecCommand } =
  require("playwright-core/lib/coreBundle").utils;
...
var StripAnsiStream = class extends import_stream.Writable {
  ...
  write(chunk, encodingOrCallback, callback) {
    ...
    return this._target.write(
      (0, import_util2.stripAnsiEscapes)(chunk.toString()),
      cb,
    );
  }
};
var terminalScreen = (() => {
  ...
  if (
    ...
    (process.env.NO_COLOR !== void 0 && process.env.NO_COLOR !== "")
  )
    useColors = false;
  ...
  return {
    ...
    colors: colors7,
    stdout: useColors
      ? originalProcessStdout
      : new StripAnsiStream(originalProcessStdout),
    stderr: useColors
      ? originalProcessStderr
      : new StripAnsiStream(originalProcessStderr),
  };
})();
...
var TerminalReporter = class {
  ...
  generateStartingMessage() {
    ...
    const shardDetails = this.config.shard
      ? `, shard ${this.config.shard.current} of ${this.config.shard.total}`
      : "";
    if (!this.totalTestCount) return "";
    return (
      "\n" +
      this.screen.colors.dim("Running ") +
      this.totalTestCount +
      this.screen.colors.dim(
        ` test${this.totalTestCount !== 1 ? "s" : ""} using `,
      ) +
      jobs +
      this.screen.colors.dim(` worker${jobs !== 1 ? "s" : ""}${shardDetails}`)
    );
  }
  getSlowTests() {
    if (!this.config.reportSlowTests) return [];
    const fileDurations = [...this.fileDurations.entries()]
      .filter(([key, value]) => value.workers.size === 1)
      .map(([key, value]) => [key, value.duration]);
  ...
  generateSummaryMessage({
    ...
    if (unexpected.length) {
      ...
      for (const test of unexpected)
        tokens.push(
          this.screen.colors.red(
            this.formatTestHeader(test, { indent: "    " }),
          ),
        );
    }
    if (interrupted.length) {
      tokens.push(
        this.screen.colors.yellow(`  ${interrupted.length} interrupted`),
      );
      for (const test of interrupted)
        tokens.push(
          this.screen.colors.yellow(
            this.formatTestHeader(test, { indent: "    " }),
          ),
        );
    }
    ...
    if (expected)
      tokens.push(
        this.screen.colors.green(`  ${expected} passed`) +
          this.screen.colors.dim(` (${msToString(this.result.duration)})`),
      );
    if (
      fatalErrors.length &&
      expected + unexpected.length + interrupted.length + flaky.length > 0
    )
      tokens.push(
        this.screen.colors.red(
          `  ${fatalErrors.length === 1 ? "1 error was not a part of any test" : fatalErrors.length + " errors were not a part of any test"}, see above for details`,
        ),
      );
    return tokens.join("\n");
  }
  ...
  _printSlowTests() {
    ...
    slowTests.forEach(([file, duration]) => {
      this.writeLine(
        this.screen.colors.yellow("  Slow test file: ") +
          file +
          this.screen.colors.yellow(` (${msToString(duration)})`),
      );
    });
    if (slowTests.length)
      this.writeLine(
  ...
  _printSummary(summary) {
    if (summary.trim()) this.writeLine(summary);
  }
  willRetry(test) {
    return (
  ...
  formatTestHeader(test, options = {}) {
    return formatTestHeader(this.screen, this.config, test, {
      ...options,
      includeTestId: this._options.includeTestId,
    });
  }
...
function formatResultErrors(screen, test, result) {
  ...
  if (test.outcome() === "unexpected") {
    ...
    for (const error of errorDetails) lines.push(error.message, "");
  }
  return lines.join("\n");
}
function formatFailure(screen, config2, test, index, options) {
  ...
  for (const result of test.results) {
    ...
    if (!printedHeader) {
      const header = formatTestHeader(screen, config2, test, {
        indent: "  ",
        index,
        mode: "error",
        includeTestId: options?.includeTestId,
      });
    ...
    if (result.retry) {
      resultLines.push("");
      resultLines.push(
        screen.colors.gray(separator(screen, `    Retry #${result.retry}`)),
      );
    }
    ...
    for (let i = 0; i < attachmentGroups.length; ++i) {
      ...
      if (attachment.name === "error-context" && attachment.path) {
        ...
        resultLines.push(
          screen.colors.dim(
            `    Error Context: ${relativeFilePath(screen, config2, attachment.path)}`,
          ),
        );
        continue;
      }
      ...
      resultLines.push(
        screen.colors.dim(
          separator(
            screen,
            `    attachment #${i + 1}: ${screen.colors.bold(attachment.name)} (${attachment.contentType})`,
          ),
        ),
      );
      if (attachment.actual?.path) {
        if (attachment.expected?.path) {
          const expectedPath = relativeFilePath(
            screen,
            config2,
            attachment.expected.path,
          );
          resultLines.push(screen.colors.dim(`    Expected: ${expectedPath}`));
        }
        const actualPath = relativeFilePath(
          screen,
          config2,
          attachment.actual.path,
        );
        resultLines.push(screen.colors.dim(`    Received: ${actualPath}`));
        if (attachment.previous?.path) {
          const previousPath = relativeFilePath(
            screen,
            config2,
            attachment.previous.path,
          );
          resultLines.push(screen.colors.dim(`    Previous: ${previousPath}`));
        }
        if (attachment.diff?.path) {
          const diffPath = relativeFilePath(
            screen,
            config2,
            attachment.diff.path,
          );
        ...
        if (attachment.name === "trace") {
          ...
          resultLines.push(
            screen.colors.dim(
              `        ${packageManagerCommand} playwright show-trace ${quotePathIfNeeded(relativePath)}`,
            ),
          );
          resultLines.push("");
        }
...
function quotePathIfNeeded(path20) {
  if (/\s/.test(path20)) return `"${path20}"`;
  return path20;
}
var kReportedSymbol = /* @__PURE__ */ Symbol("reported");
...
function formatResultFailure(screen, test, result, initialIndent) {
  ...
  if (result.status === "passed" && test.expectedStatus === "failed") {
    errorDetails.push({
      message: indent(
        screen.colors.red(`Expected to fail, but passed.`),
        initialIndent,
      ),
    });
  }
  if (result.status === "interrupted") {
    errorDetails.push({
      message: indent(
        screen.colors.red(`Test was interrupted.`),
        initialIndent,
      ),
    });
  }
...
function formatTestTitle(screen, config2, test, step, options = {}) {
  ...
  const extraTags = test.tags.filter(
    (t2) => !testTitle.includes(t2) && !config2.tags.includes(t2),
  );
...
function formatError(screen, error) {
  ...
  if (error.cause)
    tokens.push(
      screen.colors.dim("[cause]: ") + formatError(screen, error.cause).message,
    );
  return {
    location,
    message: tokens.join("\n"),
...
function separator(screen, text = "") {
  ...
  return (
    ...
    screen.colors.dim(
      "\u2500".repeat(
        Math.max(0, columns - (0, import_util2.stripAnsiEscapes)(text).length),
      ),
    )
  );
}
...
function resolveFromEnv(name) {
  ...
  if (value) return import_path2.default.resolve(process.cwd(), value);
  return void 0;
}
function resolveOutputFile(reporterName, options) {
  ...
  if (!outputFile && options.outputFile)
    outputFile = import_path2.default.resolve(
      options.configDir,
      options.outputFile,
    );
  ...
  if (!outputDir && options.outputDir)
    outputDir = import_path2.default.resolve(
      options.configDir,
      options.outputDir,
    );
  if (!outputDir && options.default)
    outputDir = (0, import_util2.resolveReporterOutputPath)(
...
function groupAttachments(attachments) {
  ...
  for (const attachment of attachments) {
    ...
    if (category === "expected") group.expected = attachment;
    else if (category === "actual") group.actual = attachment;
    else if (category === "diff") group.diff = attachment;
    else if (category === "previous") group.previous = attachment;
  }
  return result;
}
...
var Multiplexer = class {
  ...
  onConfigure(config2) {
    for (const reporter of this._reporters)
      this._wrap(() => reporter.onConfigure?.(config2));
  }
  async preprocess(params) {
    for (const reporter of this._reporters) await reporter.preprocess?.(params);
  }
  onBegin(suite) {
    for (const reporter of this._reporters)
  ...
  printsToStdio() {
    return this._reporters.some((r) => {
      let prints = false;
      this._wrap(
        () => (prints = r.printsToStdio ? r.printsToStdio() : true),
        false,
      );
      return prints;
    });
  }
  _wrap(callback, redispatch = true) {
    try {
      callback();
    } catch (e) {
      this._hasReporterErrors = true;
      if (redispatch) this.onError((0, import_util3.serializeError)(e));
    }
  }
  async _wrapAsync(callback) {
    try {
      return await callback();
    } catch (e) {
      this._hasReporterErrors = true;
      this.onError((0, import_util3.serializeError)(e));
    }
  }
};
...
var ReporterV2Wrapper = class {
  ...
  onBegin(suite) {
    ...
    for (const item of deferred) {
      if (item.error) this.onError(item.error.error, item.error.workerInfo);
      if (item.stdout)
...
// packages/playwright/src/reporters/internalReporter.ts
var { monotonicTime: monotonicTime3 } =
  require("playwright-core/lib/coreBundle").iso;
...
function addLocationAndSnippetToError(config2, error, file) {
  ...
  try {
    ...
    const codeFrame = babel.codeFrameColumns(
      source,
      { start: location },
      { highlightCode: true },
    );
    if (!file || import_fs.default.realpathSync(file) !== location.file) {
      tokens.push(
        internalScreen.colors.gray(`   at `) +
          `${relativeFilePath(internalScreen, config2, location.file)}:${location.line}`,
      );
      tokens.push("");
    }
...
var { monotonicTime: monotonicTime4, timeOrigin } =
  require("playwright-core/lib/coreBundle").iso;
var { raceAgainstDeadline: raceAgainstDeadline2 } =
  require("playwright-core/lib/coreBundle").iso;
var ProcessHost = class extends import_events.EventEmitter {
  ...
  async startRunner(runnerParams, options = {}) {
    ...
    this.process.on("message", (message) => {
      ...
      if (message.method === "__env_produced__") {
        ...
        this._producedEnv = Object.fromEntries(
          producedEnv.map((e) => [e[0], e[1] ?? void 0]),
        );
        ...
        if (!handler) {
          this.send({
            method: "__response__",
            params: { id, error: { message: "Unknown method" } },
          });
        } else {
          handler(params)
            .then((result) => {
            ...
            .catch((error2) => {
              this.send({
                method: "__response__",
                params: { id, error: { message: error2.message } },
              });
            });
        }
      } else {
    ...
    if (options.onStdOut) this.process.stdout?.on("data", options.onStdOut);
    if (options.onStdErr) this.process.stderr?.on("data", options.onStdErr);
    const error = await new Promise((resolve) => {
      this.process.once("exit", (code, signal) =>
        resolve({ unexpectedly: true, code, signal }),
      );
  ...
  async stop() {
    ...
    if (this._didExitAndRanOnExit) return;
    const exitPromise = new Promise((f) => this.once("exit", () => f()));
    const timeout = +(process.env.PWTEST_CHILD_PROCESS_TIMEOUT || 5 * 60 * 1e3);
    const result = await raceAgainstDeadline2(
      () => exitPromise,
      monotonicTime4() + timeout,
    );
    if (result.timedOut) {
      this.emit("processError", {
        message: `Error: ${this._processName} process did not exit within ${timeout}ms after stop, force-killed it`,
      });
      this._forceKill();
      await exitPromise;
    }
  ...
  _forceKill() {
    ...
    try {
      if (process.platform === "win32")
        import_child_process.default.spawnSync(`taskkill /pid ${pid} /T /F`, {
          shell: true,
        });
...
var InProcessLoaderHost = class {
  constructor(config2) {
    this._config = config2;
    this._poolBuilder =
      import_common2.poolBuilder.PoolBuilder.createForLoader();
  }
  async start(errors) {
    return true;
  }
  async loadTestFile(file, testErrors) {
    const result = await import_common2.testLoader.loadTestFile(
      file,
      this._config,
      testErrors,
    );
...
var OutOfProcessLoaderHost = class {
  constructor(config2) {
    this._config = config2;
    this._processHost = new ProcessHost(
      require.resolve("../loader/loaderProcessEntry.js"),
      "loader",
      {},
    );
  }
  async start(errors) {
    const startError = await this._processHost.startRunner(
      import_common2.ipc.serializeConfig(this._config, false),
    );
    if (startError) {
      errors.push({
...
var readFileAsync = (0, import_util4.promisify)(import_fs2.default.readFile);
var readDirAsync = (0, import_util4.promisify)(import_fs2.default.readdir);
...
function filterProjects(projects, projectNames) {
  ...
  if (unmatchedProjectNames.size) {
    const unknownProjectNames = Array.from(unmatchedProjectNames.values())
      .map((n) => `"${n}"`)
      .join(", ");
    throw new Error(
      `Project(s) ${unknownProjectNames} not found. Available projects: ${projects.map((p) => `"${p.project.name}"`).join(", ")}`,
    );
  }
  if (!result.length) {
...
function findTopLevelProjects(config2) {
  ...
  return [...closure]
    .filter((entry) => entry[1] === "top-level")
    .map((entry) => entry[0]);
}
...
async function collectFilesForProject(
  ...
  const testFileExtension = (file) =>
    extensions.has(import_path3.default.extname(file));
  const allFiles = await cachedCollectFiles(
    project.project.testDir,
    project.respectGitIgnore,
    fsCache,
  );
  const testMatch = (0, import_util5.createFileMatcher)(
    project.project.testMatch,
  );
  const testIgnore = (0, import_util5.createFileMatcher)(
    project.project.testIgnore,
  );
...
async function collectFiles(testDir, respectGitIgnore) {
  if (!import_fs2.default.existsSync(testDir)) return [];
  if (!import_fs2.default.statSync(testDir).isDirectory()) return [];
  const checkIgnores = (entryPath, rules, isDirectory, parentStatus) => {
    ...
    for (const rule of rules) {
      ...
      if (rule.match("/" + relative) || rule.match(relative)) {
        ...
        isDirectory &&
        (rule.match("/" + relative + "/") || rule.match(relative + "/"))
      ) {
        status = ruleIncludes ? "included" : "ignored";
      } else if (
        isDirectory &&
        ruleIncludes &&
        (rule.match("/" + relative, true) || rule.match(relative, true))
      ) {
  ...
  const visit = async (dir, rules, status) => {
    ...
    if (respectGitIgnore) {
      ...
      if (gitignore) {
        const content = await readFileAsync(
          import_path3.default.join(dir, gitignore.name),
          "utf8",
        );
        const newRules = content
          ...
          .map((s) => {
            s = s.trim();
            if (!s) return;
            const rule = new minimatch.Minimatch(s, {
              matchBase: true,
              dot: true,
              flipNegate: true,
            });
    ...
    for (const entry of entries) {
      ...
      const entryStatus = checkIgnores(
        entryPath,
        rules,
        entry.isDirectory(),
        status,
...
// packages/playwright/src/runner/reporterTestRun.ts
var import_common3 = require("../common");
var ReporterTestRunImpl = class {
  constructor(_rootSuite, _readonlyProjectSuites) {
    this._rootSuite = _rootSuite;
    this._readonlyProjectSuites = _readonlyProjectSuites;
    this._active = true;
    this._skipSharding = false;
    this.skip = import_common3.transform.wrapFunctionWithLocation(
      (location, target, reason) =>
        this._modifier("skip", location, target, reason),
    );
    this.fixme = import_common3.transform.wrapFunctionWithLocation(
      (location, target, reason) =>
        this._modifier("fixme", location, target, reason),
    );
    this.fail = import_common3.transform.wrapFunctionWithLocation(
      (location, target, reason) =>
        this._modifier("fail", location, target, reason),
    );
  }
  close() {
    this._active = false;
  ...
  skipSharding() {
    if (!this._active)
      throw new Error(
        `TestRun.skipSharding() can only be called from Reporter.preprocess().`,
      );
    if (this._skipSharding)
      throw new Error(
        `Multiple reporters called 'skipSharding'. Only one reporter may handle sharding.`,
      );
  ...
  exclude(target) {
    if (!this._active)
      throw new Error(
        `TestRun.exclude() can only be called from Reporter.preprocess().`,
      );
    const internalTarget = target;
    this._checkTarget("exclude", internalTarget);
    internalTarget.parent._detach(internalTarget);
  }
  _modifier(type, location, _target, reason) {
    if (!this._active)
      throw new Error(
        `TestRun.${type}() can only be called from Reporter.preprocess().`,
      );
    const target = _target;
    this._checkTarget(type, target);
    const annotation = { type, description: reason, location };
    if (target instanceof import_common3.test.Suite) {
  ...
  _checkTarget(method, target) {
    if (target === this._rootSuite)
      throw new Error(
        `TestRun.${method}() cannot be called on the root suite.`,
      );
    let suite =
      target instanceof import_common3.test.Suite ? target : target.parent;
    while (suite) {
      if (this._readonlyProjectSuites.has(suite)) {
        const targetType =
          target instanceof import_common3.test.TestCase ? " test" : "";
        throw new Error(
          `TestRun.${method}() cannot be called on a setup or teardown project${targetType}; these always run in full.`,
        );
      }
      suite = suite.parent;
    }
...
function createTestGroups(projectSuite, expectedParallelism) {
  ...
  for (const test of projectSuite.allTests()) {
    ...
    for (let parent = test.parent; parent; parent = parent.parent) {
      if (
        parent._parallelMode === "serial" ||
        parent._parallelMode === "default"
      )
  ...
  for (const withWorkerHash of groups.values()) {
    for (const withRequireFile of withWorkerHash.values()) {
      ...
      const parallelWithHooksGroupSize = Math.ceil(
        withRequireFile.parallelWithHooks.tests.length / expectedParallelism,
      );
      let lastGroup;
      for (const test of withRequireFile.parallelWithHooks.tests) {
        if (
          !lastGroup ||
          lastGroup.tests.length >= parallelWithHooksGroupSize
        ) {
...
function filterForShard(shard, weights, testGroups) {
  ...
  if (weights.length !== shard.total)
    throw new Error(
      `PWTEST_SHARD_WEIGHTS number of weights must match the shard total of ${shard.total}`,
    );
  const totalWeight = weights.reduce((a, b) => a + b, 0);
  let shardableTotal = 0;
  for (const group of testGroups) shardableTotal += group.tests.length;
  const shardSizes = weights.map((w) =>
    Math.floor((w * shardableTotal) / totalWeight),
  );
  ...
  let from = 0;
  for (let i = 0; i < shard.current - 1; i++) from += shardSizes[i];
...
var sourceMapSupport =
  require("playwright-core/lib/utilsBundle").sourceMapSupport;
var { toPosixPath } = require("playwright-core/lib/coreBundle").utils;
async function collectProjectsAndTestFiles(
  testRun,
  doNotRunTestsOutsideProjectFilter,
) {
  ...
  for (const [project, files] of allFilesForProject) {
    const matchedFiles = files.filter((file) => {
      ...
      const hasMatchingSources = sourceMapSources(file, sourceMapCache).some(
        (source) => {
          const matchesAllFileFilters = testRun.loadFileFilters.every(
  ...
  for (const [project, type] of projectClosure) {
    if (type === "dependency") {
      const treatProjectAsEmpty =
        doNotRunTestsOutsideProjectFilter &&
        !testRun.filteredProjects.includes(project);
      const files = treatProjectAsEmpty
        ? []
        : allFilesForProject.get(project) ||
          (await collectFilesForProject(project, fsCache));
...
async function loadFileSuites(testRun, mode, errors) {
  ...
  const loaderHost =
    mode === "out-of-process"
      ? new OutOfProcessLoaderHost(config2)
      : new InProcessLoaderHost(config2);
  ...
  for (const file of allTestFiles) {
    for (const dependency of import_common4.cc.dependenciesForTestFile(file)) {
      if (allTestFiles.has(dependency)) {
        const importer = import_path4.default.relative(
          config2.config.rootDir,
          file,
        );
        const importee = import_path4.default.relative(
          config2.config.rootDir,
          dependency,
        );
        errors.push({
  ...
  for (const [project, files] of testRun.projectFiles) {
    const suites = files
      .map((file) => fileSuiteByFile.get(file))
      .filter(Boolean);
...
async function createRootSuite(testRun, errors, shouldFilterOnly) {
  const config2 = testRun.config;
  const rootSuite = new import_common4.test.Suite("", "root");
  ...
  const filteredProjectSuites = /* @__PURE__ */ new Map();
  {
    for (const [project, fileSuites] of testRun.projectSuites) {
      ...
      const filteredProjectSuite = filterProjectSuite(
        projectSuite,
        testRun.preOnlyTestFilters,
      );
      filteredProjectSuites.set(project, filteredProjectSuite);
    }
  }
  if (shouldFilterOnly) {
    const filteredRoot = new import_common4.test.Suite("", "root");
    for (const filteredProjectSuite of filteredProjectSuites.values())
      filteredRoot._addSuite(filteredProjectSuite);
    import_common4.suiteUtils.filterOnly(filteredRoot);
  ...
  const projectClosure = buildProjectsClosure(
    [...filteredProjectSuites.keys()],
    (project) => filteredProjectSuites.get(project)._hasTests(),
  );
  for (const [project, type] of projectClosure) {
    if (type === "top-level") {
      project.project.repeatEach =
        project.fullConfig.configCLIOverrides.repeatEach ??
        project.project.repeatEach;
      rootSuite._addSuite(
        buildProjectSuite(project, filteredProjectSuites.get(project)),
      );
    }
  }
  if (config2.config.forbidOnly) {
    ...
    if (onlyTestsAndSuites.length > 0) {
      const configFilePath = config2.config.configFile
        ...
        : void 0;
      errors.push(
        ...createForbidOnlyErrors(
          onlyTestsAndSuites,
          config2.configCLIOverrides.forbidOnly,
          configFilePath,
        ),
      );
    }
  }
  const dependencySuites = /* @__PURE__ */ new Map();
  for (const [project, type] of projectClosure) {
    if (type !== "dependency") continue;
    const dependencySuite = buildProjectSuite(
      project,
      projectSuites.get(project),
    );
    dependencySuites.set(project, dependencySuite);
    rootSuite._prependSuite(dependencySuite);
  }
  const reporterTestRun = new ReporterTestRunImpl(
    rootSuite,
    new Set(dependencySuites.values()),
  );
  ...
  if (config2.config.shard && !reporterTestRun.shouldSkipSharding()) {
    const testGroups = [];
    for (const projectSuite of rootSuite.suites) {
      for (const group of createTestGroups(
        projectSuite,
        config2.config.shard.total,
      ))
        testGroups.push(group);
    }
    const testGroupsInThisShard = filterForShard(
      config2.config.shard,
      testRun.options.shardWeights,
      testGroups,
    );
    ...
    import_common4.suiteUtils.filterTestsRemoveEmptySuites(rootSuite, (test) =>
      testsInThisShard.has(test),
    );
  }
  if (testRun.postShardTestFilters.length)
    import_common4.suiteUtils.filterTestsRemoveEmptySuites(rootSuite, (test) =>
      testRun.postShardTestFilters.every((filter) => filter(test)),
    );
  const topLevelProjects = [];
  {
    const finalProjectClosure = buildProjectsClosure(
      rootSuite.suites.map((suite) => suite._fullProject),
    );
    for (const [project, type] of finalProjectClosure) {
      if (type === "dependency")
        rootSuite._addSuite(dependencySuites.get(project));
      else topLevelProjects.push(project);
    }
  }
  rootSuite._entries.sort(
    (a, b) =>
      config2.projects.indexOf(a._fullProject) -
      config2.projects.indexOf(b._fullProject),
  );
  testRun.rootSuite = rootSuite;
  testRun.topLevelProjects = topLevelProjects;
}
function createProjectSuite(project, fileSuites) {
  const projectSuite = new import_common4.test.Suite(
    project.project.name,
    "project",
  );
  for (const fileSuite of fileSuites)
    projectSuite._addSuite(
      import_common4.suiteUtils.bindFileSuiteToProject(project, fileSuite),
    );
  const grepMatcher = (0, import_util6.createTitleMatcher)(
    project.project.grep,
  );
  const grepInvertMatcher = project.project.grepInvert
    ? (0, import_util6.createTitleMatcher)(project.project.grepInvert)
    : null;
  import_common4.suiteUtils.filterTestsRemoveEmptySuites(
    projectSuite,
    (test) => {
      const grepTitle = test._grepTitleWithTags();
      if (grepInvertMatcher?.(grepTitle)) return false;
...
function filterProjectSuite(projectSuite, testFilters) {
  if (!testFilters.length) return projectSuite;
  const result = projectSuite._deepClone();
  import_common4.suiteUtils.filterTestsRemoveEmptySuites(result, (test) =>
    testFilters.every((filter) => filter(test)),
  );
  return result;
}
function buildProjectSuite(project, projectSuite) {
  const result = new import_common4.test.Suite(project.project.name, "project");
  ...
  for (const fileSuite of projectSuite.suites) {
    ...
    for (
      ...
      repeatEachIndex < project.project.repeatEach;
      repeatEachIndex++
    ) {
      const clone = fileSuite._deepClone();
      import_common4.suiteUtils.applyRepeatEachIndex(
        project,
        clone,
        repeatEachIndex,
      );
...
function createForbidOnlyErrors(
  onlyTestsAndSuites,
  forbidOnlyCLIFlag,
  configFilePath,
) {
  ...
  for (const testOrSuite of onlyTestsAndSuites) {
    ...
    const configFilePathName = configFilePath
      ? `'${configFilePath}'`
      : "the Playwright configuration file";
    const forbidOnlySource = forbidOnlyCLIFlag
      ? `'--forbid-only' CLI flag`
      : `'forbidOnly' option in ${configFilePathName}`;
    const error = {
...
async function requireOrImportDefaultFunction(file, expectConstructor) {
  let func = await import_common4.transform.requireOrImport(file);
  if (func && typeof func === "object" && "default" in func)
    func = func["default"];
  if (typeof func !== "function")
    throw (0, import_util6.errorWithFile)(
      file,
...
function loadGlobalHook(config2, file) {
  return requireOrImportDefaultFunction(
    import_path4.default.resolve(config2.config.rootDir, file),
    false,
  );
}
function loadReporter(config2, file) {
  return requireOrImportDefaultFunction(
    config2 ? import_path4.default.resolve(config2.config.rootDir, file) : file,
    true,
  );
}
function sourceMapSources(file, cache) {
  ...
  try {
    ...
    const sourceMapData =
      typeof sourceMap?.map === "string"
        ? JSON.parse(sourceMap.map)
        : sourceMap?.map;
    if (sourceMapData?.sources)
      sources = sourceMapData.sources.map((source) =>
        import_path4.default.resolve(
          import_path4.default.dirname(file),
          source,
        ),
      );
  } finally {
...
async function loadTestList(config2, filePath) {
  try {
    const content = await import_fs3.default.promises.readFile(
      filePath,
      "utf-8",
    );
    const lines = content
    ...
    const descriptions = lines.map((line) => {
      ...
      return {
        project,
        file: toPosixPath((0, import_util6.parseLocationArg)(tokens[0]).file),
        titlePath: tokens.slice(1),
      };
    });
    const testFilter = (test) =>
      descriptions.some((d) => {
        const [projectName, , ...titles] = test.titlePath();
        if (d.project !== void 0 && d.project !== projectName) return false;
        ...
        if (relativeFile !== d.file) return false;
        return (
          d.titlePath.length <= titles.length &&
          d.titlePath.every((_, index) => titles[index] === d.titlePath[index])
        );
      });
    const fileFilter = (file) => {
      const relativeFile = toPosixPath(
        import_path4.default.relative(config2.config.rootDir, file),
      );
    ...
    throw (0, import_util6.errorWithFile)(
      filePath,
      "Cannot read test list file: " + e.message,
    );
  }
}
...
var TeleReporterEmitter = class {
  constructor(messageSink, options = {}) {
    ...
    this._idSymbol = /* @__PURE__ */ Symbol("id");
  ...
  onConfigure(config2) {
    ...
    this._messageSink({
      method: "onConfigure",
      params: { config: this._serializeConfig(config2) },
    });
  }
  onBegin(suite) {
    const projects = suite.suites.map((projectSuite) =>
      this._serializeProject(projectSuite),
    );
    for (const project of projects)
  ...
  _serializeResultEnd(result) {
    ...
    return {
      ...
      duration: result.duration,
      status: result.status,
      errors: this._resultKnownErrorCounts.has(id)
        ? result.errors.slice(this._resultKnownAttachmentCounts.get(id))
        : result.errors,
      annotations: result.annotations?.length
        ? this._relativeAnnotationLocations(result.annotations)
        : void 0,
    };
  }
  _sendNewAttachments(result, testId) {
    const resultId = result[this._idSymbol];
    const knownAttachmentCount =
      this._resultKnownAttachmentCounts.get(resultId) ?? 0;
      ...
      this._messageSink({
        ...
        params: {
          ...
          attachments: this._serializeAttachments(
            result.attachments.slice(knownAttachmentCount),
          ),
        },
      });
    }
  ...
  _serializeAttachments(attachments) {
    return attachments.map((a) => {
      ...
      return {
        ...
        base64:
          body && !this._emitterOptions.omitBuffers
  ...
  _serializeStepEnd(step, result) {
    return {
      ...
      attachments: step.attachments.length
        ? step.attachments.map((a) => result.attachments.indexOf(a))
        : void 0,
      annotations: step.annotations.length
        ? this._relativeAnnotationLocations(step.annotations)
        : void 0,
    };
  }
  _relativeAnnotationLocations(annotations) {
    return annotations.map((annotation) => ({
      ...
      location: annotation.location
        ? this._relativeLocation(annotation.location)
        : void 0,
    }));
  }
...
var { ManualPromise: ManualPromise2 } =
  require("playwright-core/lib/coreBundle").iso;
var { calculateSha1, createGuid: createGuid2 } =
  require("playwright-core/lib/coreBundle").utils;
var { removeFolders, sanitizeForFilePath } =
  require("playwright-core/lib/coreBundle").utils;
...
var BlobReporter = class extends TeleReporterEmitter {
  constructor(options) {
    ...
    if (this._options.fileName && !this._options.fileName.endsWith(".zip"))
      throw new Error(
        `Blob report file name must end with .zip extension: ${this._options.fileName}`,
      );
  ...
  async onEnd(result) {
    ...
    zipFile.outputStream
      .pipe(import_fs4.default.createWriteStream(zipFileName))
      .on("close", () => {
    ...
    for (const { originalPath, zipEntryPath } of this._attachments) {
      if (
        !import_fs4.default
          .statSync(originalPath, { throwIfNoEntry: false })
          ?.isFile()
      )
        continue;
  ...
  async _prepareOutputFile() {
    ...
    await import_fs4.default.promises.mkdir(
      import_path6.default.dirname(outputFile),
      { recursive: true },
    );
    return outputFile;
  }
  _defaultReportName(config2) {
    ...
    if (config2.shard) {
      const paddedNumber = `${config2.shard.current}`.padStart(
        `${config2.shard.total}`.length,
        "0",
      );
...
var DotReporter = class extends TerminalReporter {
  ...
  onStdOut(chunk, test, result) {
    ...
    if (!this.config.quiet) this.screen.stdout.write(chunk);
  }
  onStdErr(chunk, test, result) {
    super.onStdErr(chunk, test, result);
    if (!this.config.quiet) this.screen.stderr.write(chunk);
  }
  onTestEnd(test, result) {
    ...
    switch (test.outcome()) {
      ...
      case "unexpected":
        this.screen.stdout.write(
          this.screen.colors.red(result.status === "timedOut" ? "T" : "F"),
        );
        break;
      case "flaky":
  ...
  async onTestPaused(test, result) {
    if (!process.stdin.isTTY && !process.env.PW_TEST_DEBUG_REPORTERS) return;
    this.screen.stdout.write("\n");
    if (test.outcome() === "unexpected") {
      this.writeLine(
        this.screen.colors.red(this.formatTestHeader(test, { indent: "  " })),
      );
      this.writeLine(this.formatResultErrors(test, result));
      markErrorsAsReported(result);
      this.writeLine(
        this.screen.colors.yellow("    Paused on error. Press Ctrl+C to end.") +
          "\n",
      );
    } else {
      this.writeLine(
...
var GitHubLogger = class {
  ...
  _log(message, type = "notice", options = {}) {
    ...
    const configs = Object.entries(options)
      ...
      .join(",");
    process.stdout.write(
      (0, import_util7.stripAnsiEscapes)(`::${type} ${configs}::${message}
`),
    );
  }
...
var GitHubReporter = class extends TerminalReporter {
  ...
  onTestEnd(test, result) {
    super.onTestEnd(test, result);
    if (this.willRetry(test)) return;
    if (!this._shouldPrintFailureAnnotations(test)) return;
  ...
  _shouldPrintFailureAnnotations(test) {
    switch (test.outcome()) {
      ...
      case "skipped":
        return (
          test.results.some((r) => r.status === "interrupted") &&
          test.results.some((r) => !!r.error)
        );
      default:
        return false;
    }
  ...
  _printSlowTestAnnotations() {
    this.getSlowTests().forEach(([file, duration]) => {
      const filePath = workspaceRelativePath(
        import_path7.default.join(process.cwd(), file),
      );
  ...
  _printFailureAnnotation(test, result, index) {
    ...
    for (const error of errors) {
      ...
      const message = [
        header,
        ...formatRetry(this.screen, result),
        error.message,
      ].join("\n");
...
function workspaceRelativePath(filePath) {
  return import_path7.default.relative(
...
var { calculateSha1: calculateSha12 } =
  require("playwright-core/lib/coreBundle").utils;
var {
  copyFileAndMakeWritable,
  removeFolders: removeFolders2,
  sanitizeForFilePath: sanitizeForFilePath2,
  toPosixPath: toPosixPath2,
} = require("playwright-core/lib/coreBundle").utils;
var {
  getPackageManagerExecCommand: getPackageManagerExecCommand2,
  isCodingAgent,
} = require("playwright-core/lib/coreBundle").utils;
var { HttpServer, serveFolder } =
  require("playwright-core/lib/coreBundle").utils;
var { gracefullyProcessExitDoNotHang } =
  require("playwright-core/lib/coreBundle").utils;
...
var HtmlReporter = class {
  ...
  onBegin(suite) {
    ...
    for (const project of this.config.projects) {
      if (
        this._isSubdirectory(outputFolder, project.outputDir) ||
        this._isSubdirectory(project.outputDir, outputFolder)
      ) {
...
`);
  ...
  _resolveOptions() {
    const outputFolder =
      reportFolderFromEnv() ??
      (0, import_util8.resolveReporterOutputPath)(
        "playwright-report",
    ...
    return {
      outputFolder,
      open:
        getHtmlReportOptionProcessEnv() || this._options.open || "on-failure",
      attachmentsBaseURL:
        process.env.PLAYWRIGHT_HTML_ATTACHMENTS_BASE_URL ||
        this._options.attachmentsBaseURL ||
        "data/",
  ...
  _isSubdirectory(parentDir, dir) {
    ...
    return (
      ...
      !import_path8.default.isAbsolute(relativePath)
    );
  }
  onError(error) {
  ...
  async onEnd(result) {
    ...
    const noSnippets =
      parseBooleanEnvVar("PLAYWRIGHT_HTML_NO_SNIPPETS") ??
      this._options.noSnippets;
    const noCopyPrompt =
      parseBooleanEnvVar("PLAYWRIGHT_HTML_NO_COPY_PROMPT") ??
      this._options.noCopyPrompt;
    const doNotInlineAssets =
      parseBooleanEnvVar("PLAYWRIGHT_HTML_DO_NOT_INLINE_ASSETS") ??
      this._options.doNotInlineAssets ??
      false;
    const mergeFiles =
      parseBooleanEnvVar("PLAYWRIGHT_HTML_MERGE_FILES") ??
      this._options.mergeFiles;
    const builder = new HtmlBuilder(
      yazl2,
      this.config,
      this._outputFolder,
      this._attachmentsBaseURL,
      doNotInlineAssets,
      {
  ...
  async onExit() {
    ...
    if (shouldOpen) {
      ...
      const relativeReportPath =
        this._outputFolder === standaloneDefaultFolder()
          ? ""
          : " " +
            import_path8.default.relative(process.cwd(), this._outputFolder);
...
function getHtmlReportOptionProcessEnv() {
  ...
  if (!htmlOpenEnv) return void 0;
  if (!isHtmlReportOption(htmlOpenEnv)) {
    writeLine(
      colors2.red(
        `Configuration Error: HTML reporter Invalid value for PLAYWRIGHT_HTML_OPEN: ${htmlOpenEnv}. Valid values are: ${htmlReportOptions.join(", ")}`,
      ),
    );
    return void 0;
  }
...
async function resolveReportFolder(reportPath) {
  const stat = await import_fs5.default.promises
    .stat(reportPath)
    .catch(() => null);
  if (!stat) throw new Error(`No report found at "${reportPath}"`);
...
async function extractReportZip(zipPath) {
  const tempDir = await import_fs5.default.promises.mkdtemp(
    import_path8.default.join(
  ...
  const hasIndex = await import_fs5.default.promises
    .access(import_path8.default.join(tempDir, "index.html"))
    .then(
      () => true,
      () => false,
    );
...
async function showHTMLReport(reportFolder, host = "localhost", port, testId) {
  ...
  writeLine("");
  writeLine(
    colors2.cyan(`  Serving HTML report at ${url}. Press Ctrl+C to quit.`),
  );
  if (testId) url += `#?testId=${testId}`;
  url = url.replace("0.0.0.0", "localhost");
  if (!isCodingAgent()) await open(url, { wait: true }).catch(() => {});
  await new Promise(() => {});
}
var HtmlBuilder = class {
  ...
  async build(metadata, projectSuites, result, topLevelErrors, machines) {
    ...
    if (!this._options.noSnippets) createSnippets(this._stepsInFile);
    let ok = true;
    for (const [fileId, { testFile, testFileSummary }] of data) {
      ...
      const testCaseSummaryComparator = (t1, t2) => {
        const w1 =
          (t1.outcome === "unexpected" ? 1e3 : 0) +
          (t1.outcome === "flaky" ? 1 : 0);
        const w2 =
          (t2.outcome === "unexpected" ? 1e3 : 0) +
    ...
    const htmlReport = {
      ...
      errors: topLevelErrors.map(
        (error) => formatError(internalScreen, error).message,
      ),
    ...
    if (this._hasTraces) {
      const traceViewerFolder = import_path8.default.join(
      ...
      const traceViewerTargetFolder = import_path8.default.join(
        this._reportFolder,
        "trace",
      );
      const traceViewerAssetsTargetFolder = import_path8.default.join(
        traceViewerTargetFolder,
        "assets",
      );
      import_fs5.default.mkdirSync(traceViewerAssetsTargetFolder, {
        recursive: true,
      });
      for (const file of import_fs5.default.readdirSync(traceViewerFolder)) {
        ...
        await copyFileAndMakeWritable(
          import_path8.default.join(traceViewerFolder, file),
          import_path8.default.join(traceViewerTargetFolder, file),
        );
      }
      for (const file of import_fs5.default.readdirSync(
        import_path8.default.join(traceViewerFolder, "assets"),
      )) {
        if (file.endsWith(".map") || file.includes("xtermModule")) continue;
        await copyFileAndMakeWritable(
          import_path8.default.join(traceViewerFolder, "assets", file),
          import_path8.default.join(traceViewerAssetsTargetFolder, file),
        );
      }
    }
  ...
  async _writeStaticAssets() {
    const appFolder = import_path8.default.join(
      require.resolve("playwright-core"),
      "..",
      "lib",
      "vite",
    ...
    const reportIndexFile = import_path8.default.join(
      this._reportFolder,
      "index.html",
    );
    if (this._doNotInlineAssets) {
      const html = await import_fs5.default.promises.readFile(
        import_path8.default.join(appFolder, "index.html"),
        "utf-8",
      );
      await Promise.all([
        import_fs5.default.promises.writeFile(reportIndexFile, html),
        import_fs5.default.promises.copyFile(
          import_path8.default.join(appFolder, "report.js"),
          import_path8.default.join(this._reportFolder, "report.js"),
        ),
        import_fs5.default.promises.copyFile(
          import_path8.default.join(appFolder, "report.css"),
          import_path8.default.join(this._reportFolder, "report.css"),
        ),
      ]);
    } else {
      let html = await import_fs5.default.promises.readFile(
        import_path8.default.join(appFolder, "index.html"),
        "utf-8",
      );
      const [js, css] = await Promise.all([
        import_fs5.default.promises.readFile(
          import_path8.default.join(appFolder, "report.js"),
          "utf-8",
        ),
        import_fs5.default.promises.readFile(
    ...
    await new Promise((resolve, reject) => {
      this._dataZipFile.end(void 0, () => {
        this._dataZipFile.outputStream
          ...
          .pipe(import_fs5.default.createWriteStream(filePath, { flags: "a" }))
          .on("close", resolve)
          .on("error", reject);
      });
    });
      ...
      .map((a) => {
        ...
        if (
          ...
          if (
            ...
            lastAttachment.body += (0, import_util8.stripAnsiEscapes)(a.body);
            return null;
          }
          a.body = (0, import_util8.stripAnsiEscapes)(a.body);
          lastAttachment = a;
          return a;
        }
        if (a.path) {
          ...
          try {
            ...
            const sha1 =
              calculateSha12(buffer) + import_path8.default.extname(a.path);
            fileName = this._attachmentsBaseURL + sha1;
            import_fs5.default.mkdirSync(
              import_path8.default.join(this._reportFolder, "data"),
              { recursive: true },
            );
            import_fs5.default.writeFileSync(
              import_path8.default.join(this._reportFolder, "data", sha1),
              buffer,
            );
          } catch (e) {}
          return {
        ...
        if (a.body instanceof Buffer) {
          ...
          const extension =
            sanitizeForFilePath2(
              import_path8.default.extname(a.name).replace(/^\./, ""),
            ) ||
            mime2.getExtension(a.contentType) ||
            "dat";
  ...
  _createTestResult(test, result) {
    return {
      ...
      retry: result.retry,
      steps: dedupeSteps(result.steps).map((s) =>
        this._createTestStep(s, result),
      ),
      errors: formatResultFailure(internalScreen, test, result, "").map(
        (error) => {
          return {
            message: error.message,
            codeframe: error.location
              ? createErrorCodeframe(error.message, error.location)
              : void 0,
          };
        },
      ),
  ...
  _createTestStep(dedupedStep, result) {
    ...
    const skipped = dedupedStep.step.annotations?.find(
      (a) => a.type === "skip",
    );
    ...
    const testStep = {
      ...
      duration,
      steps: dedupeSteps(step.steps).map((s) =>
        this._createTestStep(s, result),
      ),
  ...
  _relativeLocation(location) {
    ...
    const file = toPosixPath2(
      import_path8.default.relative(this._config.rootDir, location.file),
    );
    return {
      file,
      line: location.line,
...
function dedupeSteps(steps) {
  ...
  for (const step of steps) {
    ...
    if (!canDedupe) lastResult = void 0;
  }
  return result;
}
function createSnippets(stepsInFile) {
  for (const file of stepsInFile.keys()) {
    ...
    const highlighted = babel2.codeFrameColumns(
      source,
...
function createErrorCodeframe(message, location) {
  ...
  return babel2.codeFrameColumns(
    ...
    },
    {
      ...
      linesBelow: 100,
      message:
        (0, import_util8.stripAnsiEscapes)(message).split("\n")[0] || void 0,
    },
  );
}
...
var JSONReporter = class {
      ...
      config: {
        ...
        projects: this.config.projects.map((project) => {
          return {
            ...
            retries: project.retries,
            metadata: project.metadata,
            id: import_common5.config.getProjectId(project),
            name: project.name,
  ...
  _mergeSuites(suites) {
    ...
    for (const projectSuite of suites) {
      const projectId = import_common5.config.getProjectId(
    ...
    for (const [, suites2] of fileSuites) {
      ...
      for (const suite of suites2) this._mergeTestsFromSuite(result, suite);
      results.push(result);
    }
    return results;
  }
  _relativeLocation(location) {
    ...
    return {
      file: toPosixPath3(
        import_path9.default.relative(this.config.rootDir, location.file),
      ),
      line: location.line,
      column: location.column,
    };
  ...
  _locationMatches(s1, s2) {
    return (
      s1.file === s2.file && s1.line === s2.line && s1.column === s2.column
    );
  }
  _mergeTestsFromSuite(to, from) {
    for (const fromSuite of from.suites || []) {
      const toSuite = (to.suites || []).find(
        (s) =>
          s.title === fromSuite.title && this._locationMatches(s, fromSuite),
      );
      if (toSuite) {
    ...
    for (const spec of from.specs || []) {
      const toSpec = to.specs.find(
        (s) =>
          ...
          s.file ===
            toPosixPath3(
              import_path9.default.relative(this.config.rootDir, spec.file),
            ) &&
          s.line === spec.line &&
  ...
  _serializeSuite(projectId, projectName, suite) {
    ...
    const suites = suite.suites
      .map((suite2) => this._serializeSuite(projectId, projectName, suite2))
      .filter((s) => s);
    return {
      title: suite.title,
...
async function outputReport(report, resolvedOutputFile) {
  ...
  if (resolvedOutputFile) {
    await import_fs6.default.promises.mkdir(
      import_path9.default.dirname(resolvedOutputFile),
      { recursive: true },
    );
...
function removePrivateFields(config2) {
  return Object.fromEntries(
    Object.entries(config2).filter(([name, value]) => !name.startsWith("_")),
  );
}
...
var JUnitReporter = class {
  constructor(options) {
    ...
    this.stripANSIControlSequences = getAsBooleanFromENV(
      "PLAYWRIGHT_JUNIT_STRIP_ANSI",
      !!options.stripANSIControlSequences,
    );
    this.includeProjectInTestName = getAsBooleanFromENV(
      "PLAYWRIGHT_JUNIT_INCLUDE_PROJECT_IN_TEST_NAME",
      !!options.includeProjectInTestName,
    );
    this.includeRetries = getAsBooleanFromENV(
  ...
  async onEnd(result) {
    ...
    for (const projectSuite of this.suite.suites) {
      for (const fileSuite of projectSuite.suites)
        children.push(
          await this._buildTestSuite(projectSuite.title, fileSuite),
        );
    }
    const tokens = [];
    const self = this;
    ...
    if (this.resolvedOutputFile) {
      await import_fs7.default.promises.mkdir(
        import_path10.default.dirname(this.resolvedOutputFile),
        { recursive: true },
      );
  ...
  async _buildTestSuite(projectName, suite) {
    ...
    let duration = 0;
    const children = [];
    const testCaseNamePrefix =
      projectName && this.includeProjectInTestName ? `[${projectName}] ` : "";
    for (const test of suite.allTests()) {
      ++tests;
      if (test.outcome() === "skipped") ++skipped;
      for (const result of test.results) duration += result.duration;
      const classification = await this._addTestCase(
        suite.title,
        testCaseNamePrefix,
        test,
        children,
      );
      if (classification === "error") ++errors;
  ...
  async _addTestCase(suiteName, namePrefix, test, entries) {
    ...
    if (properties.children?.length) entry.children.push(properties);
    ...
    if (this.includeRetries) {
      ...
      return this._addFailureEntry(
        test,
        classifyResultError(test.results[0]),
        entry,
      );
    }
    entry.attributes.time =
      test.results.reduce((acc, value) => acc + value.duration, 0) / 1e3;
  ...
  _addFailureEntry(test, errorInfo, entry) {
    if (errorInfo) {
      entry.children.push({
        ...
        text: (0, import_util9.stripAnsiEscapes)(
          formatFailure(nonTerminalScreen, this.config, test),
        ),
      });
  ...
  async _appendStdIO(entry, results) {
    ...
    for (const result of results) {
      for (const item of result.stdout) systemOut.push(item.toString());
      for (const item of result.stderr) systemErr.push(item.toString());
      for (const attachment of result.attachments) {
        if (!attachment.path) continue;
        let attachmentPath = import_path10.default.relative(
          this.configDir,
          attachment.path,
        );
        try {
          if (this.resolvedOutputFile)
            attachmentPath = import_path10.default.relative(
              import_path10.default.dirname(this.resolvedOutputFile),
              attachment.path,
            );
        } catch {
...
Warning: attachment ${attachmentPath} is missing`);
  ...
  async _buildRetryEntry(result, prefix) {
    ...
    const stackTrace =
      result.error?.stack || result.error?.message || result.error?.value || "";
    entry.children.push({
      name: "stackTrace",
      text: (0, import_util9.stripAnsiEscapes)(stackTrace),
    });
...
function classifyResultError(result) {
  ...
  const rawMessage = (0, import_util9.stripAnsiEscapes)(
    error.message || error.value || "",
  );
  const nameMatch = rawMessage.match(/^(\w+): /);
  const errorName = nameMatch ? nameMatch[1] : "";
  const messageBody = nameMatch
    ? rawMessage.slice(nameMatch[0].length)
    : rawMessage;
...
function serializeXML(entry, tokens, stripANSIControlSequences) {
  ...
  for (const [name, value] of Object.entries(entry.attributes || {}))
    attrs.push(
      `${name}="${escape(String(value), stripANSIControlSequences, false)}"`,
    );
...
var LineReporter = class extends TerminalReporter {
  ...
  _dumpToStdio(test, chunk, stream) {
    ...
    if (!process.env.PW_TEST_DEBUG_REPORTERS) stream.write(`\x1B[1A\x1B[2K`);
    if (test && this._lastTest !== test) {
      const title = this.screen.colors.dim(this.formatTestTitle(test));
      stream.write(
        this.fitToScreen(title) +
          `
`,
      );
      this._lastTest = test;
  ...
  _updateLine(test, result, step) {
    ...
    const currentRetrySuffix = result.retry
      ? this.screen.colors.yellow(` (retry #${result.retry})`)
      : "";
...
var { getAsBooleanFromENV: getAsBooleanFromENV2 } =
  require("playwright-core/lib/coreBundle").utils;
var DOES_NOT_SUPPORT_UTF8_IN_TERMINAL =
...
var ListReporter = class extends TerminalReporter {
  constructor(options) {
    ...
    this._printSteps = getAsBooleanFromENV2(
      "PLAYWRIGHT_LIST_PRINT_STEPS",
      options?.printSteps,
    );
    this._printFailuresInline = getAsBooleanFromENV2(
      "PLAYWRIGHT_LIST_PRINT_FAILURES_INLINE",
      options?.printFailuresInline,
    );
  }
  onBegin(suite) {
    super.onBegin(suite);
  ...
  onTestBegin(test, result) {
    ...
    const line =
      this.screen.colors.dim(this.formatTestTitle(test)) +
  ...
  onStepBegin(test, result, step) {
    ...
    if (this._printSteps) {
      ...
      const prefix = this._testPrefix(
        this.getStepIndex(testIndex, result, step),
        "",
      );
      const line = test.title + this.screen.colors.dim(stepSuffix(step));
      this._appendLine(line, prefix);
    } else {
      this._updateOrAppendLine(
        this._testRows,
        test,
        this.screen.colors.dim(this.formatTestTitle(test, step)) +
          this._retrySuffix(result),
        this._testPrefix(testIndex, ""),
      );
    }
  }
  onStepEnd(test, result, step) {
    ...
    if (!this._printSteps) {
      if (this.screen.isTTY)
        this._updateOrAppendLine(
          this._testRows,
          test,
          this.screen.colors.dim(this.formatTestTitle(test, step.parent)) +
            this._retrySuffix(result),
          this._testPrefix(testIndex, ""),
        );
      return;
    ...
    const title = this.screen.isTTY
      ? test.title + this.screen.colors.dim(stepSuffix(step))
      : this.formatTestTitle(test, step);
    const prefix = this._testPrefix(index, "");
    let text = "";
    if (step.error) text = this.screen.colors.red(title);
    else text = title;
  ...
  async onTestPaused(test, result) {
    ...
    this._appendLine(
      this.screen.colors.yellow(
        `Paused ${test.outcome() === "unexpected" ? "on error" : "at test end"}. Press Ctrl+C to end.`,
      ),
      this._testPrefix("", ""),
    );
    await new Promise(() => {});
  ...
  onTestEnd(test, result) {
    ...
    if (!wasPaused) this._updateTestLine(test, result);
    const isFailure =
    ...
    if (result.status === "skipped") {
      ...
      const statusMark =
        result.status === "passed"
          ? POSITIVE_STATUS_MARK
          : NEGATIVE_STATUS_MARK;
      ...
      text +=
        this._retrySuffix(result) +
        this.screen.colors.dim(` (${msToString3(result.duration)})`);
    }
  ...
  _updateOrAppendLine(entityRowNumbers, entity, text, prefix) {
    ...
    if (
      row !== void 0 &&
      this.screen.isTTY &&
      this._lastRow - row < this.screen.ttyHeight
    ) {
  ...
  _testPrefix(index, statusMark) {
    const statusMarkLength = (0, import_util10.stripAnsiEscapes)(
      statusMark,
    ).length;
    ...
    return (
      "  " +
      statusMark +
      " ".repeat(3 - statusMarkLength) +
      this.screen.colors.dim(index.padStart(indexLength) + " ")
    );
  }
  _retrySuffix(result) {
    return result.retry
...
var lastStepOrdinalSymbol = /* @__PURE__ */ Symbol("lastStepOrdinal");
  ...
  onBegin(suite) {
    ...
    for (const test of tests) {
      ...
      const projectTitle = projectName
        ...
        : "";
      this._writeLine(
        `  ${testId}${projectTitle}${location} \u203A ${titles.join(" \u203A ")}`,
      );
      files.add(test.location.file);
    }
    this._writeLine(
      `Total: ${tests.length} ${tests.length === 1 ? "test" : "tests"} in ${files.size} ${files.size === 1 ? "file" : "files"}`,
    );
  }
  onError(error) {
...
// packages/playwright/src/runner/reporters.ts
var { calculateSha1: calculateSha13 } =
  require("playwright-core/lib/coreBundle").utils;
async function createReporters(config2, mode, descriptions, runOptions) {
  ...
  const someReporterPrintsToStdio = reporters.some((r) =>
    r.printsToStdio ? r.printsToStdio() : true,
  );
  if (reporters.length && !someReporterPrintsToStdio) {
    if (mode === "list") reporters.unshift(new listModeReporter_default());
    else if (mode !== "merge")
      reporters.unshift(
        !process.env.CI ? new line_default() : new dot_default(),
      );
  }
  return reporters;
}
...
function computeCommandHash(config2, runOptions) {
  const parts = [];
  if (runOptions?.projectFilter) parts.push(...runOptions.projectFilter);
  const command = {};
  if (runOptions?.locations?.length) command.locations = runOptions.locations;
  if (runOptions?.grep) command.grep = runOptions.grep;
  if (runOptions?.grepInvert) command.grepInvert = runOptions.grepInvert;
  if (runOptions?.onlyChanged) command.onlyChanged = runOptions.onlyChanged;
  if (config2.config.tags.length) command.tags = config2.config.tags.join(" ");
  if (runOptions?.testList)
    command.testList = calculateSha13(
      import_fs8.default.readFileSync(runOptions.testList),
    );
  if (runOptions?.testListInvert)
    command.testListInvert = calculateSha13(
      import_fs8.default.readFileSync(runOptions.testListInvert),
    );
...
async function applySuggestedRebaselines(config2, reporter, filteredProjects) {
  if (config2.config.updateSnapshots === "none") return;
  ...
  const patchFile = import_path12.default.join(
  ...
  for (const fileName of [...suggestedRebaselines.keys()].sort()) {
    ...
    const relativeName = import_path12.default.relative(
      process.cwd(),
      fileName,
    );
    ...
    if (config2.config.updateSourceMethod === "overwrite") {
      ...
      await import_fs9.default.promises.writeFile(
        fileName,
        applyPatchWithConflictMarkers(source, result),
      );
    } else {
      const gitFolder = findGitRoot(
        import_path12.default.dirname(fileName),
        gitCache,
      );
      const relativeToGit = import_path12.default.relative(
        gitFolder || process.cwd(),
        fileName,
      );
...
`);
  if (config2.config.updateSourceMethod === "patch") {
    await import_fs9.default.promises.mkdir(
      import_path12.default.dirname(patchFile),
      { recursive: true },
    );
  ...
  ` +
        colors3.cyan(
          "git apply " +
            import_path12.default.relative(process.cwd(), patchFile),
        ) +
        "\n",
    );
  }
...
function createPatch(fileName, before, after) {
  ...
  const text = diff.createPatch(file, before, after, void 0, void 0, {
    context: 3,
  });
  return [
...
function findGitRoot(dir, cache) {
  ...
  if (
    import_fs9.default.existsSync(gitPath) &&
    import_fs9.default.lstatSync(gitPath).isDirectory()
  ) {
    cache.set(dir, dir);
    return dir;
  }
...
// packages/playwright/src/runner/workerHost.ts
var { removeFolders: removeFolders3 } =
  require("playwright-core/lib/coreBundle").utils;
...
var WorkerHost = class extends ProcessHost {
  constructor(testGroup, options) {
    ...
    super(
      require.resolve("../worker/workerProcessEntry.js"),
      `worker-${workerIndex}`,
      {
        ...options.extraEnv,
        FORCE_COLOR: "1",
        DEBUG_COLORS:
          process.env.DEBUG_COLORS === void 0 ? "1" : process.env.DEBUG_COLORS,
      },
    );
    this._didFail = false;
    ...
    this._params = {
      ...
      artifactsDir: import_path13.default.join(
        options.outputDir,
        artifactsFolderName(workerIndex),
      ),
  ...
  async start() {
    await import_fs10.default.promises.mkdir(this._params.artifactsDir, {
      recursive: true,
    });
    return await this.startRunner(this._params, {
      onStdOut: (chunk) =>
        this.emit("stdOut", import_common6.ipc.stdioChunkToParams(chunk)),
      onStdErr: (chunk) =>
        this.emit("stdErr", import_common6.ipc.stdioChunkToParams(chunk)),
    });
  }
  async onExit() {
...
var Dispatcher = class {
  ...
  _findFirstJobToRun() {
    for (let index = 0; index < this._queue.length; index++) {
      ...
      if (
        this._isolatedJobs.has(job) &&
        this._workerSlots.some((w) => !!w.jobDispatcher)
      )
        continue;
      const projectIdWorkerLimit = this._workerLimitPerProjectId.get(
        job.projectId,
      );
      if (!projectIdWorkerLimit) return index;
      const runningWorkersWithSameProjectId = this._workerSlots.filter(
        (w) => w.jobDispatcher?.job.projectId === job.projectId,
      ).length;
      if (runningWorkersWithSameProjectId < projectIdWorkerLimit) return index;
    }
    return -1;
  }
  _scheduleJob() {
    ...
    const jobDispatcher = new JobDispatcher(
      job,
      this._testRun,
      this._ignoreMaxFailures ? void 0 : () => this.stop().catch(() => {}),
    );
  ...
  async _runJobInWorker(index, jobDispatcher) {
    ...
    if (!worker) {
      worker = this._createWorker(
        job,
        index,
        import_common7.ipc.serializeConfig(this._testRun.config, true),
      );
    ...
    if (startError) jobDispatcher.onExit(startError);
    else jobDispatcher.runInWorker(worker);
    ...
    else if (this._isWorkerRedundant(worker)) void worker.stop();
    if (!this._isStopped) {
      ...
      if (result.isolatedRetriesJob) {
        ...
        this._updateCounterForWorkerHash(
          result.isolatedRetriesJob.workerHash,
          1,
        );
      }
    }
  ...
  _checkFinished() {
    ...
    if (this._workerSlots.some((w) => !!w.jobDispatcher)) return;
  ...
  _isWorkerRedundant(worker) {
    ...
    for (const slot of this._workerSlots) {
      if (
        slot.worker &&
        !slot.worker.didSendStop() &&
        slot.worker.hash() === worker.hash()
      )
        workersWithSameHash++;
    }
    return (
      workersWithSameHash > this._queuedOrRunningHashCount.get(worker.hash())
    );
  }
  _updateCounterForWorkerHash(hash, delta) {
    this._queuedOrRunningHashCount.set(
      hash,
      delta + (this._queuedOrRunningHashCount.get(hash) || 0),
    );
  }
  ...
  _createWorker(testGroup, parallelIndex, loaderData) {
    const project = this._testRun.config.projects.find(
      (p) => p.id === testGroup.projectId,
    );
    const pauseAtEnd =
      this._testRun.topLevelProjects.includes(project) &&
      !!this._testRun.options.pauseAtEnd;
    ...
    const handleOutput = (params) => {
      ...
      const currentlyRunning =
        this._workerSlots[parallelIndex].jobDispatcher?.currentlyRunning();
    ...
    worker.on("exit", () => {
      const producedEnv =
        this._producedEnvByProjectId.get(testGroup.projectId) || {};
...
var JobDispatcher = class {
  constructor(job, testRun, onMaxFailuresReached) {
  ...
  _isStoppedByMaxFailures() {
    return (
      !!this._onMaxFailuresReached && this._testRun.hasReachedMaxFailures()
    );
  }
  _onTestBegin(params) {
  ...
  _onStepBegin(params) {
    ...
    const parentStep = params.parentStepId
      ? steps.get(params.parentStepId)
      : void 0;
    const step = {
      title: params.title,
      titlePath: () => {
  ...
  _onStepEnd(params) {
    ...
    if (!step) {
      this._testRun.reporter.onStdErr?.(
  ...
  _onAttach(params) {
    ...
    const attachment = {
      ...
      body:
        params.body !== void 0 ? Buffer.from(params.body, "base64") : void 0,
    };
    data.result.attachments.push(attachment);
    if (params.stepId) {
      const step = data.steps.get(params.stepId);
      if (step) step.attachments.push(attachment);
      else
        this._testRun.reporter.onStdErr?.(
          "Internal error: step id not found: " + params.stepId,
        );
    }
  }
  ...
  _massSkipTestsFromRemaining(testIds, errors) {
    ...
    if (errors.length) {
      this._testRun.hasWorkerErrors = true;
      for (const error of errors) this._testRun.reporter.onError?.(error);
    }
  }
  _onDone(params) {
    if (
      !this._remainingByTestId.size &&
      !this._failedTests.size &&
      !params.fatalErrors.length &&
      !params.skipTestsDueToSetupFailure.length &&
      !params.fatalUnknownTestIds &&
      !params.unexpectedExitError &&
      !params.stoppedDueToUnhandledErrorInTestFail
    ) {
    ...
    if (params.fatalErrors.length) {
      this._massSkipTestsFromRemaining(
        new Set(this._remainingByTestId.keys()),
        params.fatalErrors,
      );
    }
    this._massSkipTestsFromRemaining(
      new Set(params.skipTestsDueToSetupFailure),
      [],
    );
    if (params.unexpectedExitError) {
      if (this._currentlyRunning)
        this._massSkipTestsFromRemaining(
          /* @__PURE__ */ new Set([this._currentlyRunning.test.id]),
          [params.unexpectedExitError],
        );
      else
        this._massSkipTestsFromRemaining(
    ...
    for (const failedTest of this._failedTests) {
      ...
      if (
        outermostSerialSuite &&
        !this._failedWithNonRetriableError.has(outermostSerialSuite)
      )
        serialSuitesWithFailures.add(outermostSerialSuite);
    }
    const testsBelongingToSomeSerialSuiteWithFailures = [
      ...this._remainingByTestId.values(),
    ].filter((test) => {
    ...
    this._massSkipTestsFromRemaining(
      new Set(
        testsBelongingToSomeSerialSuiteWithFailures.map((test) => test.id),
      ),
      [],
    );
    ...
    const isolatedRetries = [];
    for (const test of retryCandidates) {
      if (test.results.length < test.retries + 1) {
        if (this._testRun.config.retryStrategy === "immediate")
          remaining.push(test);
        else isolatedRetries.push(test);
      }
    }
    const remainingJob = remaining.length
      ? { ...this.job, tests: remaining }
      : void 0;
    const isolatedRetriesJob = isolatedRetries.length
      ? { ...this.job, tests: isolatedRetries }
      : void 0;
  ...
  runInWorker(worker) {
    ...
    const runPayload = {
      ...
      entries: this.job.tests.map((test) => {
        return {
          ...
          planAnnotations: test._planAnnotations,
        };
      }),
    };
    ...
    this._listeners = [
      eventsHelper.addEventListener(
        worker,
        "testBegin",
        this._onTestBegin.bind(this),
      ),
      eventsHelper.addEventListener(
        worker,
        "testEnd",
        this._onTestEnd.bind(this),
      ),
      eventsHelper.addEventListener(
        worker,
        "stepBegin",
        this._onStepBegin.bind(this),
      ),
      eventsHelper.addEventListener(
        worker,
        "stepEnd",
        this._onStepEnd.bind(this),
      ),
      eventsHelper.addEventListener(
        worker,
        "attach",
        this._onAttach.bind(this),
      ),
      eventsHelper.addEventListener(
        worker,
        "testPaused",
        this._onTestPaused.bind(this, worker),
      ),
  ...
  _onTestPaused(worker, params) {
    ...
    const sendMessage = async (message) => {
      try {
        ...
        const response = await worker.sendCustomMessage({
          testId: test.id,
          request: message.request,
        });
        if (response.error)
          addLocationAndSnippetToError(
            this._testRun.config.config,
            response.error,
          );
        return response;
      } catch (e) {
        const error = (0, import_util11.serializeError)(e);
  ...
  skipWholeJob() {
    const allTestsSkipped = this.job.tests.every(
  ...
  _reportTestEnd(test, result) {
    ...
    if (!hadMaxFailures && this._isStoppedByMaxFailures()) {
      ...
      this._testRun.reporter.onError?.({
        message: colors4.red(
          `Testing stopped early after ${this._testRun.config.config.maxFailures} maximum allowed failures.`,
        ),
      });
    }
  }
...
var FixedNodeSIGINTHandler = class {
  ...
  static {
    this._dispatch = () => {
      if (this._ignoreNextSIGINTs) return;
      this._ignoreNextSIGINTs = true;
      setTimeout(() => {
        this._ignoreNextSIGINTs = false;
        if (!this._handlers.length) this._uninstall();
      }, 1e3);
...
var { ManualPromise: ManualPromise4 } =
  require("playwright-core/lib/coreBundle").iso;
var { monotonicTime: monotonicTime5 } =
  require("playwright-core/lib/coreBundle").iso;
var TaskRunner = class _TaskRunner {
  ...
  async run(context, deadline, cancelPromise) {
    const { status, cleanup } = await this.runDeferCleanup(
      context,
      deadline,
      cancelPromise,
    );
  ...
  async runDeferCleanup(
    context,
    deadline,
    cancelPromise = new ManualPromise4(),
  ) {
    const sigintWatcher = new SigIntWatcher();
    const timeoutWatcher = new TimeoutWatcher(deadline);
    const teardownRunner = new _TaskRunner(
      this._reporter,
      this._globalTimeoutForError,
    );
    ...
    const taskLoop = async () => {
      for (const task of this._tasks) {
        ...
        try {
          ...
          errors.push((0, import_util12.serializeError)(e));
        } finally {
    ...
    if (sigintWatcher.hadSignal() || cancelPromise?.isDone()) {
      ...
      this._reporter.onError?.({
        message: colors5.red(
          `Timed out waiting ${this._globalTimeoutForError / 1e3}s for the ${currentTaskName} to run`,
        ),
      });
      status = "timedout";
    ...
    const cleanup = () =>
      teardownRunner.runDeferCleanup(context, deadline).then((r) => r.status);
...
async function detectChangedTestFiles(baseCommit, configDir) {
  function gitFileList(args) {
    try {
      return import_child_process2.default
      ...
      const error = _error;
      const unknownRevision = error.output.some((line) =>
        line?.includes("unknown revision"),
      );
      if (unknownRevision) {
        const isShallowClone =
          import_child_process2.default
  ...
  const untrackedFiles = gitFileList([
    ...
    "--others",
    "--exclude-standard",
  ]).map((file) => import_path14.default.join(configDir, file));
  const [gitRoot] = gitFileList(["rev-parse", "--show-toplevel"]);
  const trackedFilesWithChanges = gitFileList([
    "diff",
    baseCommit,
    "--name-only",
  ]).map((file) => import_path14.default.join(gitRoot, file));
  return new Set(
    import_common8.cc.affectedTestFiles([
      ...untrackedFiles,
...
var { ManualPromise: ManualPromise5 } =
  require("playwright-core/lib/coreBundle").iso;
var { monotonicTime: monotonicTime6 } =
  require("playwright-core/lib/coreBundle").iso;
var { removeFolders: removeFolders4 } =
  require("playwright-core/lib/coreBundle").utils;
var readDirAsync2 = (0, import_util13.promisify)(import_fs11.default.readdir);
var TestRun = class {
  constructor(config2, reporter, options) {
    ...
    this.filteredProjects = filterProjects(
      config2.projects,
      this.options.projectFilter,
    );
  }
  onTestPaused(params) {
  ...
  result() {
    const hasFailedTests = this.rootSuite
      ?.allTests()
      .some((test) => !test.ok());
    const hasFlakyTests = this.rootSuite
      ?.allTests()
      .some((test) => test.outcome() === "flaky");
    return this.hasWorkerErrors ||
      this.reporter.hasReporterErrors() ||
      this.hasReachedMaxFailures() ||
      hasFailedTests ||
      (this.config.config.failOnFlakyTests && hasFlakyTests)
      ? "failed"
      : "passed";
  }
};
...
async function finishTaskRun(testRun, status) {
  if (status === "passed") status = testRun.result();
...
function createGlobalSetupTasks(config2) {
  return [
    ...
    ...config2.globalTeardowns
      .map((file) => createGlobalTeardownTask(file, config2))
      .reverse(),
...
function createClearCacheTask(config2) {
  return {
    title: "clear cache",
    setup: async () => {
      await (0, import_util14.removeDirAndLogToConsole)(
        import_common9.cc.cacheDir,
      );
...
function createPluginSetupTasks(config2) {
  return config2.plugins.map((plugin) => ({
    ...
    setup: async ({ reporter }) => {
      ...
      else plugin.instance = plugin.factory;
      await plugin.instance?.setup?.(
        config2.config,
        config2.configDir,
        reporter,
      );
...
function createGlobalSetupTask(file, config2) {
  ...
  if (config2.globalSetups.length > 1) title += ` (${file})`;
...
function createGlobalTeardownTask(file, config2) {
  ...
  if (config2.globalTeardowns.length > 1) title += ` (${file})`;
  return {
    title,
...
function createRemoveOutputDirsTask() {
  return {
    title: "clear output",
    setup: async (testRun) => {
      if (testRun.options.preserveOutputDir) return;
      const outputDirs = /* @__PURE__ */ new Set();
      testRun.filteredProjects.forEach((p) =>
        outputDirs.add(p.project.outputDir),
      );
      await Promise.all(
        Array.from(outputDirs).map((outputDir) =>
          removeFolders4([outputDir]).then(async ([error]) => {
            if (!error) return;
            if (error.code === "EBUSY") {
              ...
              await Promise.all(
                entries.map((entry) =>
                  removeFolders4([
                    import_path15.default.join(outputDir, entry),
                  ]),
                ),
              );
            } else {
...
function createListFilesTask() {
  return {
    ...
    setup: async (testRun, errors) => {
      ...
      for (const [project, files] of testRun.projectFiles) {
        const projectSuite = new import_common9.test.Suite(
          project.project.name,
          "project",
        );
        ...
        const suites = files.map((file) => {
          const title = import_path15.default.relative(
            testRun.config.config.rootDir,
            file,
          );
          const suite = new import_common9.test.Suite(title, "file");
  ...
  return {
    ...
    setup: async (testRun, errors, softErrors) => {
      if (testRun.options.locations?.length) {
        const { testFilter, fileFilter } =
          import_common9.suiteUtils.createFiltersFromArguments(
            testRun.options.locations,
          );
        testRun.loadFileFilters.push(fileFilter);
        testRun.preOnlyTestFilters.push(testFilter);
      }
      if (testRun.options.testList) {
        const { testFilter, fileFilter } = await loadTestList(
          testRun.config,
          testRun.options.testList,
        );
        testRun.preOnlyTestFilters.push(testFilter);
        testRun.loadFileFilters.push(fileFilter);
      }
      if (testRun.options.testListInvert) {
        const { testFilter } = await loadTestList(
          testRun.config,
          testRun.options.testListInvert,
        );
        testRun.preOnlyTestFilters.push((test) => !testFilter(test));
      }
      if (testRun.options.grep || testRun.options.grepInvert) {
        const grepMatcher = testRun.options.grep
          ? (0, import_util14.createTitleMatcher)(
              (0, import_util14.forceRegExp)(testRun.options.grep),
            )
          : () => true;
        const grepInvertMatcher = testRun.options.grepInvert
          ? (0, import_util14.createTitleMatcher)(
              (0, import_util14.forceRegExp)(testRun.options.grepInvert),
            )
          : () => false;
      ...
      await collectProjectsAndTestFiles(
        testRun,
        !!options.doNotRunDepsOutsideProjectFilter,
      );
      await loadFileSuites(
        testRun,
        mode,
        options.failOnLoadErrors ? errors : softErrors,
      );
      ...
      if (testRun.options.onlyChanged) {
        const changedFiles = await detectChangedTestFiles(
          ...
          testRun.config.configDir,
        );
        testRun.preOnlyTestFilters.push((test) =>
          changedFiles.has(test.location.file),
        );
      }
      await createRootSuite(
        testRun,
        options.failOnLoadErrors ? errors : softErrors,
        !!options.filterOnly,
      );
      if (
        options.failOnLoadErrors &&
        !testRun.rootSuite?.allTests().length &&
        !testRun.options.passWithNoTests &&
        !testRun.config.config.shard &&
        !testRun.options.onlyChanged &&
        !testRun.options.testList &&
        !testRun.options.testListInvert
      ) {
...
function createApplyRebaselinesTask() {
  return {
    ...
    teardown: async (testRun) => {
      await applySuggestedRebaselines(
        testRun.config,
        testRun.reporter,
...
function createPhasesTask() {
  return {
    ...
    setup: async (testRun) => {
      ...
      const projectToSuite = new Map(
        testRun.rootSuite.suites.map((suite) => [suite._fullProject, suite]),
      );
      const allProjects = [...projectToSuite.keys()];
      const teardownToSetups = buildTeardownToSetupsMap(allProjects);
      const ignoreMaxFailuresProjectIds = new Set(
        testRun.config.config.maxFailures > 0
          ? [...teardownToSetups.keys()].map((project) => project.id)
          : [],
      );
      ...
      for (let i = 0; i < projectToSuite.size; i++) {
        ...
        for (const project of projectToSuite.keys()) {
          ...
          const projectsThatShouldFinishFirst = [
            ...project.deps,
            ...(teardownToSetupsDependents.get(project) || []),
          ];
        ...
        for (const ignoreMaxFailures of [false, true]) {
          const projects = phaseProjects.filter(
            (project) =>
              ignoreMaxFailuresProjectIds.has(project.id) === ignoreMaxFailures,
          );
          if (!projects.length) continue;
          let testGroupsInPhase = 0;
          const phase = {
            dispatcher: new Dispatcher(testRun, { ignoreMaxFailures }),
            projects: [],
          };
          ...
          for (const project of projects) {
            ...
            const testGroups = createTestGroups(
              projectSuite,
              testRun.config.config.workers,
            );
            phase.projects.push({ project, projectSuite, testGroups });
            testGroupsInPhase += Math.min(
              project.workers ?? Number.MAX_SAFE_INTEGER,
              testGroups.length,
            );
          }
          debug5("pw:test:task")(
            `created phase #${testRun.phases.length} with ${phase.projects.map((p) => p.project.project.name).sort()} projects, ${testGroupsInPhase} testGroups`,
          );
          maxConcurrentTestGroups = Math.max(
            maxConcurrentTestGroups,
            testGroupsInPhase,
          );
        }
      }
      testRun.config.config.metadata.actualWorkers = Math.min(
        testRun.config.config.workers,
        maxConcurrentTestGroups,
      );
    },
...
function createRunTestsTask() {
  return {
    ...
    setup: async (testRun) => {
      ...
      const teardownToSetups = buildTeardownToSetupsMap(
        testRun.phases
          .map((phase) => phase.projects.map((p) => p.project))
          .flat(),
      );
      for (const { dispatcher, projects } of testRun.phases) {
        ...
        for (const { project, testGroups } of projects) {
          ...
          const hasFailedDeps = project.deps.some(
            (p) => !successfulProjects.has(p),
          );
        ...
        if (phaseTestGroups.length) {
          ...
          for (const [
            projectId,
            envProduced,
          ] of dispatcher.producedEnvByProjectId()) {
        ...
        if (!testRun.hasWorkerErrors) {
          for (const { project, projectSuite } of projects) {
            ...
            if (
              !hasFailedDeps &&
              !projectSuite.allTests().some((test) => !test.ok())
            )
    ...
    teardown: async ({ phases }) => {
      for (const { dispatcher } of phases.reverse()) await dispatcher.stop();
    },
  };
}
...
var LastRunReporter = class {
  constructor(filteredProjects, listMode, lastFailedFileOverride) {
    ...
    const override =
      lastFailedFileOverride ?? process.env.PLAYWRIGHT_LAST_RUN_OUTPUT_FILE;
    if (override) {
      this._lastRunFile = import_path16.default.resolve(
        process.cwd(),
        override,
      );
    } else {
      const [project] = filteredProjects;
      if (project)
        this._lastRunFile = import_path16.default.join(
          project.project.outputDir,
          ".last-run.json",
        );
    }
  ...
  async filterLastFailed() {
    if (!this._lastRunFile) return void 0;
    try {
      const lastRunInfo = JSON.parse(
        await import_fs12.default.promises.readFile(this._lastRunFile, "utf8"),
      );
  ...
  async onEnd(result) {
    if (!this._lastRunFile || this._listMode) return;
    const lastRunInfo = {
      status: result.status,
      failedTests:
        this._suite
    ...
    await import_fs12.default.promises.mkdir(
      import_path16.default.dirname(this._lastRunFile),
      { recursive: true },
    );
    await import_fs12.default.promises.writeFile(
      this._lastRunFile,
      JSON.stringify(lastRunInfo, void 0, 2),
    );
  }
};

// packages/playwright/src/runner/testRunner.ts
var { ManualPromise: ManualPromise6 } =
  require("playwright-core/lib/coreBundle").iso;
var { setPlaywrightTestProcessEnv } =
  require("playwright-core/lib/coreBundle").utils;
var { gracefullyProcessExitDoNotHang: gracefullyProcessExitDoNotHang2 } =
  require("playwright-core/lib/coreBundle").utils;
var TestRunnerEvent = {
...
var TestRunner = class extends import_events2.default {
  constructor(configLocation, configCLIOverrides) {
    ...
    this._watcher = new FSWatcher((events) => {
      ...
      events.forEach((f) =>
        import_common10.cc.collectAffectedTestFiles(f.file, collector),
      );
  ...
  hasSomeBrowsers() {
    for (const browserName of ["chromium", "webkit", "firefox"]) {
      try {
        import_coreBundle2.registry.registry
          .findExecutable(browserName)
  ...
  async installBrowsers() {
    const executables =
      import_coreBundle2.registry.registry.defaultExecutables();
    await import_coreBundle2.registry.registry.install(executables);
  }
  async loadConfig() {
    const { config: config2, error } = await this._loadConfig(
      this._configCLIOverrides,
    );
    if (config2) return config2;
    throw new Error(
      "Failed to load config: " + (error ? error.message : "Unknown error"),
    );
  }
  async runGlobalSetup(userReporters) {
    await this.runGlobalTeardown();
    const reporter = new InternalReporter(userReporters);
    const config2 = await this._loadConfigOrReportError(
      reporter,
      this._configCLIOverrides,
    );
    if (!config2) return { status: "failed", env: [] };
    const { status, cleanup } = await runTasksDeferCleanup(
      new TestRun(config2, reporter),
      [...createGlobalSetupTasks(config2)],
    );
    const env = [];
    for (const key of /* @__PURE__ */ new Set([
      ...Object.keys(process.env),
      ...Object.keys(this._startingEnv),
    ])) {
      if (this._startingEnv[key] !== process.env[key])
        env.push([key, process.env[key] ?? null]);
    }
    if (status !== "passed") await cleanup();
    else this._globalSetup = { cleanup };
    return { status, env };
  }
  ...
  async clearCache(userReporter) {
    ...
    if (!config2) return { status: "failed" };
  ...
  async listTests(userReporter, params) {
    ...
    this._queue = this._queue
      .then(async () => {
        const { config: config2, status } = await this._innerListTests(
          userReporter,
          params,
        );
        if (config2) await this._updateWatchedDirs(config2);
        result = { status };
      })
  ...
  async _updateWatcher(reportPending) {
    await this._watcher.update(
      [...this._watchedProjectDirs, ...this._watchedTestDependencies],
      [...this._ignoredProjectOutputs],
      reportPending,
    );
  }
  ...
  async _innerRunTests(userReporter, params) {
    ...
    const overrides = {
      ...
      use: {
        ...
        ...(params.trace === "on"
          ? { trace: { mode: "on", sources: false, live: true } }
          : {}),
    ...
    const config2 = await this._loadConfigOrReportError(
      new InternalReporter([userReporter]),
      overrides,
    );
    if (!config2) return { status: "failed" };
    const options = {
    ...
    const configReporters = params.disableConfigReporters
      ? []
      : await createReporters(config2, "test", void 0, options);
    ...
    const tasks = [
      ...
      createLoadTask("out-of-process", {
        filterOnly: true,
        failOnLoadErrors: !!params.failOnLoadErrors,
        doNotRunDepsOutsideProjectFilter:
          params.doNotRunDepsOutsideProjectFilter,
      }),
  ...
  async watch(fileNames) {
    ...
    for (const fileName of fileNames) {
      ...
      import_common10.cc
        .dependenciesForTestFile(fileName)
        .forEach((file) => this._watchedTestDependencies.add(file));
    }
  ...
  async findRelatedTestFiles(files, userReporter) {
    ...
    const reporter = new InternalReporter(
      userReporter ? [userReporter, errorReporter] : [errorReporter],
    );
    const config2 = await this._loadConfigOrReportError(reporter);
    if (!config2) return { errors: errorReporter.errors(), testFiles: [] };
    const status = await runTasks(new TestRun(config2, reporter), [
      ...createPluginSetupTasks(config2),
      createLoadTask("out-of-process", {
    ...
    return { testFiles: import_common10.cc.affectedTestFiles(files) };
  }
  async stopTests() {
  ...
  async _loadConfig(overrides) {
    try {
      const config2 = await import_common10.configLoader.loadConfig(
        this.configLocation,
        overrides,
      );
      if (!this._plugins) {
        webServerPluginsForConfig(config2).forEach((p) =>
          config2.plugins.push({ factory: p }),
        );
      ...
      return { config: null, error: (0, import_util15.serializeError)(e) };
    }
  }
  lastLoadedConfig() {
...
async function resolveCtDirs(config2) {
  ...
  const templateDir = await import_fs13.default.promises
    .realpath(
      import_path17.default.normalize(
        import_path17.default.join(config2.configDir, relativeTemplateDir),
      ),
    )
    .catch(() => void 0);
  ...
  const outDir = use.ctCacheDir
    ...
    : import_path17.default.resolve(templateDir, ".cache");
  return {
    outDir,
    templateDir,
  };
...
async function runAllTestsWithConfig(config2, options) {
  ...
  const reporters = await createReporters(
    config2,
    options.listMode ? "list" : "test",
    void 0,
    options,
  );
  const lastRun = new LastRunReporter(
    filteredProjects,
    options.listMode,
    options.lastFailedFile,
  );
  if (options.lastFailed) {
    const lastFailedTestIds = await lastRun.filterLastFailed();
    if (lastFailedTestIds) options = { ...options, lastFailedTestIds };
  }
  const reporter = new InternalReporter([...reporters, lastRun]);
  const tasks = options.listMode
    ? [
        createLoadTask("in-process", {
          failOnLoadErrors: true,
          filterOnly: false,
        }),
        createReportBeginTask(),
  ...
  const testRun = new TestRun(config2, reporter, {
    ...options,
    pauseAtEnd: config2.configCLIOverrides.pause,
    pauseOnError: config2.configCLIOverrides.pause,
  });
...
var import_common11 = require("../common");
...
var { ManualPromise: ManualPromise7 } =
  require("playwright-core/lib/coreBundle").iso;
var { isUnderTest } = require("playwright-core/lib/coreBundle").utils;
var { HttpServer: HttpServer2 } =
  require("playwright-core/lib/coreBundle").utils;
var { gracefullyProcessExitDoNotHang: gracefullyProcessExitDoNotHang3 } =
  require("playwright-core/lib/coreBundle").utils;
...
var TestServer = class {
  ...
  _allowedFileRoots() {
    const roots = /* @__PURE__ */ new Set([
      process.cwd(),
      this._configLocation.configDir,
    ]);
...
var TestServerDispatcher = class {
  constructor(configLocation, configCLIOverrides) {
    ...
    this.transport = {
      ...
      onclose: () => {
        if (this._closeOnDisconnect) gracefullyProcessExitDoNotHang3(0);
      },
    };
    this._dispatchEvent = (method, params) =>
      this.transport.sendEvent?.(method, params);
    this._testRunner.on(TestRunnerEvent.TestFilesChanged, (testFiles) =>
      this._dispatchEvent("testFilesChanged", { testFiles }),
    );
    this._testRunner.on(TestRunnerEvent.TestPaused, (params) =>
      this._dispatchEvent("testPaused", { errors: params.errors }),
    );
  }
  ...
  async _collectingReporter() {
    const report = [];
    return {
      reporter: await createReporterForTestServer(this._serializer, (e) =>
        report.push(e),
      ),
      report,
    };
  ...
  async runGlobalSetup(params) {
    ...
    const { status, env } = await this._testRunner.runGlobalSetup([
      reporter,
      new list_default(),
    ]);
  ...
  _setInterceptStdio(interceptStdio) {
    ...
    if (interceptStdio) {
      if (debug6.log === originalDebugLog) {
        debug6.log = (...args) => {
          const string = import_util16.default.format(...args) + "\n";
...
async function runUIMode(configFile, configCLIOverrides, options) {
  const configLocation =
    import_common11.configLoader.resolveConfigLocation(configFile);
  return await innerRunTestServer(
    configLocation,
    configCLIOverrides,
    options,
    async (server, cancelPromise) => {
      await import_coreBundle3.server.installRootRedirect(server, void 0, {
        ...options,
        webApp: "uiMode.html",
      });
      if (options.host !== void 0 || options.port !== void 0) {
        await import_coreBundle3.server.openTraceInBrowser(
        ...
        const page = await import_coreBundle3.server.openTraceViewerApp(
          server.urlPrefix("precise"),
          "chromium",
          {
            headless:
              isUnderTest() && process.env.PWTEST_HEADED_FOR_TEST !== "1",
...
async function installedChromiumChannelForUI(
  ...
  const config2 = await import_common11.configLoader
    .loadConfig(configLocation, configCLIOverrides)
    .catch((e) => null);
  if (!config2) return void 0;
  if (
    config2.projects.some(
      (p) =>
...
async function createReporterForTestServer(file, messageSink) {
  const reporterConstructor = file
    ? await loadReporter(null, file)
    : uiModeReporter_default;
  return wrapReporterAsV2(
    new reporterConstructor({
      _send: messageSink,
    }),
  );
}
...
var statusEx = /* @__PURE__ */ Symbol("statusEx");
...
var TeleSuiteUpdater = class {
  ...
  _createReporter() {
    return {
      ...
      onConfigure: (config2) => {
        ...
        void this._lastRunReceiver.dispatch({
          method: "onConfigure",
      ...
      onBegin: (suite) => {
        ...
        if (this._testResultsSnapshot) {
          for (const test of this.rootSuite.allTests())
            test.results =
              this._testResultsSnapshot?.get(test.id) || test.results;
      ...
      onTestEnd: (test, testResult) => {
        if (test.outcome() === "skipped") ++this.progress.skipped;
        else if (test.outcome() === "unexpected") ++this.progress.failed;
        else ++this.progress.passed;
  ...
  processListReport(report) {
    ...
    this._testResultsSnapshot = new Map(
      tests.map((test) => [test.id, test.results]),
    );
...
function createPathResolve(pathSeparator) {
  return (rootDir, relativePath) => {
    ...
    for (const segment of [
      ...rootDir.split(pathSeparator),
      ...relativePath.split(pathSeparator),
    ]) {
      const isAfterDrive =
        pathSeparator === "\\" &&
        segments.length === 1 &&
...
var TestServerConnection = class {
  ...
  async _sendMessage(method, params) {
    ...
    const error = new TestServerConnectionClosedError(
      `${method}: test server connection closed`,
    );
  ...
  _dispatchEvent(method, params) {
    ...
    else if (method === "testPaused") this._onTestPausedEmitter.fire(params);
  }
  async initialize(params) {
...
var { ManualPromise: ManualPromise8 } =
  require("playwright-core/lib/coreBundle").iso;
var { createGuid: createGuid3 } =
  require("playwright-core/lib/coreBundle").utils;
var { getPackageManagerExecCommand: getPackageManagerExecCommand3 } =
  require("playwright-core/lib/coreBundle").utils;
var { eventsHelper: eventsHelper2 } =
  require("playwright-core/lib/coreBundle").utils;
...
async function runWatchModeLoop(configLocation, initialOptions) {
  ...
  const transport = new InMemoryTransport(async (data) => {
    ...
    try {
      const result2 = await testServerDispatcher.transport.dispatch(
        method,
        params,
      );
  ...
  const teleSuiteUpdater = new TeleSuiteUpdater({
    pathSeparator: import_path18.default.sep,
    onUpdate() {},
  });
  ...
  testServerConnection.onTestFilesChanged(({ testFiles }) => {
    ...
    queue = queue.then(async () => {
      ...
      const { report: report2 } = await testServerConnection.listTests({
  ...
  testServerConnection.onReport((report2) =>
    teleSuiteUpdater.processTestReportEvent(report2),
  );
  ...
  while (true) {
    ...
    if (command === "changed") waitForCommand.dispose();
    if (bufferMode && command === "changed") continue;
    const shouldRunChangedFiles = bufferMode
      ? command === "run"
      : command === "changed";
    ...
    if (command === "project") {
      const { selectedProjects } = await enquirer
        .prompt({
          type: "multiselect",
    ...
    if (command === "file") {
      const { filePattern } = await enquirer
        .prompt({
          type: "text",
          name: "filePattern",
      ...
      if (filePattern.trim()) options.files = filePattern.split(" ");
      else options.files = void 0;
      await runTests(options, testServerConnection);
      lastRun = { type: "regular" };
      continue;
    }
    if (command === "grep") {
      const { testPattern } = await enquirer
        .prompt({
          type: "text",
          name: "testPattern",
      ...
      if (testPattern.trim()) options.grep = testPattern;
      else options.grep = void 0;
      await runTests(options, testServerConnection);
      lastRun = { type: "regular" };
      continue;
    }
    if (command === "failed") {
      const failedTestIds = teleSuiteUpdater.rootSuite
        .allTests()
        .filter((t2) => !t2.ok())
        .map((t2) => t2.id);
...
function readKeyPress(handler) {
  ...
  const listener = eventsHelper2.addEventListener(
    process.stdin,
    "keypress",
    (text, key) => {
...
function printConfiguration(options, title) {
  ...
  if (options.grep) tokens.push(colors6.red(`--grep ${options.grep}`));
  if (options.files) tokens.push(...options.files.map((a) => colors6.bold(a)));
  if (title) tokens.push(colors6.dim(`(${title})`));
  ...
  lines.push(
    `${colors6.dim("Show & reuse browser:")} ${colors6.bold(showBrowserServer ? "on" : "off")}`,
  );
...
`);
  if (dirtyTestFiles.size === 0) {
    process.stdout
      .write(`${colors6.dim("Waiting for file changes. Press")} ${colors6.bold("q")} ${colors6.dim("to quit or")} ${colors6.bold("h")} ${colors6.dim("for more options.")}

`);
    return;
  }
  process.stdout
    .write(`${colors6.dim(`${dirtyTestFiles.size} test ${dirtyTestFiles.size === 1 ? "file" : "files"} changed:`)}
...
async function toggleShowBrowser() {
  if (!showBrowserServer) {
    showBrowserServer = new import_coreBundle4.remote.PlaywrightServer({
      mode: "extension",
      path: "/" + createGuid3(),
      maxConnections: 1,
    });
    ...
    process.stdout
      .write(`${colors6.dim("Show & reuse browser:")} ${colors6.bold("on")}
`);
  } else {
    ...
    process.stdout
      .write(`${colors6.dim("Show & reuse browser:")} ${colors6.bold("off")}
`);
  }
}
  ...
  traverse(value) {
    ...
    if (Array.isArray(value)) {
      for (let i = 0; i < value.length; i++) {
        if (typeof value[i] === "string") value[i] = this.intern(value[i]);
        else this.traverse(value[i]);
      }
    } else {
      for (const name in value) {
        if (typeof value[name] === "string")
          value[name] = this.intern(value[name]);
        else this.traverse(value[name]);
      }
    }
  }
  intern(value) {
...
async function createMergedReport(
  config2,
  dir,
  ...
  const eventData = await mergeEvents(
    dir,
    shardFiles,
    stringPool,
    printStatus,
  ...
  const pathSeparator = rootDirOverride
    ? import_path19.default.sep
    : (eventData.pathSeparatorFromMetadata ?? import_path19.default.sep);
  const pathPackage =
    pathSeparator === "/"
      ? import_path19.default.posix
      : import_path19.default.win32;
  const receiver = new TeleReporterReceiver(multiplexer, {
    ...
    resolvePath: (rootDir, relativePath) =>
      stringPool.internString(
        pathPackage.normalize(pathPackage.join(rootDir, relativePath)),
      ),
  ...
  for (const {
    ...
    if (config3?.tags?.length)
      eventPatchers.patchers.push(
        new GlobalErrorPatcher(config3.tags.join(" ")),
      );
...
function parseCommonEvents(reportJsonl) {
  return splitBufferLines(reportJsonl)
    ...
    .filter((line) => commonEventRegex.test(line))
...
async function extractAndParseReports(
  dir,
  shardFiles,
  internalizer,
  printStatus,
  ...
  const shardEvents = [];
  await import_fs14.default.promises.mkdir(
    import_path19.default.join(dir, "resources"),
    { recursive: true },
  );
  ...
  for (const file of shardFiles) {
    ...
    printStatus(
      `extracting: ${(0, import_util17.relativeFilePath)(absolutePath)}`,
    );
...
function findMetadata(events, file) {
  ...
  if (metadata.version > currentBlobReportVersion)
    throw new Error(
      `Blob report ${file} was created with a newer version of Playwright.`,
    );
  return metadata;
}
async function mergeEvents(
  ...
  const projectEvents = [];
  const endEvents = [];
  const blobs = await extractAndParseReports(
    dir,
    shardReportFiles,
    internalizer,
    printStatus,
  ...
  for (let i = 0; i < blobs.length; ++i) {
    ...
    eventPatchers.patchers.push(
      new IdsPatcher(stringPool, metadata.name, String(i), globalTestIdSet),
    );
    if (rootDirOverride)
      eventPatchers.patchers.push(
        new PathSeparatorPatcher(metadata.pathSeparator),
      );
  ...
  return {
    ...
    reports,
    epilogue: [mergeEndEvents(endEvents), { method: "onExit", params: void 0 }],
...
function mergeConfigureEvents(configureEvents, rootDirOverride) {
  ...
  if (rootDirOverride) {
    ...
    const rootDirs = new Set(
      configureEvents.map((e) => e.params.config.rootDir),
    );
    if (rootDirs.size > 1) {
      throw new Error(
        [
...
function mergeConfigs(to, from) {
  return {
    ...
    metadata: {
      ...
      ...from.metadata,
      actualWorkers:
        (to.metadata.actualWorkers || 0) + (from.metadata.actualWorkers || 0),
    },
    shard: null,
...
function mergeEndEvents(endEvents) {
  ...
  for (const { event } of endEvents) {
    ...
    if (shardResult.status === "failed") status = "failed";
    else if (shardResult.status === "timedout" && status !== "failed")
      status = "timedout";
    else if (
      shardResult.status === "interrupted" &&
      status !== "failed" &&
      status !== "timedout"
    )
      status = "interrupted";
...
var IdsPatcher = class {
  ...
  _updateTestIds(suite) {
    suite.entries.forEach((entry) => {
      if ("testId" in entry) this._updateTestId(entry);
      else this._updateTestIds(entry);
    });
  }
  _updateTestId(test) {
  ...
  _mapTestId(testId) {
    ...
    if (this._testIdsMap.has(t1)) return this._testIdsMap.get(t1);
...
var AttachmentPathPatcher = class {
  ...
  _patchAttachments(attachments) {
    ...
    for (const attachment of attachments) {
      if (!attachment.path) continue;
      const joined = import_path19.default.resolve(
        resourceRoot,
        attachment.path,
      );
  ...
  patchEvent(jsonEvent) {
    ...
    if (jsonEvent.method === "onTestEnd") {
      ...
      test.annotations?.forEach((annotation) =>
        this._updateAnnotationLocation(annotation),
      );
      const testResult = jsonEvent.params.result;
      testResult.annotations?.forEach((annotation) =>
        this._updateAnnotationLocation(annotation),
      );
...
var GlobalErrorPatcher = class {
  ...
  patchEvent(event) {
    ...
    if (error.message !== void 0) error.message = this._prefix + error.message;
    if (error.stack !== void 0) error.stack = this._prefix + error.stack;
  }
};
var WorkerIndexPatcher = class {
  ...
  patchEvent(event) {
    if (event.method === "onTestBegin") {
      this._maxWorkerIndex = Math.max(
        this._maxWorkerIndex,
        event.params.result.workerIndex,
      );
...
var BlobModernizer = class {
  ...
  _modernize(fromVersion, event) {
    ...
    for (
      ...
      events = this[`_modernize_${version}_to_${version + 1}`].call(
        this,
        events,
      );
    return events;
