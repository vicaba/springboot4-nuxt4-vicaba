...
const { minimatch } = require("playwright-core/lib/utilsBundle");
...
const { stringifyStackFrames, filteredStackTrace } =
  require("playwright-core/lib/coreBundle").utils;
const { ansiRegex, isString, stripAnsiEscapes } =
...
function resolveReporterOutputPath(defaultValue, configDir, configValue) {
  if (configValue) return import_path.default.resolve(configDir, configValue);
...
function folderIsModule(folder) {
  ...
  if (!packageJsonIsModuleCache.has(packageJsonPath)) {
    ...
    try {
      isModule =
        JSON.parse(import_fs.default.readFileSync(packageJsonPath, "utf8"))
          .type === "module";
    } catch {}
  ...
  if (!packageJsonMainFieldCache.has(packageJsonPath)) {
    ...
    try {
      mainField = JSON.parse(
        import_fs.default.readFileSync(packageJsonPath, "utf8"),
      ).main;
    } catch {}
    ...
    if (afterPathMapping) {
      ...
      const mainFieldResolved = mainField
        ? resolveImportSpecifierExtension(
            import_path.default.resolve(resolved, mainField),
          )
        : void 0;
      return (
        mainFieldResolved ||
        resolveImportSpecifierExtension(
          import_path.default.join(resolved, "index"),
        )
      );
    }
