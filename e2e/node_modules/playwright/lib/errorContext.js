...
function buildErrorContext(options) {
  ...
  if (!meaningfulErrors.length && !pageSnapshot) return void 0;
  const lines = [
    fixTestInstructions,
    "# Test info",
    "",
  ...
  if (meaningfulErrors.length) {
    ...
    for (const error of meaningfulErrors) {
      ...
      if (error.errorContext) {
        lines.push("", "```yaml", error.errorContext, "```");
      }
    }
  }
  if (pageSnapshot) {
...
function buildCodeFrame(error, testLocation) {
  ...
  if (!errorLocation) return void 0;
  let source;
  try {
  ...
  const message =
    stripAnsiEscapes(error.message || "").split("\n")[0] || void 0;
  const frame = scope.map(
    (line, index) =>
      `${start + index + 1 === errorLocation.line ? "> " : "  "}${(start + index + 1).toString().padEnd(lineNumberWidth, " ")} | ${line}`,
  );
  if (message)
    frame.splice(
      errorLocation.line - start,
      0,
      `${" ".repeat(lineNumberWidth + 2)} | ${" ".repeat(Math.max(0, errorLocation.column - 2))} ^ ${message}`,
    );
  return frame.join("\n");
}
