...
const utilityFixtures = {
  ...
  _setupArtifacts: [
    async (
      { playwright, screenshot, _combinedContextOptions },
      use,
      testInfo,
    ) => {
      testInfo.setTimeout(testInfo.project.timeout);
      const artifactsRecorder = new ArtifactsRecorder(
        playwright,
        tracing().artifactsDir(),
        screenshot,
      );
      ...
      const csiListener = {
        onApiCallBegin: (data, channel) => {
          ...
          const isExpectCall =
            data.apiName === "locator._expect" ||
            data.apiName === "frame._expect" ||
            data.apiName === "page._expectScreenshot";
        ...
        runAfterCreateBrowserContext: async (context) => {
          ...
          if (currentTestInfo) {
            ...
            currentTestInfo._onCustomMessageCallback = (0,
            import_browserBackend.createCustomMessageHandler)(
              currentTestInfo,
              context,
            );
            await (0, import_browserBackend.runDaemonForContext)(
              currentTestInfo,
              context,
            );
          }
...
const playwrightFixtures = {
  ...
  headless: [
    ({ launchOptions }, use) => use(launchOptions.headless ?? true),
    { scope: "worker", option: true, box: true },
  ],
  channel: [
    ({ launchOptions }, use) => use(launchOptions.channel),
    { scope: "worker", option: true, box: true },
  ],
  launchOptions: [{}, { scope: "worker", option: true, box: true }],
  connectOptions: [
    async ({ _optionConnectOptions }, use) => {
  ...
  _browserOptions: [
    async ({ playwright, headless, channel, launchOptions }, use) => {
      const options = {
        handleSIGINT: false,
        ...launchOptions,
      ...
      if (headless !== void 0) options.headless = headless;
      if (channel !== void 0) options.channel = channel;
          ...
          connectOptions.wsEndpoint,
          {
            ...
            headers: {
              ...
              "x-playwright-launch-options":
                jsonStringifyForceASCII(_browserOptions),
  ...
  acceptDownloads: [
    ({ contextOptions }, use) => use(contextOptions.acceptDownloads ?? true),
    { option: true, box: true },
  ],
  bypassCSP: [
    ({ contextOptions }, use) => use(contextOptions.bypassCSP ?? false),
    { option: true, box: true },
  ],
  colorScheme: [
    ({ contextOptions }, use) =>
      use(
        contextOptions.colorScheme === void 0
          ? "light"
          : contextOptions.colorScheme,
      ),
    { option: true, box: true },
  ],
  deviceScaleFactor: [
    ({ contextOptions }, use) => use(contextOptions.deviceScaleFactor),
    { option: true, box: true },
  ],
  extraHTTPHeaders: [
    ({ contextOptions }, use) => use(contextOptions.extraHTTPHeaders),
    { option: true, box: true },
  ],
  geolocation: [
    ({ contextOptions }, use) => use(contextOptions.geolocation),
    { option: true, box: true },
  ],
  hasTouch: [
    ({ contextOptions }, use) => use(contextOptions.hasTouch ?? false),
    { option: true, box: true },
  ],
  httpCredentials: [
    ({ contextOptions }, use) => use(contextOptions.httpCredentials),
    { option: true, box: true },
  ],
  ignoreHTTPSErrors: [
    ({ contextOptions }, use) => use(contextOptions.ignoreHTTPSErrors ?? false),
    { option: true, box: true },
  ],
  isMobile: [
    ({ contextOptions }, use) => use(contextOptions.isMobile ?? false),
    { option: true, box: true },
  ],
  javaScriptEnabled: [
    ({ contextOptions }, use) => use(contextOptions.javaScriptEnabled ?? true),
    { option: true, box: true },
  ],
  locale: [
    ({ contextOptions }, use) => use(contextOptions.locale ?? "en-US"),
    { option: true, box: true },
  ],
  offline: [
    ({ contextOptions }, use) => use(contextOptions.offline ?? false),
    { option: true, box: true },
  ],
  permissions: [
    ({ contextOptions }, use) => use(contextOptions.permissions),
    { option: true, box: true },
  ],
  proxy: [
    ({ contextOptions }, use) => use(contextOptions.proxy),
    { option: true, box: true },
  ],
  storageState: [
    ({ contextOptions }, use) => use(contextOptions.storageState),
    { option: true, box: true },
  ],
  clientCertificates: [
    ({ contextOptions }, use) => use(contextOptions.clientCertificates),
    { option: true, box: true },
  ],
  timezoneId: [
    ({ contextOptions }, use) => use(contextOptions.timezoneId),
    { option: true, box: true },
  ],
  userAgent: [
    ({ contextOptions }, use) => use(contextOptions.userAgent),
    { option: true, box: true },
  ],
  viewport: [
    ({ contextOptions }, use) =>
      use(
        contextOptions.viewport === void 0
  ...
  serviceWorkers: [
    ({ contextOptions }, use) => use(contextOptions.serviceWorkers ?? "allow"),
    { option: true, box: true },
  ],
  ...
  _combinedContextOptions: [
    async (
      ...
      const options = {};
      if (acceptDownloads !== void 0) options.acceptDownloads = acceptDownloads;
      if (bypassCSP !== void 0) options.bypassCSP = bypassCSP;
      if (colorScheme !== void 0) options.colorScheme = colorScheme;
      if (deviceScaleFactor !== void 0)
        options.deviceScaleFactor = deviceScaleFactor;
      if (extraHTTPHeaders !== void 0)
        options.extraHTTPHeaders = extraHTTPHeaders;
      if (geolocation !== void 0) options.geolocation = geolocation;
      if (hasTouch !== void 0) options.hasTouch = hasTouch;
      if (httpCredentials !== void 0) options.httpCredentials = httpCredentials;
      if (ignoreHTTPSErrors !== void 0)
        options.ignoreHTTPSErrors = ignoreHTTPSErrors;
      if (isMobile !== void 0) options.isMobile = isMobile;
      if (javaScriptEnabled !== void 0)
        options.javaScriptEnabled = javaScriptEnabled;
      if (locale !== void 0) options.locale = locale;
      if (offline !== void 0) options.offline = offline;
      if (permissions !== void 0) options.permissions = permissions;
      if (proxy !== void 0) options.proxy = proxy;
      if (storageState !== void 0) options.storageState = storageState;
      if (clientCertificates?.length)
        options.clientCertificates =
          resolveClientCerticates(clientCertificates);
      if (timezoneId !== void 0) options.timezoneId = timezoneId;
      if (userAgent !== void 0) options.userAgent = userAgent;
      if (viewport !== void 0) options.viewport = viewport;
      if (baseURL !== void 0) options.baseURL = baseURL;
      if (serviceWorkers !== void 0) options.serviceWorkers = serviceWorkers;
      await use({
        ...contextOptions,
        ...options,
      });
  ...
  _setupContextOptions: [
    async (
      { playwright, actionTimeout, navigationTimeout, testIdAttribute },
      use,
      _testInfo,
    ) => {
      ...
      testInfo._onCustomMessageCallback = () =>
        Promise.reject(
          new Error(
            "Only tests that use default Playwright context or page fixture support test_debug",
          ),
        );
      if (debugMode() === "inspector") testInfo._setIgnoreTimeouts(true);
  ...
  _contextFactory: [
    async (
      ...
      const captureVideo =
        shouldCaptureVideo(videoMode, testInfo) && !_reuseContext;
      ...
      await use(async (options) => {
        ...
        const close = async () => {
          ...
          const preserveVideo =
            captureVideo && shouldPreserveVideo(videoMode, testInfo);
          if (preserveVideo) {
            const { pagesWithVideo: pagesForVideo } = contexts.get(context);
            const videos = pagesForVideo
              .map((p) => p.video())
              .filter((video2) => !!video2);
            await Promise.all(
              videos.map(async (v) => {
                try {
                  const savedPath = testInfo.outputPath(
                    `video${counter ? "-" + counter : ""}.webm`,
                  );
                  ++counter;
  ...
  _reuseContext: [
    async ({ video, _optionContextReuseMode, reuseContext }, use) => {
      let mode = _optionContextReuseMode;
      if (process.env.PW_TEST_REUSE_CONTEXT || reuseContext)
        mode = "when-possible";
      const reuse =
        mode === "when-possible" && normalizeVideoMode(video) === "off";
      await use(reuse);
    },
    { scope: "worker", title: "context", box: true },
  ],
  context: async (
    { browser, video, _reuseContext, _contextFactory },
    use,
    testInfoPublic,
  ) => {
    ...
    const context = await browserImpl._wrapApiCall(
      () => browserImpl._newContextForReuse(),
      { internal: true },
    );
    ...
    const closeReason =
      testInfo.status === "timedOut"
        ? "Test timeout of " + testInfo.timeout + "ms exceeded."
        : "Test ended.";
    await browserImpl._wrapApiCall(
      () => browserImpl._disconnectFromReusedContext(closeReason),
      { internal: true },
    );
  },
...
function connectOptionsFromEnv() {
  ...
  if (!wsEndpoint) return void 0;
  const headers = process.env.PW_TEST_CONNECT_HEADERS
    ? JSON.parse(process.env.PW_TEST_CONNECT_HEADERS)
    : void 0;
  return {
    wsEndpoint,
    headers,
...
class SnapshotRecorder {
  ...
  fixOrdinal() {
    this._ordinal = this.testInfo.attachments.filter(
      (a) => a.name === this._name,
    ).length;
  }
  shouldCaptureUponFinish() {
    return (
      ...
      (this._mode === "on-first-failure" &&
        this.testInfo._isFailure() &&
        this.testInfo.retry === 0)
    );
  }
  async maybeCapture() {
    if (!this.shouldCaptureUponFinish()) return;
    await Promise.all(
      this._artifactsRecorder._playwright
        ._allPages()
        .map((page) => this._snapshotPage(page, false)),
    );
  }
  async persistTemporary() {
    if (this.shouldCaptureUponFinish()) {
      await Promise.all(
        this._temporary.map(async (file) => {
          try {
  ...
  async captureTemporary(context) {
    if (
      this._mode === "on" ||
      this._mode === "only-on-failure" ||
      (this._mode === "on-first-failure" && this.testInfo.retry === 0)
    )
      await Promise.all(
        context.pages().map((page) => this._snapshotPage(page, true)),
      );
  }
  ...
  _createAttachmentPath() {
    ...
    ++this._ordinal;
    const path2 = this.testInfo.outputPath(
      `test-${testFailed ? "failed" : "finished"}-${index}${this._extension}`,
    );
    return path2;
  }
  _createTemporaryArtifact(...name) {
    const file = import_path.default.join(
      this._artifactsRecorder._artifactsDir,
      ...name,
    );
    return file;
  }
  async _snapshotPage(page, temporary) {
    if (page[this.testInfo._uniqueSymbol]) return;
    page[this.testInfo._uniqueSymbol] = true;
    try {
      const path2 = temporary
        ? this._createTemporaryArtifact(createGuid() + this._extension)
        : this._createAttachmentPath();
      await this._doSnapshot(page, path2);
      if (temporary) this._temporary.push(path2);
      else this._attach(path2);
    } catch {}
  }
  get testInfo() {
...
class ArtifactsRecorder {
  constructor(playwright, artifactsDir, screenshot) {
    ...
    const screenshotOptions =
      typeof screenshot === "string" ? void 0 : screenshot;
    this._startedCollectingArtifacts = /* @__PURE__ */ Symbol(
      "startedCollectingArtifacts",
    );
    this._screenshotRecorder = new SnapshotRecorder(
      this,
      normalizeScreenshotMode(screenshot),
      "screenshot",
      "image/png",
      ".png",
      async (page, path2) => {
        await page._wrapApiCall(
          async () => {
  ...
  async willStartTest(testInfo) {
    ...
    testInfo._onDidFinishTestFunctionCallbacks.add(() =>
      this.didFinishTestFunction(),
    );
    this._screenshotRecorder.fixOrdinal();
    await Promise.all(
      this._playwright
        ._allContexts()
        .map((context) => this.didCreateBrowserContext(context)),
    );
    const existingApiRequests = Array.from(this._playwright.request._contexts);
    await Promise.all(
      existingApiRequests.map((c) => this.didCreateRequestContext(c)),
    );
  }
  ...
  async _takePageSnapshot(context) {
    ...
    if (this._testInfo.errors.some((e) => e.errorContext)) return;
    ...
    try {
      await page._wrapApiCall(
        async () => {
          this._pageSnapshot = await page.ariaSnapshot({
            mode: "ai",
            timeout: 5e3,
          });
        },
  ...
  async didFinishTest() {
    ...
    await Promise.all(
      leftoverContexts
        ...
        .concat(
          leftoverApiRequests.map(async (context2) => {
            await this._stopTracing(context2, context2.tracing);
          }),
        ),
    );
    ...
    if (context) await this._takePageSnapshot(context);
    if (this._testInfo.errors.length > 0) {
      const hasMatcherAriaSnapshot = this._testInfo.errors.some(
        (e) => e.errorContext,
      );
      const errorContextContent = (0, import_errorContext.buildErrorContext)({
        titlePath: this._testInfo.titlePath,
        location: {
          file: this._testInfo.file,
          line: this._testInfo.line,
          column: this._testInfo.column,
        },
  ...
  async _startTraceChunkOnContextCreation(channelOwner, tracing2) {
    await channelOwner._wrapApiCall(
      async () => {
...
async function installScreencastTitleUpdater(testInfo, context, testAnnotate) {
  if (!testAnnotate) return;
  const testTitle =
    testAnnotate.level === "file"
