...
var cacheDir =
  ...
  (() => {
    ...
    return import_path.default.join(
      import_os.default.tmpdir(),
      `playwright-transform-cache-` + process.geteuid?.(),
    );
  })();
  ...
  sourceMapSupport.install({
    ...
    retrieveSourceMap(source) {
      ...
      try {
        return {
          map: JSON.parse(
            import_fs.default.readFileSync(sourceMapPath, "utf-8"),
          ),
          url: source,
        };
      } catch {
...
function readCodeCache(codePath) {
  ...
  const sha1Length = 40;
  if (
    firstLine.length !== "// ".length + sha1Length ||
...
function getFromCompilationCache(filename, contentHash, moduleUrl) {
  ...
  return {
    addToCache: (code, map, data) => {
      if ((0, import_globals.isWorkerProcess)()) return {};
      clearOldCacheEntries(cacheFolderName, filePathHash);
      import_fs.default.mkdirSync(import_path.default.dirname(cachePath), {
        recursive: true,
      });
      if (map)
        import_fs.default.writeFileSync(
      ...
      if (data.size)
        import_fs.default.writeFileSync(
          dataPath,
          JSON.stringify(Object.fromEntries(data.entries()), void 0, 2),
          "utf8",
        );
      writeCodeCache(codePath, code);
      const serializedCache = _innerAddToCompilationCacheAndSerialize(
        filename,
        { codePath, sourceMapPath, dataPath, moduleUrl },
      );
...
function serializeCompilationCache() {
  return {
    ...
    fileDependencies: [...fileDependencies.entries()].map(
      ([filename, deps]) => [filename, [...deps]],
    ),
    externalDependencies: [...externalDependencies.entries()].map(
      ([filename, deps]) => [filename, [...deps]],
    ),
  };
}
function addToCompilationCache(payload) {
  ...
  for (const entry of payload.fileDependencies) {
    ...
    fileDependencies.set(
      entry[0],
      /* @__PURE__ */ new Set([...entry[1], ...existing]),
    );
  }
...
function calculateCachePath(filePath, cacheFolderName, hashPrefix) {
  const fileName2 =
    ...
    "_" +
    import_path.default
      .basename(filePath, import_path.default.extname(filePath))
      .replace(/\W/g, "");
...
function clearOldCacheEntries(cacheFolderName, filePathHash) {
  ...
  try {
    const cachedRelevantFiles = import_fs.default
      .readdirSync(cachePath)
      .filter((file2) => file2.startsWith(filePathHash));
    for (const file2 of cachedRelevantFiles)
      import_fs.default.rmSync(import_path.default.join(cachePath, file2), {
        force: true,
      });
  } catch {}
}
...
function stopCollectingFileDeps(filename) {
  if (!depsCollector2) return;
  depsCollector2.delete(filename);
  for (const dep of depsCollector2) {
    if (belongsToNodeModules(dep)) depsCollector2.delete(dep);
  }
...
function setExternalDependencies(filename, deps) {
  const depsSet = new Set(
    deps.filter((dep) => !belongsToNodeModules(dep) && dep !== filename),
  );
  externalDependencies.set(filename, depsSet);
}
function fileDependenciesForTest() {
  return Object.fromEntries(
    [...fileDependencies.entries()].map((entry) => [
      import_path.default.basename(entry[0]),
      [...entry[1]].map((f) => import_path.default.basename(f)).sort(),
    ]),
  );
}
...
function affectedTestFiles(changes) {
  ...
  for (const change of changes) collectAffectedTestFiles(change, result2);
  return [...result2];
}
...
function dependenciesForTestFile(filename) {
  ...
  for (const dep of externalDependencies.get(filename) || []) result2.add(dep);
  return result2;
}
...
function belongsToNodeModules(file2) {
  if (
    file2.includes(
      `${import_path.default.sep}node_modules${import_path.default.sep}`,
    )
  )
    return true;
  if (
    file2.startsWith(kPlaywrightInternalPrefix) &&
    (file2.endsWith(".js") || file2.endsWith(".mjs"))
  )
    return true;
  return false;
}
async function getUserData(pluginName) {
  ...
  for (const [fileName2, cache] of memoryCache) {
    ...
    if (!import_fs.default.existsSync(cache.dataPath)) continue;
    const data = JSON.parse(
      await import_fs.default.promises.readFile(cache.dataPath, "utf8"),
    );
    if (data[pluginName]) result2.set(fileName2, data[pluginName]);
  }
  return result2;
}
...
var { stringifyStackFrames, filteredStackTrace } =
  require("playwright-core/lib/coreBundle").utils;
var { ansiRegex, isString, stripAnsiEscapes } =
...
function filterStackTrace(e) {
  ...
  const stackLines = stringifyStackFrames(
    filteredStackTrace(e.stack?.split("\n") || []),
  );
  return {
...
function createFileMatcher(patterns) {
  ...
  for (const pattern of Array.isArray(patterns) ? patterns : [patterns]) {
    if (isRegExp(pattern)) {
      ...
      if (!pattern.startsWith("**/")) filePatterns.push("**/" + pattern);
  ...
  return (filePath) => {
    ...
    if (import_path2.default.sep === "\\") {
      ...
      for (const re of reList) {
        re.lastIndex = 0;
        if (re.test(unixPath)) return true;
      }
    }
...
function mergeObjects(a, b, c) {
  ...
  for (const x of [b, c].filter(Boolean)) {
    for (const [name, value] of Object.entries(x)) {
      if (!Object.is(value, void 0)) result2[name] = value;
    }
  }
  return result2;
}
...
function relativeFilePath(file2) {
  if (!import_path2.default.isAbsolute(file2)) return file2;
  return import_path2.default.relative(process.cwd(), file2);
}
function formatLocation(location) {
  return (
    relativeFilePath(location.file) +
    ":" +
    location.line +
    ":" +
    location.column
...
function folderIsModule(folder) {
  ...
  if (!packageJsonIsModuleCache.has(packageJsonPath)) {
    ...
    try {
      isModule2 =
        JSON.parse(import_fs2.default.readFileSync(packageJsonPath, "utf8"))
          .type === "module";
    } catch {}
  ...
  if (!packageJsonMainFieldCache.has(packageJsonPath)) {
    ...
    try {
      mainField = JSON.parse(
        import_fs2.default.readFileSync(packageJsonPath, "utf8"),
      ).main;
    } catch {}
...
function resolveImportSpecifierExtension(resolved) {
  ...
  for (const [ext, others] of kExtLookups) {
    ...
    for (const other of others) {
      const modified =
        resolved.substring(0, resolved.length - ext.length) + other;
...
function resolveImportSpecifierAfterMapping(resolved, afterPathMapping) {
  ...
  if (dirExists(resolved)) {
    ...
    if (afterPathMapping) {
      ...
      const mainFieldResolved = mainField
        ? resolveImportSpecifierExtension(
            import_path2.default.resolve(resolved, mainField),
          )
        : void 0;
      return (
        mainFieldResolved ||
        resolveImportSpecifierExtension(
          import_path2.default.join(resolved, "index"),
        )
      );
    }
...
var FullConfigInternal = class {
  constructor(location, userConfig, configCLIOverrides, metadata) {
    ...
    const packageJsonDir = packageJsonPath
      ? import_path3.default.dirname(packageJsonPath)
      : process.cwd();
    this.configDir = configDir;
    this.configCLIOverrides = configCLIOverrides;
    const privateConfiguration = userConfig["@playwright/test"];
    this.plugins = (privateConfiguration?.plugins || []).map((p) => ({
      factory: p,
    }));
    ...
    this.globalSetups = (
      Array.isArray(userConfig.globalSetup)
        ? userConfig.globalSetup
        : [userConfig.globalSetup]
    )
      .map((s) => resolveScript(s, configDir))
      .filter((script) => script !== void 0);
    this.globalTeardowns = (
      Array.isArray(userConfig.globalTeardown)
        ? userConfig.globalTeardown
        : [userConfig.globalTeardown]
    )
      .map((s) => resolveScript(s, configDir))
      .filter((script) => script !== void 0);
    userConfig.metadata = userConfig.metadata || {};
    const globalTags = Array.isArray(userConfig.tag)
      ? userConfig.tag
      : userConfig.tag
        ? [userConfig.tag]
        : [];
    for (const tag of globalTags) {
      if (tag[0] !== "@")
        throw new Error(
          `Tag must start with "@" symbol, got "${tag}" instead.`,
        );
    }
    this.config = {
      ...
      failOnFlakyTests: takeFirst(
        configCLIOverrides.failOnFlakyTests,
        userConfig.failOnFlakyTests,
        false,
      ),
      forbidOnly: takeFirst(
        configCLIOverrides.forbidOnly,
        userConfig.forbidOnly,
        false,
      ),
      fullyParallel: takeFirst(
        configCLIOverrides.fullyParallel,
        userConfig.fullyParallel,
        false,
      ),
      globalSetup: this.globalSetups[0] ?? null,
      globalTeardown: this.globalTeardowns[0] ?? null,
      globalTimeout: takeFirst(
        configCLIOverrides.debug ? 0 : void 0,
        configCLIOverrides.globalTimeout,
        userConfig.globalTimeout,
        0,
      ),
      grep: takeFirst(userConfig.grep, defaultGrep),
      grepInvert: takeFirst(userConfig.grepInvert, null),
      maxFailures: takeFirst(
        configCLIOverrides.debug ? 1 : void 0,
        configCLIOverrides.maxFailures,
      ...
      reporter: takeFirst(
        configCLIOverrides.reporter,
        resolveReporters(userConfig.reporter, configDir),
        [[defaultReporter]],
      ),
      ...
      tags: globalTags,
      updateSnapshots: takeFirst(
        configCLIOverrides.updateSnapshots,
        userConfig.updateSnapshots,
        "missing",
      ),
      updateSourceMethod: takeFirst(
        configCLIOverrides.updateSourceMethod,
        userConfig.updateSourceMethod,
        "patch",
      ),
      version: import_package2.packageJSON.version,
      workers: resolveWorkers(
        takeFirst(
          configCLIOverrides.debug || configCLIOverrides.pause ? 1 : void 0,
          configCLIOverrides.workers,
          userConfig.workers,
          "50%",
        ),
      ),
    ...
    const projectConfigs = configCLIOverrides.projects ||
      userConfig.projects || [{ ...userConfig, workers: void 0 }];
    this.projects = projectConfigs.map(
      (p) =>
        new FullProjectInternal(
          configDir,
          userConfig,
          this,
          p,
          this.configCLIOverrides,
          packageJsonDir,
        ),
    );
...
var FullProjectInternal = class {
  constructor(
    ...
    const testDir = takeFirst(
      pathResolve(configDir, projectConfig.testDir),
      pathResolve(configDir, config.testDir),
      fullConfig.configDir,
    );
    this.snapshotPathTemplate = takeFirst(
      projectConfig.snapshotPathTemplate,
    ...
    this.project = {
      ...
      outputDir: takeFirst(
        configCLIOverrides.outputDir,
        pathResolve(configDir, projectConfig.outputDir),
        pathResolve(configDir, config.outputDir),
        import_path3.default.join(packageJsonDir, "test-results"),
      ),
      ...
      testDir,
      snapshotDir: takeFirst(
        pathResolve(configDir, projectConfig.snapshotDir),
        pathResolve(configDir, config.snapshotDir),
        testDir,
      ),
      ...
      timeout: takeFirst(
        configCLIOverrides.debug === "inspector" ? 0 : void 0,
        configCLIOverrides.timeout,
      ...
      ignoreSnapshots: takeFirst(
        configCLIOverrides.ignoreSnapshots,
        projectConfig.ignoreSnapshots,
        config.ignoreSnapshots,
        false,
      ),
    };
    this.fullyParallel = takeFirst(
      configCLIOverrides.fullyParallel,
      projectConfig.fullyParallel,
      config.fullyParallel,
      void 0,
    );
    this.expect = takeFirst(projectConfig.expect, config.expect, {});
    if (this.expect.toHaveScreenshot?.stylePath) {
      const stylePaths = Array.isArray(this.expect.toHaveScreenshot.stylePath)
        ? this.expect.toHaveScreenshot.stylePath
        : [this.expect.toHaveScreenshot.stylePath];
      this.expect.toHaveScreenshot.stylePath = stylePaths.map((stylePath) =>
        import_path3.default.resolve(configDir, stylePath),
      );
    }
    this.respectGitIgnore = takeFirst(
      projectConfig.respectGitIgnore,
      config.respectGitIgnore,
      !projectConfig.testDir && !config.testDir,
    );
    this.workers = projectConfig.workers
      ? resolveWorkers(projectConfig.workers)
      : void 0;
...
function resolveWorkers(workers) {
  if (typeof workers === "string") {
    if (workers.endsWith("%")) {
      ...
      if (percent < 1)
        throw new Error(
          `Workers must be a positive number, received ${percent}.`,
        );
    ...
    if (parsedWorkers < 1)
      throw new Error(
        `Workers must be a positive number, received ${parsedWorkers}.`,
      );
...
function resolveProjectDependencies(projects) {
  ...
  for (const project of projects) {
    for (const dependencyName of project.project.dependencies) {
      const dependencies = projects.filter(
        (p) => p.project.name === dependencyName,
      );
      if (!dependencies.length)
        throw new Error(
          `Project '${project.project.name}' depends on unknown project '${dependencyName}'`,
        );
      if (dependencies.length > 1)
        throw new Error(
          `Project dependencies should have unique names, reading ${dependencyName}`,
        );
      project.deps.push(...dependencies);
    }
    if (project.project.teardown) {
      const teardowns = projects.filter(
        (p) => p.project.name === project.project.teardown,
      );
      if (!teardowns.length)
        throw new Error(
          `Project '${project.project.name}' has unknown teardown project '${project.project.teardown}'`,
        );
      if (teardowns.length > 1)
        throw new Error(
          `Project teardowns should have unique names, reading ${project.project.teardown}`,
        );
  ...
  for (const teardown of teardownSet) {
    if (teardown.deps.length)
      throw new Error(
        `Teardown project ${teardown.project.name} must not have dependencies`,
      );
  }
  for (const project of projects) {
    for (const dep of project.deps) {
      if (teardownSet.has(dep))
        throw new Error(
          `Project ${project.project.name} must not depend on a teardown project ${dep.project.name}`,
        );
    }
  }
}
...
var configInternalSymbol = /* @__PURE__ */ Symbol("configInternalSymbol");
...
function resolveConfigFile(baseConfigFile, referencedConfigFile) {
  ...
  let resolvedConfigFile = import_path4.default.resolve(
    currentDir,
    referencedConfigFile,
  );
  if (
    referencedConfigFile.includes("/") &&
    referencedConfigFile.includes(".") &&
    !import_fs4.default.existsSync(resolvedConfigFile)
  )
    resolvedConfigFile = import_path4.default.join(
      currentDir,
      "node_modules",
      referencedConfigFile,
    );
...
function innerLoadTsConfig(
  ...
  if (parsedConfig.compilerOptions?.baseUrl !== void 0) {
    result2.absoluteBaseUrl = import_path4.default.resolve(
      import_path4.default.dirname(configFilePath),
  ...
  for (const ref of parsedConfig.references || [])
    references.push(
      innerLoadTsConfig(
        resolveConfigFile(configFilePath, ref.path),
        references,
        visited,
      ),
    );
  if (
    import_path4.default.basename(configFilePath) === "jsconfig.json" &&
    result2.allowJs === void 0
  )
...
function resolve(specifier, context, nextResolve) {
  if (context.parentURL && context.parentURL.startsWith("file://")) {
    ...
    if (resolved !== void 0) {
      specifier = new Set(context.conditions).has("import")
  ...
  if (result2?.url && result2.url.startsWith("file://"))
    currentFileDepsCollector()?.add(
      import_url.default.fileURLToPath(result2.url),
    );
  return result2;
}
...
function load(moduleUrl, context, nextLoad) {
  ...
  if (!moduleUrl.startsWith("file://")) return nextLoad(moduleUrl, context);
  const filename = import_url.default.fileURLToPath(moduleUrl);
  if (!shouldTransform(filename)) return nextLoad(moduleUrl, context);
  const isRequire = !new Set(context.conditions).has("import");
  const format = isRequire
    ? "commonjs"
    : kSupportedFormats.get(context.format) ||
      (fileIsModule(filename) ? "module" : "commonjs");
  const code = import_fs5.default.readFileSync(filename, "utf-8");
  const transformed = transformHook(
    code,
    filename,
    format === "module" ? moduleUrl : void 0,
  );
  return {
    format,
...
function addHook(transformHook2, shouldTransform2, extensions) {
  ...
  for (const extension of extensionsToOverwrite) {
    let newLoader2 = function (mod, filename, ...loaderArgs) {
      if (
        allSupportedExtensions.has(import_path5.default.extname(filename)) &&
        shouldTransform2(filename)
      ) {
...
var PortTransport = class {
  constructor(port, handler) {
    ...
    port.addEventListener("message", async (event) => {
      ...
      if (id) this._port.postMessage({ ackId: id, result: handlerResult });
    });
    this._resetRef();
  }
  post(method, params) {
...
async function setTransformConfig(config) {
  ...
  if (loaderChannel) await loaderChannel.send("setTransformConfig", { config });
}
...
function validateTsConfig(tsconfig) {
  ...
  return {
    ...
    pathsBase,
    paths: Object.entries(tsconfig.paths?.mapping || {})
...
function resolveHook(filename, specifier) {
  ...
  if (isRelativeSpecifier(specifier))
    return resolveImportSpecifierAfterMapping(
      import_path6.default.resolve(
        import_path6.default.dirname(filename),
        specifier,
      ),
      false,
    );
    ...
    for (const { key, values } of tsconfig.paths) {
      ...
      if (key.includes("*")) {
        if (keyPrefix) {
          ...
          matchedPartOfSpecifier = matchedPartOfSpecifier.substring(
            keyPrefix.length,
            matchedPartOfSpecifier.length,
          );
        }
        if (keySuffix) {
          ...
          matchedPartOfSpecifier = matchedPartOfSpecifier.substring(
            0,
            matchedPartOfSpecifier.length - keySuffix.length,
          );
        }
      } else {
...
function transformHook(originalCode, filename, moduleUrl) {
  ...
  const pluginsEpilogue = hasPreprocessor
    ? [[process.env.PW_TEST_SOURCE_TRANSFORM]]
    : [];
  ...
  const { cachedCode, addToCache, serializedCache } = getFromCompilationCache(
    filename,
    hash,
    moduleUrl,
  );
  if (cachedCode !== void 0) return { code: cachedCode, serializedCache };
  process.env.BROWSERSLIST_IGNORE_OLD_DATA = "true";
  const { babelTransform } = require(
    (0, import_package3.libPath)("transform", "babelBundle"),
  );
  transformData = /* @__PURE__ */ new Map();
  const setTransformDataForPlugin = (key, value) =>
    transformData.set(key, value);
  const wrappedPrologue = pluginsPrologue.map(([name, opts]) => [
    name,
    { ...(opts || {}), setTransformData: setTransformDataForPlugin },
  ]);
  const babelResult = babelTransform(
    originalCode,
    filename,
    !!moduleUrl,
    wrappedPrologue,
    pluginsEpilogue,
    _transformConfig.jsxImportSource,
  );
  if (!babelResult?.code) return { code: originalCode, serializedCache };
...
async function requireOrImport(file) {
  ...
  if (isModule) {
    ...
    if (loaderChannel) {
      await eval(`import(${JSON.stringify(fileName + ".esm.preflight")})`)
        .catch((error) =>
          debugTest(
            "Failed to load preflight for " +
              file +
              ", source maps may be missing for errors thrown during loading.",
            error,
          ),
        )
        .finally(nextTask);
  ...
  if (depsCollector) {
    ...
    if (module2) {
      ...
      for (const dep of cjsDeps) depsCollector.add(dep);
    }
  }
  return result;
}
...
function installCJSHooks() {
  ...
  function resolveFilename(specifier, parent, ...rest) {
    if (parent) {
      ...
      if (resolved !== void 0) specifier = resolved;
    }
...
var collectCJSDependencies = (module2, dependencies) => {
  module2.children.forEach((child) => {
    if (
      !belongsToNodeModules(child.filename) &&
      !dependencies.has(child.filename)
    ) {
...
function wrapFunctionWithLocation(func) {
  return (...args) => {
    ...
    Error.prepareStackTrace = (error, stackFrames) => {
      ...
      const file2 =
        fileName2 && fileName2.startsWith("file://")
          ? import_url2.default.fileURLToPath(fileName2)
          : fileName2;
      return {
        file: file2,
...
function registerESMLoader() {
  if (process.env.PW_DISABLE_TS_ESM) return;
  ...
  nodeModule.register(
    import_url2.default.pathToFileURL(
  ...
  void loaderChannel.send("setSingleTSConfig", {
    tsconfig: _singleTSConfigPath,
  });
  void loaderChannel.send("setTransformConfig", { config: _transformConfig });
  void loaderChannel.send("addToCompilationCache", {
    cache: serializeCompilationCache(),
  });
}
async function startCollectingFileDeps2() {
  startCollectingFileDeps();
  if (loaderChannel) await loaderChannel.send("startCollectingFileDeps", {});
}
...
var kDefineConfigWasUsed = /* @__PURE__ */ Symbol("defineConfigWasUsed");
var defineConfig = (...configs) => {
  ...
  for (let i = 1; i < configs.length; ++i) {
    ...
    result2 = {
      ...
      webServer: [
        ...(Array.isArray(result2.webServer)
          ? result2.webServer
          : result2.webServer
            ? [result2.webServer]
            : []),
    ...
    if (!result2.projects && !config.projects) continue;
...
async function deserializeConfig(data) {
  if (data.compilationCache) addToCompilationCache(data.compilationCache);
  return await loadConfig(
    data.location,
    data.configCLIOverrides,
    void 0,
    data.metadata ? JSON.parse(data.metadata) : void 0,
  );
}
...
async function loadConfig(
  ...
  const jsxImportSource = import_path7.default.dirname(
...
function validateConfig(file2, config) {
  if (typeof config !== "object" || !config)
    throw errorWithFile(
      file2,
      `Configuration file must export a single object`,
    );
  ...
  if ("shard" in config && config.shard !== void 0 && config.shard !== null) {
    ...
    if (
      ...
      throw errorWithFile(
        file2,
        `config.shard.total must be a positive number`,
      );
    if (
  ...
  if ("retryStrategy" in config && config.retryStrategy !== void 0) {
    if (
      typeof config.retryStrategy !== "string" ||
      !["immediate", "isolated"].includes(config.retryStrategy)
    )
      throw errorWithFile(
        file2,
        `config.retryStrategy must be one of "immediate" or "isolated"`,
      );
  }
  if ("tsconfig" in config && config.tsconfig !== void 0) {
    ...
    if (
      !import_fs7.default.existsSync(
        import_path7.default.resolve(file2, "..", config.tsconfig),
      )
    )
...
function isOptionFixture(registration) {
  ...
  constructor(
    ...
    for (const list of fixturesList) {
      ...
      if (Object.entries(selectedOverrides).length)
        this._appendFixtureList(
          { fixtures: selectedOverrides, location: optionOverrides.location },
          !!disallowWorkerFixtures,
          true,
        );
    }
    if (optionOverrides) {
      for (const key of overrideKeys) {
        ...
        if (registration && !isOptionFixture(registration))
          this._addLoadError(
            `Fixture "${key}" cannot be overridden in the configuration "use" section. Only fixtures registered with { option: true } can be set in the config.`,
            optionOverrides.location,
          );
      }
    }
    ...
    for (const entry of Object.entries(fixtures)) {
      ...
      if (previous && options) {
        if (previous.scope !== options.scope) {
          this._addLoadError(
            `Fixture "${name}" has already been registered as a { scope: '${previous.scope}' } fixture defined in ${formatLocation(previous.location)}.`,
            location,
          );
          continue;
        }
        if (previous.auto !== options.auto) {
          this._addLoadError(
            `Fixture "${name}" has already been registered as a { auto: '${previous.scope}' } fixture defined in ${formatLocation(previous.location)}.`,
            location,
          );
          continue;
        }
        if (previous.option !== options.option && options.option !== void 0) {
          this._addLoadError(
            `Fixture "${name}" has already been registered as a { option: ${previous.option} } fixture defined in ${formatLocation(previous.location)}.`,
            location,
          );
          continue;
        }
      ...
      if (!kScopeOrder.includes(options.scope)) {
        this._addLoadError(
          `Fixture "${name}" has unknown { scope: '${options.scope}' }.`,
          location,
        );
        continue;
      }
...
Make it top-level in the test file or put in the configuration file.`,
      ...
      if (fn === void 0 && previous && isOptionFixture(previous)) {
        let original = previous;
        while (!original.optionOverride && original.super)
          original = original.super;
        fn = original.fn;
      }
      const deps = fixtureParameterNames(fn, location, (e) =>
        this._onLoadError(e),
      );
      const registration = {
        id: "",
  ...
  validate() {
    ...
    const visit = (registration, boxedOnly) => {
      ...
      for (const name of registration.deps) {
        ...
        if (
          kScopeOrder.indexOf(registration.scope) >
          kScopeOrder.indexOf(dep.scope)
        ) {
          addDependencyError(
            `${registration.scope} fixture "${registration.name}" cannot depend on a ${dep.scope} fixture "${name}" defined in ${formatPotentiallyInternalLocation(dep.location)}.`,
            registration.location,
          );
          continue;
        }
        if (!markers.has(dep)) {
          ...
          addDependencyError(
            `Fixtures ${names2.join(" -> ")} -> "${dep.name}" form a dependency cycle: ${regs.map((r) => formatPotentiallyInternalLocation(r.location)).join(" -> ")} -> ${formatPotentiallyInternalLocation(dep.location)}`,
            dep.location,
          );
          continue;
        }
  ...
  validateFunction(fn, prefix, location) {
    for (const name of fixtureParameterNames(fn, location, (e) =>
      this._onLoadError(e),
    )) {
      const registration = this._registrations.get(name);
      if (!registration)
        this._addLoadError(
          `${prefix} has unknown parameter "${name}".`,
          location,
        );
    }
  }
  resolve(name, forFixture) {
    if (name === forFixture?.name) return forFixture.super;
...
var signatureSymbol = /* @__PURE__ */ Symbol("signature");
...
function splitByComma(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    if (s[i] === "{" || s[i] === "[") {
      ...
      if (token) result2.push(token);
      start = i + 1;
    }
  }
  const lastToken = s.substring(start).trim();
  if (lastToken) result2.push(lastToken);
  return result2;
}
...
function registrationId(registration) {
  ...
  const key =
    registration.name +
    "@@@" +
    (registration.super ? registrationId(registration.super) : "");
...
function toTestInfoErrorPayload(error) {
  const result2 = {};
  if (error.message !== void 0) result2.message = error.message;
  if (error.stack !== void 0) result2.stack = error.stack;
  if (error.value !== void 0) result2.value = error.value;
...
var PoolBuilder = class _PoolBuilder {
  ...
  _buildPoolForTest(test, testErrors) {
    ...
    for (const parent of parents) {
      if (parent._use.length)
        pool = new FixturePool(
          parent._use,
          (e) => this._handleLoadError(e, testErrors),
          pool,
          parent._type === "describe",
        );
      for (const hook of parent._hooks)
        pool.validateFunction(hook.fn, hook.type + " hook", hook.location);
      for (const modifier of parent._modifiers)
        pool.validateFunction(
          modifier.fn,
  ...
  _handleLoadError(e, testErrors) {
    if (testErrors) testErrors.push(e);
    else throw new Error(`${formatLocation(e.location)}: ${e.message}`);
  }
};
...
var import_package4 = require("../package");
var { ManualPromise } = require("playwright-core/lib/coreBundle").iso;
var { setTimeOrigin } = require("playwright-core/lib/coreBundle").iso;
var { startProfiling, stopProfiling } =
  require("playwright-core/lib/coreBundle").utils;
var { setBoxedStackPrefixes } = require("playwright-core/lib/coreBundle").utils;
...
setBoxedStackPrefixes([import_package4.packageRoot]);
  ...
  process.on("message", async (message) => {
    ...
    if (message.method === "__stop__") {
      const keys = /* @__PURE__ */ new Set([
        ...
        ...Object.keys(startingEnv),
      ]);
      const producedEnv = [...keys]
        .filter((key) => startingEnv[key] !== process.env[key])
...
async function gracefullyCloseAndExit(forceExit) {
  ...
  if (!gracefullyCloseCalled) {
    ...
    if (processName) await stopProfiling(processName).catch(() => {});
    process.exit(0);
  }
}
...
function filterTestsRemoveEmptySuites(suite, filter) {
  const filteredSuites = suite.suites.filter((child) =>
    filterTestsRemoveEmptySuites(child, filter),
  );
...
function bindFileSuiteToProject(project, suite) {
  const relativeFile = import_path8.default.relative(
...
function applyRepeatEachIndex(project, fileSuite, repeatEachIndex) {
  fileSuite.forEachTest((test, suite) => {
    if (repeatEachIndex) {
      ...
      const testId =
        suite._fileId + "-" + calculateSha13(testIdExpression).slice(0, 20);
      test.id = testId;
...
function filterSuiteWithOnlySemantics(suite, suiteFilter, testFilter) {
  const onlySuites = suite.suites.filter(
    (child) =>
      filterSuiteWithOnlySemantics(child, suiteFilter, testFilter) ||
      suiteFilter(child),
  );
...
function createFiltersFromArguments(args) {
  const matchers = args.map((arg) => {
    ...
    const locationMatcher2 = (file2, line, column) =>
      fileMatcher(file2) &&
  ...
  const locationMatcher = (file2, line, column) =>
    matchers.some((m) => m.locationMatcher(file2, line, column));
      ...
      oneOf: [
        {
          type: "string",
          pattern: "^@",
          patternError: "Tag must start with '@'",
        },
        {
          type: "array",
          items: {
...
var testTypeSymbol = /* @__PURE__ */ Symbol("testType");
var TestTypeImpl = class _TestTypeImpl {
  constructor(fixtures) {
    this.fixtures = fixtures;
    const test = wrapFunctionWithLocation(
      this._createTest.bind(this, "default"),
    );
    ...
    test.describe = wrapFunctionWithLocation(
      this._describe.bind(this, "default"),
    );
    test.describe.only = wrapFunctionWithLocation(
      this._describe.bind(this, "only"),
    );
    test.describe.configure = wrapFunctionWithLocation(
      this._configure.bind(this),
    );
    test.describe.fixme = wrapFunctionWithLocation(
      this._describe.bind(this, "fixme"),
    );
    test.describe.parallel = wrapFunctionWithLocation(
      this._describe.bind(this, "parallel"),
    );
    test.describe.parallel.only = wrapFunctionWithLocation(
      this._describe.bind(this, "parallel.only"),
    );
    test.describe.serial = wrapFunctionWithLocation(
      this._describe.bind(this, "serial"),
    );
    test.describe.serial.only = wrapFunctionWithLocation(
      this._describe.bind(this, "serial.only"),
    );
    test.describe.skip = wrapFunctionWithLocation(
      this._describe.bind(this, "skip"),
    );
    test.beforeEach = wrapFunctionWithLocation(
      this._hook.bind(this, "beforeEach"),
    );
    test.afterEach = wrapFunctionWithLocation(
      this._hook.bind(this, "afterEach"),
    );
    test.beforeAll = wrapFunctionWithLocation(
      this._hook.bind(this, "beforeAll"),
    );
    ...
    test.fail.only = wrapFunctionWithLocation(
      this._createTest.bind(this, "fail.only"),
    );
  ...
  _describe(type, location, titleOrFn, fnOrDetails, fn) {
    ...
    for (let parent = suite; parent; parent = parent.parent) {
      if (
        ...
        throw new Error(
          "describe.parallel cannot be nested inside describe.serial",
        );
      if (
        ...
        throw new Error(
          "describe.parallel cannot be nested inside describe with default mode",
        );
    }
  ...
  _configure(location, options) {
    ...
    if (!suite) return;
    if (options.timeout !== void 0) suite._timeout = options.timeout;
    if (options.retries !== void 0) suite._retries = options.retries;
    if (options.mode !== void 0) {
      if (suite._parallelMode !== "none")
        throw new Error(
          `"${suite._parallelMode}" mode is already assigned for the enclosing scope.`,
        );
      ...
      for (let parent = suite.parent; parent; parent = parent.parent) {
        if (
          ...
          throw new Error(
            "describe with parallel mode cannot be nested inside describe with serial mode",
          );
        if (
          parent._parallelMode === "default" &&
          suite._parallelMode === "parallel"
          ...
          throw new Error(
            "describe with parallel mode cannot be nested inside describe with default mode",
          );
      }
    }
  }
  _modifier(type, location, ...modifierArgs) {
    ...
    if (suite) {
      if (
        typeof modifierArgs[0] === "string" &&
        typeof modifierArgs[1] === "function" &&
        (type === "skip" || type === "fixme" || type === "fail")
      ) {
      ...
      if (
        ...
        typeof modifierArgs[2] === "function" &&
        (type === "skip" || type === "fixme" || type === "fail")
      ) {
        this._createTest(
          type,
    ...
    if (typeof modifierArgs[0] === "function")
      throw new Error(
        `test.${type}() with a function can only be called inside describe block`,
      );
  ...
  _abort(location, message) {
    ...
    if (!testInfo)
      throw new Error(
        `test.abort() can only be called inside a test or fixture`,
      );
  ...
  async _step(expectation, title, body, options = {}) {
    ...
    return await currentZone()
      .with("stepZone", step)
      .run(async () => {
        try {
          let result2 = void 0;
          result2 = await raceAgainstDeadline(
            async () => {
              try {
                return await step.info._runStepBody(
                  expectation === "skip",
                  body,
                  step.location,
                );
              } catch (e) {
                if (result2?.timedOut) testInfo._failWithError(e);
                throw e;
              }
            },
          ...
          if (result2.timedOut)
            throw new TimeoutError(
              `Step timeout of ${options.timeout}ms exceeded.`,
            );
          step.complete({});
...
Did you mean to call test.extend() with fixtures instead?`);
    const newFixtures = testTypeImpl.fixtures.filter(
      (theirs) =>
        !result2.fixtures.find((ours) => ours.fixtures === theirs.fixtures),
    );
...
var Suite = class _Suite extends Base {
  ...
  _detach(child) {
    ...
    if (this._entries.length === 0) this.parent?._detach(this);
  }
  allTests() {
    const result2 = [];
    const visit = (suite) => {
      for (const entry of suite._entries) {
        if (entry instanceof _Suite) visit(entry);
  ...
  _collectGrepTitlePath(path10) {
    if (this.parent) this.parent._collectGrepTitlePath(path10);
    if (this.title || this._type !== "describe") path10.push(this.title);
    path10.push(...this._tags);
  }
  _collectTagTitlePath(path10) {
    this.parent?._collectTagTitlePath(path10);
    if (this._type === "describe") path10.push(this.title);
    path10.push(...this._tags);
  }
  _getOnlyItems() {
    const items = [];
  ...
  _deepClone() {
    ...
    for (const entry of this._entries) {
      if (entry instanceof _Suite) suite._addSuite(entry._deepClone());
      else suite._addTest(entry._clone());
    }
    return suite;
  }
  _deepSerialize() {
    ...
    suite.entries = [];
    for (const entry of this._entries) {
      if (entry instanceof _Suite) suite.entries.push(entry._deepSerialize());
      else suite.entries.push(entry._serialize());
    }
    return suite;
  }
  static _deepParse(data) {
    ...
    for (const entry of data.entries) {
      if (entry.kind === "suite") suite._addSuite(_Suite._deepParse(entry));
      else suite._addTest(TestCase._parse(entry));
    }
    return suite;
  }
  forEachTest(visitor) {
    for (const entry of this._entries) {
      if (entry instanceof _Suite) entry.forEachTest(visitor);
      else visitor(entry, this);
    }
  }
  _serialize() {
    return {
...
var TestCase = class _TestCase extends Base {
  ...
  _applyPlanAnnotation(annotation) {
...
async function loadTestFile(file2, config, testErrors) {
  if (cachedFileSuites.has(file2)) return cachedFileSuites.get(file2);
  const suite = new Suite(
    import_path9.default.relative(config.config.rootDir, file2) ||
      import_path9.default.basename(file2),
    "file",
  );
  ...
  if (!(0, import_globals3.isWorkerProcess)()) await startCollectingFileDeps2();
  try {
  ...
  }
  {
    ...
    if (files.size === 1) {
      ...
      if (suite.location.file !== mappedFile) {
        if (
          import_path9.default.extname(mappedFile) !==
          import_path9.default.extname(suite.location.file)
        )
