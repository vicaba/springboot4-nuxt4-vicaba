...
const { startProfiling, stopProfiling } =
  require("playwright-core/lib/coreBundle").utils;
async function runTests(args, opts) {
  await startProfiling();
  const cliOverrides = overridesFromOptions(opts);
  cliOverrides.argv = process.argv;
  const config = await import_common.configLoader.loadConfigFromFile(
    opts.config,
    cliOverrides,
    opts.deps === false,
  );
  const options = {
    ...
    testList: opts.testList
      ? import_path.default.resolve(process.cwd(), opts.testList)
      : void 0,
    testListInvert: opts.testListInvert
      ? import_path.default.resolve(process.cwd(), opts.testListInvert)
      : void 0,
    shardWeights: resolveShardWeightsOption(),
  };
  import_runner.projectUtils.filterProjects(
    config.projects,
  ...
  if (opts.ui || opts.uiHost || opts.uiPort) {
    if (opts.onlyChanged)
      throw new Error(
        `--only-changed is not supported in UI mode. If you'd like that to change, see https://github.com/microsoft/playwright/issues/15075 for more details.`,
      );
    const status2 = await import_runner.testServer.runUIMode(
      opts.config,
      cliOverrides,
      {
        host: opts.uiHost,
        ...
        reporter: Array.isArray(opts.reporter)
          ? opts.reporter
          : opts.reporter
            ? [opts.reporter]
            : void 0,
    ...
    const exitCode2 =
      status2 === "interrupted" ? 130 : status2 === "passed" ? 0 : 1;
  ...
  if (process.env.PWTEST_WATCH) {
    if (opts.onlyChanged)
      throw new Error(
        `--only-changed is not supported in watch mode. If you'd like that to change, file an issue and let us know about your usecase for it.`,
      );
  ...
  const status = await import_runner.testRunner.runAllTestsWithConfig(
    config,
    options,
  );
  await stopProfiling("runner");
...
async function runTestServerAction(opts) {
  ...
  const status = await import_runner.testServer.runTestServer(
    opts.config,
    {},
    { host, port },
  );
...
async function clearCache(opts) {
  const runner = new import_runner.testRunner.TestRunner(
    import_common.configLoader.resolveConfigLocation(opts.config),
    {},
  );
  const { status } = await runner.clearCache(
    import_runner.runnerReporters.createErrorCollectingReporter(
...
function overridesFromOptions(options) {
  ...
  const overrides = {
    ...
    outputDir: options.output
      ? import_path.default.resolve(process.cwd(), options.output)
      : void 0,
    ...
    tsconfig: options.tsconfig
      ? import_path.default.resolve(process.cwd(), options.tsconfig)
      : void 0,
    ignoreSnapshots: options.ignoreSnapshots
      ? !!options.ignoreSnapshots
      : void 0,
  ...
  if (options.browser) {
    ...
    if (!["all", "chromium", "firefox", "webkit"].includes(browserOpt))
      throw new Error(
        `Unsupported browser "${options.browser}", must be one of "all", "chromium", "firefox" or "webkit"`,
      );
    const browserNames =
      browserOpt === "all" ? ["chromium", "firefox", "webkit"] : [browserOpt];
  ...
  if (options.headed) overrides.use.headless = false;
...
function resolveReporterOption(reporter) {
  if (!reporter || !reporter.length) return void 0;
...
function resolveShardWeightsOption() {
  ...
  if (!shardWeights) return void 0;
  return shardWeights.split(":").map((w) => {
    const weight = parseInt(w, 10);
    if (isNaN(weight) || weight < 0)
      throw new Error(
        `PWTEST_SHARD_WEIGHTS="${shardWeights}" weights must be non-negative numbers`,
      );
    return weight;
  });
}
function resolveReporter(id) {
  if (import_common.builtInReporters.includes(id)) return id;
