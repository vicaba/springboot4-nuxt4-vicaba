...
const { tomlArray, tomlBasicString, tomlMultilineBasicString } =
...
async function loadAgentSpecs() {
  ...
  return Promise.all(
    files
      ...
      .map((file) =>
        (0, import_agentParser.parseAgentSpec)(
          import_path.default.join(__dirname, file),
        ),
      ),
  );
}
class ClaudeGenerator {
  static async init(fullConfig, projectName, prompts) {
    ...
    await import_fs.default.promises.mkdir(".claude/agents", {
      recursive: true,
    });
    for (const agent of agents)
      await writeFile(
        `.claude/agents/${agent.name}.md`,
        ClaudeGenerator.agentSpec(agent),
        "\u{1F916}",
        "agent definition",
      );
    const mcpServer =
  ...
  static agentSpec(agent) {
    ...
    function asClaudeTool(tool) {
      ...
      if (!second) return (claudeToolMap.get(first) || [first]).join(", ");
...
class CodexGenerator {
  static async init(fullConfig, projectName, prompts) {
    ...
    const agents = await loadAgentSpecs();
    await import_fs.default.promises.mkdir(".codex/agents", {
      recursive: true,
    });
    for (const agent of agents)
      await writeFile(
        `.codex/agents/${CodexGenerator.codexName(agent)}.toml`,
        CodexGenerator.agentSpec(agent),
        "\u{1F916}",
        "agent definition",
      );
    initRepoDone();
  ...
  static agentSpec(agent) {
    ...
    for (const tool of agent.tools) {
      const [first, second] = tool.split("/");
      if (second && first === mcpName) enabledTools.push(second);
    }
    const sandboxMode = agent.tools.includes("edit")
      ? "workspace-write"
      : "read-only";
    const mcpServer =
      process.platform === "win32"
    ...
    lines.push(
      `description = ${tomlBasicString(agent.description + examples)}`,
    );
    lines.push(`sandbox_mode = ${tomlBasicString(sandboxMode)}`);
    lines.push(
      `developer_instructions = ${tomlMultilineBasicString(agent.instructions)}`,
    );
    lines.push("");
...
class OpencodeGenerator {
  static async init(fullConfig, projectName, prompts) {
    ...
    for (const agent of agents) {
      ...
      prompt.push("");
      prompt.push(
        ...agent.examples.map((example) => `<example>${example}</example>`),
      );
      await writeFile(
        `.opencode/prompts/${agent.name}.md`,
        prompt.join("\n"),
        "\u{1F916}",
        "agent definition",
      );
    ...
    await writeFile(
      "opencode.json",
      OpencodeGenerator.configuration(agents),
      "\u{1F527}",
  ...
  static configuration(agents) {
    ...
    for (const agent of agents) {
      ...
      for (const tool of agent.tools) asOpencodeTool(tools, tool);
    }
...
class CopilotGenerator {
  static async init(fullConfig, projectName, prompts) {
    ...
    const agents = await loadAgentSpecs();
    await import_fs.default.promises.mkdir(".github/agents", {
      recursive: true,
    });
    for (const agent of agents)
      await writeFile(
        `.github/agents/${agent.name}.agent.md`,
        CopilotGenerator.agentSpec(agent),
        "\u{1F916}",
        "agent definition",
      );
    await deleteFile(
      `.github/chatmodes/ \u{1F3AD} planner.chatmode.md`,
      "legacy planner chatmode",
    );
    await deleteFile(
      `.github/chatmodes/\u{1F3AD} generator.chatmode.md`,
      "legacy generator chatmode",
    );
    await deleteFile(
      `.github/chatmodes/\u{1F3AD} healer.chatmode.md`,
      "legacy healer chatmode",
    );
    await deleteFile(
      `.github/agents/ \u{1F3AD} planner.agent.md`,
      "legacy planner agent",
    );
    await deleteFile(
      `.github/agents/\u{1F3AD} generator.agent.md`,
      "legacy generator agent",
    );
    await deleteFile(
      `.github/agents/\u{1F3AD} healer.agent.md`,
      "legacy healer agent",
    );
    ...
    if (!import_fs.default.existsSync(".github/copilot-setup-steps.yml")) {
      const yaml2 = import_fs.default.readFileSync(
        import_path.default.join(__dirname, "copilot-setup-steps.yml"),
        "utf-8",
      );
      await writeFile(
    ...
    console.log("");
    console.log(
      " \u{1F527} TODO: GitHub > Settings > Copilot > Coding agent > MCP configuration",
    );
    console.log(
      "------------------------------------------------------------------",
    );
...
class VSCodeGenerator {
  static async init(fullConfig, projectName) {
    ...
    await import_fs.default.promises.mkdir(".github/chatmodes", {
      recursive: true,
    });
    for (const agent of agents)
      await writeFile(
        `.github/chatmodes/${nameMap.get(agent.name)}.chatmode.md`,
        VSCodeGenerator.agentSpec(agent),
        "\u{1F916}",
        "chatmode definition",
      );
    await VSCodeGenerator.appendToMCPJson();
  ...
  static agentSpec(agent) {
    const vscodeToolMap = /* @__PURE__ */ new Map([
      [
        "search",
        ["search/listDirectory", "search/fileSearch", "search/textSearch"],
      ],
      ["read", ["search/readFile"]],
      ["edit", ["edit/editFiles"]],
      ["write", ["edit/createFile", "edit/createDirectory"]],
    ]);
    const vscodeToolsOrder = [
      "edit/createFile",
    ...
    function asVscodeTool(tool) {
      const [first, second] = tool.split("/");
      if (second) return `${vscodeMcpName}/${second}`;
...
async function writeFile(filePath, content, icon, description) {
  console.log(
    ` ${icon} ${import_path.default.relative(process.cwd(), filePath)} ${colors.dim("- " + description)}`,
  );
...
async function deleteFile(filePath, description) {
  ...
  console.log(
    ` \u2702\uFE0F  ${import_path.default.relative(process.cwd(), filePath)} ${colors.dim("- " + description)}`,
  );
...
async function initRepo(fullConfig, projectName, options) {
  ...
  console.log(
    ` \u{1F3AD} Using project "${project.project.name}" as a primary project`,
  );
  if (!import_fs.default.existsSync("specs")) {
    await import_fs.default.promises.mkdir("specs");
    await writeFile(
      import_path.default.join("specs", "README.md"),
      `# Specs
...
`,
  ...
  if (!seedFile) {
    ...
    await writeFile(
      seedFile,
      import_seed.seedFileContent,
      "\u{1F331}",
      "default environment seed file",
    );
  }
  if (options.promptsFolder) {
    ...
    for (const promptFile of await import_fs.default.promises.readdir(
      __dirname,
    )) {
      if (!promptFile.endsWith(".prompt.md")) continue;
      const shortName = promptFile.replace(".prompt.md", "");
      const fileName = options.promptSuffix
        ? `${shortName}.${options.promptSuffix}.md`
        : `${shortName}.md`;
      ...
      await writeFile(
        import_path.default.join(options.promptsFolder, fileName),
        content,
        "\u{1F4DD}",
        "prompt template",
      );
