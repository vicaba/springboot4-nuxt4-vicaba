...
const { setBoxedStackPrefixes } =
  require("playwright-core/lib/coreBundle").utils;
const { gracefullyProcessExitDoNotHang } =
  require("playwright-core/lib/coreBundle").utils;
...
function addTestCommand(program2) {
  ...
  const options = testOptions.sort((a, b) =>
    a[0].replace(/-/g, "").localeCompare(b[0].replace(/-/g, "")),
  );
  options.forEach(([name, { description, choices, preset }]) => {
    ...
    if (preset) option.preset(preset);
  ...
  command.action(async (args, opts) => {
    ...
    const postDashCount =
      dashDashIndex >= 0 ? process.argv.length - 1 - dashDashIndex : 0;
...
function addShowReportCommand(program2) {
  ...
  command.action(async (report, options) => {
    await (0, import_reportActions.showReport)(
      report,
      options.host,
      +options.port,
    );
...
function addMergeReportsCommand(program2) {
  ...
  command.description(
    "merge multiple blob reports (for sharded tests) into a single report",
  );
  ...
  command.option(
    "--reporter <reporter>",
    `Reporter to use, comma-separated, can be ${import_common.builtInReporters.map((name) => `"${name}"`).join(", ")} (default: "${import_common.config.defaultReporter}")`,
  );
  command.addHelpText(
    "afterAll",
    `
...
function addTestMCPServerCommand(program2) {
  ...
  command.action(async (options) => {
    ...
    const factory = {
      ...
      toolSchemas: import_testBackend.testServerBackendTools.map(
        (tool) => tool.schema,
      ),
      create: async () =>
        new import_testBackend.TestServerBackend(options.config, {
          muteConsole: options.port === void 0,
          headless: options.headless,
        }),
      disposed: async () => {},
    };
    await import_coreBundle.tools.start(factory, {
...
function addInitAgentsCommand(program2) {
  ...
  command.option(
    "-c, --config <file>",
    `Configuration file to find a project to use for seed test`,
  );
  command.option("--project <project>", "Project to use for seed test");
  command.option(
    "--prompts",
    "Whether to include prompts in the agent initialization",
  );
  command.action(async (opts) => {
    const loadedConfig = await import_common.configLoader.loadConfigFromFile(
      opts.config,
    );
    if (opts.loop === "opencode") {
      ...
      await import_generateAgents.CodexGenerator.init(
        loadedConfig,
        opts.project,
        opts.prompts,
      );
...
function addInitSkillsCommand(program2) {
  ...
  command.action(async (opts) => {
    try {
      await import_coreBundle.tools.installSkills(
        import_coreBundle.tools.allSkills,
        opts.loop,
      );
    } catch (e) {
      console.error(e);
...
const testOptions = [
  ...
  [
    "--browser <browser>",
    {
      description: `Browser to use for tests, one of "all", "chromium", "firefox" or "webkit" (default: "chromium")`,
    },
  ],
  [
    "-c, --config <file>",
    {
      description: `Configuration file, or a test directory with optional "playwright.config.{m,c}?{js,ts}"`,
    },
  ],
  [
    "--debug [mode]",
    {
      ...
      choices: ["inspector", "cli"],
      preset: "inspector",
    },
  ],
  [
    "--fail-on-flaky-tests",
    { description: `Fail if any test is flagged as flaky (default: false)` },
  ],
  [
    "--forbid-only",
    { description: `Fail if test.only is called (default: false)` },
  ],
  [
    "--fully-parallel",
    { description: `Run all tests in parallel (default: false)` },
  ],
  [
    "--global-timeout <timeout>",
    {
      description: `Maximum time this test suite can run in milliseconds (default: unlimited)`,
    },
  ],
  [
    "-g, --grep <grep>",
    {
      description: `Only run tests matching this regular expression (default: ".*")`,
    },
  ],
  [
    "-G, --grep-invert <grep>",
  ...
  [
    "--headed",
    { description: `Run tests in headed browsers (default: headless)` },
  ],
  [
    "--ignore-snapshots",
  ...
  [
    "--last-failed-file <file>",
    {
      description: `Override the default path for the last-run JSON file used with --last-failed (default: <outputDir>/.last-run.json). Same as PLAYWRIGHT_LAST_RUN_OUTPUT_FILE environment variable.`,
    },
  ],
  [
    "--list",
  ...
  [
    "--output <dir>",
    { description: `Folder for output artifacts (default: "test-results")` },
  ],
  [
    "--only-changed [ref]",
    {
      description: `Only run test files that have been changed between 'HEAD' and 'ref'. Defaults to running all uncommitted changes. Only supports Git.`,
    },
  ],
  [
    "--pass-with-no-tests",
  ...
  [
    ...
    {
      description: `Only run tests from the specified list of projects, supports '*' wildcard (default: run all projects)`,
    },
  ],
  ...
  [
    "--reporter <reporter>",
    {
      description: `Reporter to use, comma-separated, can be ${import_common.builtInReporters.map((name) => `"${name}"`).join(", ")} (default: "${import_common.config.defaultReporter}")`,
    },
  ],
  [
    "--retries <retries>",
    {
      description: `Maximum retry count for flaky tests, zero for no retries (default: no retries)`,
    },
  ],
  [
    "--run-agents <mode>",
    {
      description: `Run agents to generate the code for page.perform`,
  ...
  [
    "--test-list <file>",
    {
      description: `Path to a file containing a list of tests to run. See https://playwright.dev/docs/test-cli for more details.`,
    },
  ],
  [
    "--test-list-invert <file>",
    {
      description: `Path to a file containing a list of tests to skip. See https://playwright.dev/docs/test-cli for more details.`,
    },
  ],
  [
    "--timeout <timeout>",
    {
      description: `Specify test timeout threshold in milliseconds, zero for unlimited (default: ${import_common.config.defaultTimeout})`,
    },
  ],
  [
    "--trace <mode>",
    { description: `Force tracing mode`, choices: kTraceModes },
  ],
  [
    "--tsconfig <path>",
    {
      description: `Path to a single tsconfig applicable to all imported files (default: look up tsconfig for each imported file separately)`,
    },
  ],
  ...
  [
    "--update-source-method <method>",
    {
      description: `Chooses the way source is updated (default: "patch")`,
      choices: ["overwrite", "3way", "patch"],
    },
  ],
  [
    "-j, --workers <workers>",
    {
      description: `Number of concurrent workers or percentage of logical CPU cores, use 1 to run in a single worker (default: 50%)`,
    },
  ],
