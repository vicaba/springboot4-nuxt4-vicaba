...
function relativeFilePath(file) {
  if (!import_path.default.isAbsolute(file)) return file;
...
function getContainedPath(parentPath, subPath = "") {
  ...
  if (
    ...
    resolvedPath.startsWith(parentPath + import_path.default.sep)
  )
    return resolvedPath;
  return null;
}
...
async function normalizeAndSaveAttachment(outputPath, name, options = {}) {
  ...
  if (
    (options.path !== void 0 ? 1 : 0) + (options.body !== void 0 ? 1 : 0) !==
    1
  )
    throw new Error(`Exactly one of "path" and "body" must be specified`);
  if (options.path !== void 0) {
    const hash = calculateSha1(options.path);
    if (!isString(name)) throw new Error('"name" should be string.');
    const sanitizedNamePrefix = sanitizeForFilePath(name) + "-";
    const dest = import_path.default.join(
      outputPath,
      "attachments",
      sanitizedNamePrefix + hash + import_path.default.extname(options.path),
    );
    await import_fs.default.promises.mkdir(import_path.default.dirname(dest), {
      recursive: true,
    });
    await import_fs.default.promises.copyFile(options.path, dest);
    const contentType =
      options.contentType ??
      (mime.getType(import_path.default.basename(options.path)) ||
...
var Fixture = class {
  constructor(runner, registration) {
    ...
    const isUserFixture =
      ...
      filterStackFile(this.registration.location.file);
    const title = this.registration.customTitle || this.registration.name;
    const location = isUserFixture ? this.registration.location : void 0;
    this._stepInfo = {
      title: `Fixture ${escapeWithQuotes(title, '"')}`,
      category: "fixture",
      location,
    };
    if (this.registration.box === "self") this._stepInfo = void 0;
    ...
    this._setupDescription = {
      ...
      location,
      slot:
        this.registration.timeout !== void 0
          ? {
  ...
  async setup(testInfo, runnable) {
    ...
    const run = () =>
      testInfo._runWithTimeout(
        { ...runnable, fixture: this._setupDescription },
        () => this._setupInternal(testInfo),
      );
    if (this._stepInfo) await testInfo._runAsStep(this._stepInfo, run);
    else await run();
  }
    ...
    this._selfTeardownComplete = (async () => {
      try {
        ...
        if (!useFuncStarted.isDone())
          throw new Error(
            `use() was not called in fixture "${this.registration.name}"`,
          );
      } catch (error) {
        this.failed = true;
        if (!useFuncStarted.isDone()) useFuncStarted.reject(error);
        else throw error;
      }
    })();
  ...
  async teardown(testInfo, runnable) {
    try {
      ...
      if (!testInfo._timeoutManager.isTimeExhaustedFor(fixtureRunnable)) {
        const run = () =>
          testInfo._runWithTimeout(fixtureRunnable, () =>
  ...
  async _teardownInternal() {
    ...
    if (this._usages.size !== 0) {
      console.error(
        "Internal error: fixture integrity at",
        this._teardownDescription.title,
      );
      this._usages.clear();
    }
...
var FixtureRunner = class {
  ...
  async resolveParametersForFunction(fn, testInfo, autoFixtures, runnable) {
    ...
    for (const registration of this.pool.autoFixtures()) {
      ...
      if (autoFixtures === "all-hooks-only")
        shouldRun =
          registration.scope === "worker" ||
          registration.auto === "all-hooks-included";
    ...
    auto.sort(
      (r1, r2) =>
        (r1.scope === "worker" ? 0 : 1) - (r2.scope === "worker" ? 0 : 1),
    );
  ...
  async resolveParametersAndRunFunction(fn, testInfo, autoFixtures, runnable) {
    const params = await this.resolveParametersForFunction(
      fn,
      testInfo,
      autoFixtures,
      runnable,
...
function getRequiredFixtureNames(fn, location) {
  return import_common.fixtures.fixtureParameterNames(
    fn,
...
var TimeoutManager = class {
  ...
  setIgnoreTimeouts(ignoreTimeouts) {
    if (this._ignoreTimeouts === ignoreTimeouts) return;
    this._ignoreTimeouts = ignoreTimeouts;
    if (this._running) {
      if (ignoreTimeouts)
        this._running.slot.elapsed += monotonicTime() - this._running.start;
      else this._running.start = monotonicTime();
      this._updateTimeout(this._running);
    }
  }
  interrupt() {
    if (this._running)
      this._running.timeoutPromise.reject(
        this._createTimeoutError(this._running),
      );
  }
  ...
  async withRunnable(runnable, cb) {
    ...
    try {
      if (debugTest.enabled) {
        debugTitle = runnable.fixture
          ? `${runnable.fixture.phase} "${runnable.fixture.title}"`
          : runnable.type;
        const location = runnable.location
          ? ` at "${formatLocation(runnable.location)}"`
          : ``;
      ...
      if (debugTest.enabled) debugTest(`finished ${debugTitle}`);
    }
  }
  _updateTimeout(running) {
    ...
    running.deadline =
      running.start + (running.slot.timeout - running.slot.elapsed);
  ...
  _createTimeoutError(running) {
    ...
    switch (runnable.type) {
      ...
      case "teardown": {
        ...
        else message = `Worker teardown timeout of ${timeout}ms exceeded.`;
        break;
      }
      case "skip":
      case "slow":
...
var { monotonicTime: monotonicTime2 } =
  require("playwright-core/lib/coreBundle").iso;
var { calculateSha1: calculateSha12, createGuid } =
  require("playwright-core/lib/coreBundle").utils;
var { SerializedFS } = require("playwright-core/lib/coreBundle").utils;
var { filteredStackTrace: filteredStackTrace2 } =
  require("playwright-core/lib/coreBundle").utils;
...
var TestTracing = class {
  ...
  _shouldCaptureTrace() {
    ...
    if (this._options?.mode === "on-all-retries" && this._testInfo.retry > 0)
      return true;
    if (
      this._options?.mode === "retain-on-first-failure" &&
      this._testInfo.retry === 0
    )
      return true;
    if (this._options?.mode === "retain-on-failure-and-retries") return true;
    return false;
  }
  async startIfNeeded(value) {
    ...
    if (!this._liveTraceFile && this._options.live) {
      this._liveTraceFile = {
        file: import_path2.default.join(
          this._tracesDir,
          `${this._testInfo.testId}-test.trace`,
        ),
        fs: new SerializedFS(),
      };
      this._liveTraceFile.fs.mkdir(
        import_path2.default.dirname(this._liveTraceFile.file),
      );
      const data =
        this._traceEvents.map((e) => JSON.stringify(e)).join("\n") + "\n";
  ...
  maybeGenerateNextTraceRecordingPath() {
    if (
      this._didFinishTestFunctionAndAfterEachHooks &&
      this._shouldAbandonTrace()
    )
      return;
  ...
  _shouldAbandonTrace() {
    ...
    return (
      !testFailed &&
      (this._options.mode === "retain-on-failure" ||
        this._options.mode === "retain-on-first-failure")
    );
  }
  async stopIfNeeded() {
    ...
    if (this._options?.sources) {
      ...
      for (const sourceFile of sourceFiles) {
        await import_fs2.default.promises
          .readFile(sourceFile, "utf8")
          .then((source) => {
            zipFile.addBuffer(
              Buffer.from(source),
              "resources/src@" + calculateSha12(sourceFile) + ".txt",
            );
          })
          .catch(() => {});
      }
    ...
    for (const event of this._traceEvents.filter((e) => e.type === "after")) {
      for (const attachment of event.attachments || []) {
        let contentPromise;
        if (attachment.path)
          contentPromise = import_fs2.default.promises
    ...
    await new Promise((resolve, reject) => {
      zipFile.end(void 0, () => {
        zipFile.outputStream
          .pipe(
            import_fs2.default.createWriteStream(
              this._generateNextTraceRecordingPath(),
            ),
          )
          .on("close", resolve)
          .on("error", reject);
  ...
  _appendTraceEvent(event) {
    ...
    if (this._liveTraceFile)
      this._liveTraceFile.fs.appendFile(
        this._liveTraceFile.file,
...
async function mergeTraceFiles(fileName, temporaryTraceFiles) {
  temporaryTraceFiles = temporaryTraceFiles.filter((file) =>
    import_fs2.default.existsSync(file),
  );
  ...
  zipFile.end(void 0, () => {
    zipFile.outputStream
      .pipe(import_fs2.default.createWriteStream(fileName))
      .on("close", () => {
        void Promise.all(
          temporaryTraceFiles.map((tempFile) =>
            import_fs2.default.promises.unlink(tempFile),
          ),
        )
          .then(() => {
...
var {
  captureRawStack,
  stringifyStackFrames: stringifyStackFrames2,
  filteredStackTrace: filteredStackTrace3,
} = require("playwright-core/lib/coreBundle").utils;
var { escapeWithQuotes: escapeWithQuotes2 } =
  require("playwright-core/lib/coreBundle").iso;
var { monotonicTime: monotonicTime3 } =
  require("playwright-core/lib/coreBundle").iso;
var { createGuid: createGuid2 } =
  require("playwright-core/lib/coreBundle").utils;
var { sanitizeForFilePath: sanitizeForFilePath2, trimLongString } =
  require("playwright-core/lib/coreBundle").utils;
...
var TestInfoImpl = class {
  constructor(
    ...
    this._snapshotNames = {
      lastAnonymousSnapshotIndex: 0,
      lastNamedSnapshotIndex: {},
    };
    this._ariaSnapshotNames = {
      lastAnonymousSnapshotIndex: 0,
      lastNamedSnapshotIndex: {},
    };
    ...
    this.outputDir = (() => {
      const relativeTestFilePath = import_path3.default.relative(
        this.project.testDir,
        this._requireFile.replace(
          /\.(spec|test)\.(js|ts|jsx|tsx|mjs|mts|cjs|cts)$/,
          "",
        ),
      );
      const sanitizedRelativePath = relativeTestFilePath.replace(
      ...
      let testOutputDir = trimLongString(
        sanitizedRelativePath +
          "-" +
          sanitizeForFilePath2(fullTitleWithoutSpec),
    ...
    this.snapshotDir = (() => {
      ...
      return import_path3.default.join(
        this.project.snapshotDir,
        relativeTestFilePath + "-snapshots",
      );
    })();
    this._attachmentsPush = this.attachments.push.bind(this.attachments);
    const attachmentsPush = (...attachments) => {
      for (const a of attachments) this._attach(a, this._parentStep()?.stepId);
    ...
    this.skip = import_common2.transform.wrapFunctionWithLocation(
      (location, ...args) => this._modifier("skip", location, args),
    );
    this.fixme = import_common2.transform.wrapFunctionWithLocation(
      (location, ...args) => this._modifier("fixme", location, args),
    );
    this.fail = import_common2.transform.wrapFunctionWithLocation(
      (location, ...args) => this._modifier("fail", location, args),
    );
    this.slow = import_common2.transform.wrapFunctionWithLocation(
      (location, ...args) => this._modifier("slow", location, args),
    );
  }
  get error() {
  ...
  _deadline() {
    return {
      deadline: this._timeoutManager.currentSlotDeadline(),
  ...
  _findLastPredefinedStep(steps) {
    for (let i = steps.length - 1; i >= 0; i--) {
      ...
      if (
        (steps[i].category === "hook" || steps[i].category === "fixture") &&
        !steps[i].endWallTime
      )
        return steps[i];
    }
  ...
  _parentStep() {
    return (
      currentZone().data("stepZone") ??
      this._findLastPredefinedStep(this._steps)
    );
  }
...
${stringifyStackFrames2(step.boxedStack).join("\n")}`;
        ...
        if (result.shouldNotRetryTest) this._hasNonRetriableError = true;
        if (!step.error) {
        ...
        if (!step.group) {
          const payload = {
            ...
            error: step.error
              ? import_common2.ipc.toTestInfoErrorPayload(step.error)
              : void 0,
        ...
        if (step.group !== "internal") {
          const errorForTrace = step.error
            ...
            : void 0;
          const attachments = step.attachmentIndices.map(
            (i) => this.attachments[i],
          );
          this._tracing.appendAfterActionForStep(
            stepId,
            errorForTrace,
            attachments,
            step.info.annotations,
          );
        }
      },
    };
  ...
  _failWithError(root) {
    ...
    const visit = (error) => {
      ...
      const step =
        error === root && typeof error === "object"
          ? error?.[stepSymbol]
          : void 0;
...
${stringifyStackFrames2(step.boxedStack).join("\n")}`;
  ...
  async _runWithTimeout(runnable, cb) {
    try {
      ...
      if (
        !this._interruptedPromise.isDone() &&
        error instanceof TimeoutManagerError
      )
  ...
  _currentHookType() {
    ...
    return ["beforeAll", "afterAll", "beforeEach", "afterEach"].includes(type)
      ? type
      : void 0;
  }
  ...
  async _didFinishTestFunction() {
    const shouldPause =
      (this._workerParams.pauseAtEnd && !this._isFailure()) ||
      (this._workerParams.pauseOnError && this._isFailure());
    if (shouldPause) {
      await Promise.race([
        this._callbacks.onTestPaused({
          testId: this.testId,
          errors: this._isFailure()
            ? this.errors.map(import_common2.ipc.toTestInfoErrorPayload)
            : [],
          status: this.status,
        }),
    ...
    for (const cb of this._onDidFinishTestFunctionCallbacks) await cb();
  }
  // ------------ TestInfo methods ------------
  ...
  _attach(attachment, stepId) {
    ...
    if (step) {
      ...
      this._tracing.appendBeforeActionForStep({
        stepId: stepId2,
        title: `Attach ${escapeWithQuotes2(attachment.name, '"')}`,
        category: "test.attach",
        stack: [],
      });
  ...
  _resolveSnapshotPaths(kind, name, updateSnapshotIndex, anonymousExtension) {
    const snapshotNames =
      kind === "aria" ? this._ariaSnapshotNames : this._snapshotNames;
    const defaultExtensions = {
      aria: ".aria.yml",
      screenshot: ".png",
      snapshot: ".txt",
    };
    const ariaAwareExtname = (filePath) =>
      kind === "aria" && filePath.endsWith(".aria.yml")
        ? ".aria.yml"
    ...
    if (!name) {
      ...
      subPath = sanitizeFilePathBeforeExtension(
        trimLongString(fullTitleWithoutSpec) + ext,
        ext,
      );
      relativeOutputPath = sanitizeFilePathBeforeExtension(
      ...
      const index =
        (snapshotNames.lastNamedSnapshotIndex[relativeOutputPath] || 0) + 1;
      if (updateSnapshotIndex === "updateSnapshotIndex")
        snapshotNames.lastNamedSnapshotIndex[relativeOutputPath] = index;
      if (index > 1)
        relativeOutputPath = addSuffixToFilePath(
          relativeOutputPath,
          `-${index - 1}`,
        );
    }
    ...
    if (kind === "screenshot") {
      template =
        this._projectInternal.expect?.toHaveScreenshot?.pathTemplate ||
        this._projectInternal.snapshotPathTemplate ||
        legacyTemplate;
    } else if (kind === "aria") {
      const ariaDefaultTemplate =
        "{snapshotDir}/{testFileDir}/{testFileName}-snapshots/{arg}{ext}";
      template =
        this._projectInternal.expect?.toMatchAriaSnapshot?.pathTemplate ||
        this._projectInternal.snapshotPathTemplate ||
        ariaDefaultTemplate;
    } else {
      template = this._projectInternal.snapshotPathTemplate || legacyTemplate;
    }
    const nameArgument = import_path3.default.join(
      import_path3.default.dirname(subPath),
      import_path3.default.basename(subPath, ext),
    );
    const absoluteSnapshotPath = this._applyPathTemplate(
      template,
      nameArgument,
      ext,
    );
  ...
  _applyPathTemplate(template, nameArgument, ext) {
    ...
    const parsedRelativeTestFilePath =
      import_path3.default.parse(relativeTestFilePath);
  ...
  snapshotPath(...args) {
    ...
    if (!["snapshot", "screenshot", "aria"].includes(kind))
      throw new Error(
        `testInfo.snapshotPath: unknown kind "${kind}", must be one of "snapshot", "screenshot" or "aria"`,
      );
    return this._resolveSnapshotPaths(
      kind,
...
var TestStepInfoImpl = class {
  ...
  async _runStepBody(skip, body, location) {
    ...
    try {
      ...
      if (e instanceof StepSkipError) return void 0;
      throw e;
    }
  }
  async attach(name, options) {
    this._testInfo._attach(
      await normalizeAndSaveAttachment(
        this._testInfo.outputPath(),
        name,
        options,
      ),
      this._stepId,
...
var { filteredStackTrace: filteredStackTrace4 } =
  require("playwright-core/lib/coreBundle").utils;
var WorkerMain = class extends import_common3.ProcessRunner {
  constructor(params) {
    ...
    process.stdout.write = (chunk, cb) => {
      this.dispatchEvent(
        "stdOut",
        import_common3.ipc.stdioChunkToParams(chunk),
      );
  ...
  async gracefullyClose() {
    try {
      ...
      const fakeTestInfo = new TestInfoImpl(
        this._config,
        this._project,
        this._params,
        void 0,
      ...
      await fakeTestInfo
        ._runWithTimeout(runnable, () => this._loadIfNeeded())
        .catch(() => {});
      await this._fixtureRunner
        .teardownScope("test", fakeTestInfo, runnable)
        .catch(() => {});
      await this._fixtureRunner
        .teardownScope("worker", fakeTestInfo, runnable)
        .catch(() => {});
      await fakeTestInfo
        ._runWithTimeout(runnable, () => gracefullyCloseAll())
        .catch(() => {});
    ...
    if (this._fatalErrors.length) {
      this._appendProcessTeardownDiagnostics(
        this._fatalErrors[this._fatalErrors.length - 1],
      );
      const payload = {
        fatalErrors: this._fatalErrors.map(
          import_common3.ipc.toTestInfoErrorPayload,
        ),
      };
  ...
  _appendProcessTeardownDiagnostics(error) {
    ...
    const count =
      this._totalRunningTests === 1
        ? "1 test"
        : `${this._totalRunningTests} tests`;
    let lastMessage = "";
    ...
    const message = [
      ...
      ...this._lastRunningTests.map((test) =>
        formatTestTitle(test, this._project.project.name),
      ),
    ].join("\n");
    if (error.message) {
      if (error.stack) {
        ...
        if (index !== -1) {
          ...
          error.stack =
            error.stack.substring(0, index) +
            message +
            error.stack.substring(index);
        }
      }
  ...
  unhandledError(error) {
    ...
    const shouldContinueInThisWorker =
      this._currentTest.expectedStatus === "failed" && isExpectError;
  ...
  async _loadIfNeeded() {
    ...
    const config = await import_common3.configLoader.deserializeConfig(
      this._params.config,
    );
    const project = config.projects.find(
      (p) => p.id === this._params.projectId,
    );
    if (!project)
      throw new Error(
        `Project "${this._params.projectId}" not found in the worker process. Make sure project name does not change.`,
      );
    this._config = config;
    this._project = project;
    this._poolBuilder = import_common3.poolBuilder.PoolBuilder.createForWorker(
      this._project,
    );
  ...
  async runTestGroup(runPayload) {
    ...
    try {
      ...
      const fileSuite = await import_common3.testLoader.loadTestFile(
        runPayload.file,
        this._config,
      );
      const suite = import_common3.suiteUtils.bindFileSuiteToProject(
        this._project,
        fileSuite,
      );
      if (this._params.repeatEachIndex)
        import_common3.suiteUtils.applyRepeatEachIndex(
          this._project,
          suite,
          this._params.repeatEachIndex,
        );
      import_common3.suiteUtils.filterTestsRemoveEmptySuites(suite, (test) =>
        entries.has(test.id),
      );
      ...
      for (const test of tests) unknownTestIds.delete(test.id);
      ...
      const donePayload = {
        ...
        fatalUnknownTestIds,
        stoppedDueToUnhandledErrorInTestFail:
          this._stoppedDueToUnhandledErrorInTestFail,
      };
  ...
  async customMessage(payload) {
    try {
      ...
      const response = await this._currentTest._onCustomMessageCallback?.(
        payload.request,
      );
      return { response };
    } catch (error) {
      return {
        response: {},
        error: import_common3.ipc.toTestInfoErrorPayload(testInfoError(error)),
      };
    }
  }
  resume(payload) {
  ...
  async _runTest(test, retry, nextTest) {
    ...
    (0, import_expect.setExpectConfig)({
      ...
      toMatchAriaSnapshot:
        testInfo._projectInternal.expect?.toMatchAriaSnapshot,
    ...
    await (async () => {
      ...
      await testInfo._runAsStep(
        ...
        async () => {
          ...
          const params = await this._fixtureRunner.resolveParametersForFunction(
            test.fn,
            testInfo,
            "test",
            { type: "test" },
          );
          if (params !== null) testFunctionParams = params.result;
        },
      );
    ...
    testInfo._allowSkips = true;
    const afterHooksTimeout = calculateMaxTimeout(
      this._project.project.timeout,
      testInfo.timeout,
    );
    const afterHooksSlot = { timeout: afterHooksTimeout, elapsed: 0 };
    await testInfo
      ._runAsStep({ title: "After Hooks", category: "hook" }, async () => {
        let firstAfterHooksError;
        try {
          await testInfo._runWithTimeout(
            { type: "test", slot: afterHooksSlot },
            () => testInfo._didFinishTestFunction(),
          );
        } catch (error) {
    ...
    if (this._isStopped) {
      ...
      await testInfo
        ._runAsStep({ title: "Worker Cleanup", category: "hook" }, async () => {
    ...
    await testInfo
      ._runWithTimeout({ type: "test", slot: tracingSlot }, async () => {
        await testInfo._tracing.stopIfNeeded();
      })
      .catch(() => {});
    testInfo.duration =
      (testInfo._timeoutManager.defaultSlot().elapsed +
        afterHooksSlot.elapsed) |
      0;
    this._currentTest = null;
    ...
    const preserveOutput =
      this._config.config.preserveOutput === "always" ||
      (this._config.config.preserveOutput === "failures-only" &&
        testInfo._isFailure());
    if (!preserveOutput) await removeFolders([testInfo.outputDir]);
  }
  _collectHooksAndModifiers(suite, type, testInfo) {
    const runnables = [];
    for (const modifier of suite._modifiers) {
      const modifierType = this._fixtureRunner.dependsOnWorkerFixturesOnly(
        modifier.fn,
        modifier.location,
      )
        ? "beforeAll"
      ...
      const fn = async (fixtures3) => {
        ...
        testInfo._modifier(modifier.type, modifier.location, [
          !!result,
  ...
  async _runBeforeAllHooksForSuite(suite, testInfo) {
    ...
    await this._runAllHooksForSuite(
      suite,
      testInfo,
      "beforeAll",
      extraAnnotations,
  ...
  async _runAllHooksForSuite(suite, testInfo, type, extraAnnotations) {
    ...
    for (const hook of this._collectHooksAndModifiers(suite, type, testInfo)) {
      try {
        await testInfo._runAsStep(
          { title: hook.title, category: "hook", location: hook.location },
          async () => {
            const timeSlot = {
              timeout: this._project.project.timeout,
              elapsed: 0,
            ...
            try {
              await this._fixtureRunner.resolveParametersAndRunFunction(
                hook.fn,
                testInfo,
                "all-hooks-only",
                runnable,
              ...
              if (extraAnnotations) {
                const newAnnotations = testInfo.annotations.filter(
                  (a) => !existingAnnotations.has(a),
                );
                extraAnnotations.push(...newAnnotations);
              }
              await this._fixtureRunner.teardownScope(
                "test",
                testInfo,
                runnable,
              );
        ...
        firstError = firstError ?? error;
        if (type === "beforeAll" && error instanceof TestSkipError) break;
  ...
  async _runEachHooksForSuites(suites, type, testInfo, slot) {
    let firstError;
    const hooks = suites
      .map((suite) => this._collectHooksAndModifiers(suite, type, testInfo))
      .flat();
    for (const hook of hooks) {
      ...
      try {
        ...
        firstError = firstError ?? error;
        if (error instanceof TestSkipError) break;
      }
    }
    if (firstError) throw firstError;
  }
