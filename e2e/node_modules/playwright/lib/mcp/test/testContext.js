...
const { ManualPromise, signalToPromise } =
...
class GeneratorJournal {
  ...
  logStep(title, code) {
    if (title) this._steps.push({ title, code });
  }
  journal() {
    const result = [];
    result.push(`# Plan`);
    ...
    result.push(
      `# Seed file: ${toPosixPath(import_path.default.relative(this._rootPath, this._seed.file))}`,
    );
    result.push("```ts");
...
class TestContext {
  constructor(clientInfo, configPath, options) {
    ...
    this._configLocation = import_common.configLoader.resolveConfigLocation(
      configPath || clientInfo.cwd,
    );
    this.rootPath = clientInfo.cwd || this._configLocation.configDir;
    if (options?.headless !== void 0) this.computedHeaded = !options.headless;
    else
      this.computedHeaded =
        !process.env.CI &&
        !(import_os.default.platform() === "linux" && !process.env.DISPLAY);
  }
  async _enqueue(fn) {
  ...
  async _cleanupTestRunner() {
    if (!this._testRunnerAndScreen) return;
  ...
  async getOrCreateSeedFile(seedFile, projectName) {
    ...
    if (!seedFile) {
      ...
      if (!resolvedSeedFile) throw new Error("seed test not found.");
      seedFile = resolvedSeedFile;
    }
    const seedFileContent = await import_fs.default.promises.readFile(
      seedFile,
      "utf8",
    );
    return {
  ...
  async _runTestsImpl(params, signal) {
    ...
    claimStdio();
    try {
      const setupReporter = new MCPListReporter({
        configDir,
        screen,
        includeTestId: true,
      });
      ...
      if (status2 !== "passed")
        return {
          output: testRunnerAndScreen.output.join("\n"),
          status: status2,
        };
    } finally {
      releaseStdio();
    ...
    const cleanup = async () => {
      ...
      try {
        ...
        if (status === "passed") status = result.status;
      } finally {
        releaseStdio();
      }
    };
    ...
    const abortPromise = abort
      ? abort.promise.then(() => "interrupted")
      : new Promise(() => {});
    try {
      const reporter = new MCPListReporter({
        configDir,
        screen,
        includeTestId: true,
      });
      ...
      if (status === "paused") {
        const response = await testRunnerAndScreen.sendMessageToPausedTest({
          request: { initialize: { clientInfo: this._clientInfo } },
        });
        if (response.error) throw new Error(response.error.message);
        testRunnerAndScreen.output.push(
          response.response.initialize.pausedMessage,
        );
        return { output: testRunnerAndScreen.output.join("\n"), status };
      }
    } catch (e) {
  ...
  async close() {
    await this._enqueue(() => this._cleanupTestRunner()).catch((e) =>
      debug("pw:mcp:error")(e),
    );
  }
  async sendMessageToPausedTest(request) {
    ...
    if (typeof request?.callTool?.arguments?.["intent"] === "string") {
      const response = import_coreBundle.tools.parseResponse(
        result.response.callTool,
      );
      if (response && !response.isError && response.code)
        this.generatorJournal?.logStep(
          request.callTool.arguments["intent"],
          response.code,
        );
    }
