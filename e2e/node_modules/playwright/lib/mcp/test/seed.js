...
function seedProject(fullConfig, projectName) {
  ...
  const project = fullConfig.projects.find(
...
async function findSeedFile(project) {
  const files =
    await import_runner.projectUtils.collectFilesForProject(project);
  return files.find((file) =>
    import_path.default.basename(file).includes("seed"),
  );
}
