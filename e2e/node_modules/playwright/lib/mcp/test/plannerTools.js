  ...
  schema: {
    ...
    inputSchema: z.object({
      project: z
        ...
        .optional()
        .describe(
          'Project to use for setup. For example: "chromium", if no project is provided uses the first project in the config.',
        ),
      seedFile: z
        .string()
        .optional()
        .describe(
          'A seed file contains a single test that is used to setup the page for testing, for example: "tests/seed.spec.ts". If no seed file is provided, a default seed file is created.',
        ),
    }),
    type: "readOnly",
  },
  handle: async (context, params, signal) => {
    const seed = await context.getOrCreateSeedFile(
      params.seedFile,
      params.project,
    );
    const { output, status } = await context.runSeedTest(
      seed.file,
      seed.projectName,
      signal,
    );
...
const planSchema = z.object({
  overview: z
    .string()
    .describe("A brief overview of the application to be tested"),
  suites: z.array(
    z.object({
      name: z.string().describe("The name of the suite"),
      seedFile: z
        .string()
        .describe("A seed file that was used to setup the page for testing."),
      tests: z.array(
        z.object({
          ...
          file: z
            .string()
            .describe(
              'The file the test should be saved to, for example: "tests/<suite-name>/<test-name>.spec.ts".',
            ),
          steps: z.array(
            z.object({
              perform: z
                ...
                .optional()
                .describe(
                  `Action to perform. For example: 'Click on the "Submit" button'.`,
                ),
              expect: z
                .string()
                .array()
                .describe(
                  `Expected result of the action where appropriate. For example: 'The page should show the "Thank you for your submission" message'`,
                ),
            }),
          ),
        }),
...
const saveTestPlan = (0, import_testTool.defineTestTool)({
  schema: {
    ...
    inputSchema: planSchema.extend({
      name: z
        .string()
        .describe('The name of the test plan, for example: "Test Plan".'),
      fileName: z
        .string()
        .describe(
          'The file to save the test plan to, for example: "spec/test.plan.md". Relative to the workspace root.',
        ),
    }),
    type: "readOnly",
  },
  handle: async (context, params) => {
    ...
    if (!resolvedFile)
      throw new Error(
        `Plan file name must be a relative path inside the workspace: ${params.fileName}`,
      );
    await import_fs.default.promises.mkdir(
      import_path.default.dirname(resolvedFile),
      { recursive: true },
    );
