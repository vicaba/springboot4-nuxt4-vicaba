...
function createCustomMessageHandler(testInfo, context) {
  ...
  return async (data) => {
    if (!tools) ({ tools } = require("playwright-core/lib/coreBundle"));
    const toolList = tools.filteredTools(config);
    if (data.initialize) {
      if (backend) throw new Error("MCP backend is already initialized");
      backend = new tools.BrowserBackend(config, context, toolList);
      await backend.initialize(data.initialize.clientInfo);
      const pausedMessage = await generatePausedMessage(
        tools,
        testInfo,
        context,
      );
    ...
    if (data.callTool) {
      if (!backend) throw new Error("MCP backend is not initialized");
      return {
        callTool: await backend.callTool(
          data.callTool.name,
          data.callTool.arguments,
        ),
      };
    }
    if (data.close) {
...
async function generatePausedMessage(tools, testInfo, context) {
  ...
  for (let i = 0; i < context.pages().length; i++) {
    ...
    let console2 = testInfo.errors.length
      ? await tools.Tab.collectConsoleMessages(page)
      : [];
    console2 = console2.filter((msg) => msg.type === "error");
    if (console2.length) {
      lines.push("- Console Messages:");
      for (const message of console2) lines.push(`  - ${message.toString()}`);
    }
    lines.push(
      `- Page Snapshot:`,
      "```yaml",
...
async function runDaemonForContext(testInfo, context) {
  if (testInfo._configInternal.configCLIOverrides.debug !== "cli") return false;
  const sessionName = `tw-${import_crypto.default.randomBytes(3).toString("hex")}`;
  await context
    .browser()
    .bind(sessionName, { workspaceDir: testInfo.project.testDir });
  console.log(
    [
