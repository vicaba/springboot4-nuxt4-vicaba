...
const { isPathInside, resolveWithinRoot } =
...
const setupPage = (0, import_testTool.defineTestTool)({
  schema: {
    ...
    inputSchema: z.object({
      plan: z
        .string()
        .describe(
          "The plan for the test. This should be the actual test plan with all the steps.",
        ),
      project: z
        .string()
        .optional()
  ...
  handle: async (context, params, signal) => {
    ...
    context.generatorJournal = new import_testContext.GeneratorJournal(
      context.rootPath,
      params.plan,
      seed,
    );
...
const generatorReadLog = (0, import_testTool.defineTestTool)({
  ...
  handle: async (context) => {
    if (!context.generatorJournal)
      throw new Error(
        `Please setup page using "${setupPage.schema.name}" first.`,
      );
...
const generatorWriteTest = (0, import_testTool.defineTestTool)({
  ...
  handle: async (context, params) => {
    ...
    if (!testRunner)
      throw new Error(
        "No test runner found, please setup page and perform actions first.",
      );
...
Test written to ${params.fileName}`,
      ...
      dirs.push(
        import_path.default
          .relative(context.rootPath, projectTestDir)
          .replace(/\\/g, "/"),
      );
    }
    throw new Error(
      `Test file did not match any of the test dirs: ${dirs.join(", ")}`,
    );
  },
});
