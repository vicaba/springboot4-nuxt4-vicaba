...
class TestServerBackend extends import_events.default {
  ...
  async callTool(name, args, signal) {
    const tool = testServerBackendTools.find(
      (tool2) => tool2.schema.name === name,
    );
    if (!tool)
      throw new Error(
        `Tool not found: ${name}. Available tools: ${testServerBackendTools.map((tool2) => tool2.schema.name).join(", ")}`,
      );
    try {
...
function wrapBrowserTool(tool) {
  const inputSchema = typesWithIntent.includes(tool.schema.type)
    ? tool.schema.inputSchema.extend({
        intent: zod
          .string()
          .describe(
            "The intent of the call, for example the test step description plan idea",
          ),
      })
  ...
  return {
    ...
    handle: async (context, params, _signal) => {
      const response = await context.sendMessageToPausedTest({
        callTool: { name: tool.schema.name, arguments: params },
      });
