...
const listTests = (0, import_testTool.defineTestTool)({
  ...
  handle: async (context) => {
    ...
    const reporter = new import_runner.ListModeReporter({
      screen,
      includeTestId: true,
    });
  ...
  schema: {
    ...
    inputSchema: z.object({
      locations: z
        ...
        .optional()
        .describe(
          'Folder, file or location to run: "test/e2e" or "test/e2e/file.spec.ts" or "test/e2e/file.spec.ts:20"',
        ),
      projects: z
        .array(z.string())
        .optional()
        .describe(
          'Projects to run, projects from playwright.config.ts, by default runs all projects. Running with "chromium" is a good start',
        ),
    }),
    type: "readOnly",
  },
...
const debugTest = (0, import_testTool.defineTestTool)({
    ...
    inputSchema: z.object({
      test: z.object({
        ...
        title: z
          .string()
          .describe(
            "Human readable test title for granting permission to debug the test.",
          ),
      }),
    }),
    type: "readOnly",
  ...
  handle: async (context, params, signal) => {
    const { output, status } =
      await context.runTestsWithGlobalSetupAndPossiblePause(
        {
    ...
    return {
      ...
      isError: status !== "paused" && status !== "passed",
    };
  },
});
